...
This document describes how to build `libktx`, the portable
KTX loader tests and the KTX tools.

Status
------

Construction of a new build system and portable loader tests is
underway.  At present the iOS, Mac OS X, Linux and Windows builds of
...
The project includes separate make/project/solution files for the
...
The KTX source distribution contains make, project and solution files generated
with GYP.
...
Use the solutions under one of `build/msvs/win/vs20{08,10,10e,13,13e,15}` to build the
library, load tests and tools for Windows Win32 or x64.

Use the make files under `build/make/linux` to build the library, load tests
and tools for Linux.
