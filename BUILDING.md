...
load tests to run on iOS using OpenGL ES 1.1 and OpenGL ES 3.0.

Use the generated projects under `build/xcode/mac` to build the library,
load tests and tools to run under OpenGL 3.3 on OS X.
...
automatically added to the project.pbxproj file. So, as noted above, changes
should be made only to the GYP files.
