...
ktxValidator::ktxValidator() :
    ktxApp(myversion, mydefversion, options) {
...
void ktxValidator::usage() {
	cerr << "Usage: " << name << " [options] [<infile> ...]\n"
	                             "\n"
...
int ktxValidator::main(int argc, _TCHAR *argv[]) {
...
void ktxValidator::validateFile(const _tstring &filename) {
...
void ktxValidator::validateHeader(validationContext &ctx) {
	...
	if (ctx.header.supercompressionScheme > KTX_SS_BEGIN_VENDOR_RANGE && ctx.header.supercompressionScheme < KTX_SS_END_VENDOR_RANGE) {
	...
	switch (ctx.header.supercompressionScheme) {
				...
				if (KHR_DFDVAL(bdb, MODEL) == KHR_DF_MODEL_UASTC) {
					...
					if (numSamples > 1)
						addIssue(logger::eError, DFD.InvalidSampleCount,
						         "UASTC", "1");
					if (KHR_DFDVAL(bdb, TEXELBLOCKDIMENSION0) != 3 && KHR_DFDVAL(bdb, TEXELBLOCKDIMENSION1) != 3 && (bdb[KHR_DF_WORD_TEXELBLOCKDIMENSION0] & 0xffff0000) != 0)
					...
					if (channelID != KHR_DF_CHANNEL_UASTC_RGB && channelID != KHR_DF_CHANNEL_UASTC_RGBA && channelID != KHR_DF_CHANNEL_UASTC_RRR && channelID != KHR_DF_CHANNEL_UASTC_RRRG)
					...
					// Check the basics
					if (KHR_DFDVAL(bdb, VENDORID) != KHR_DF_VENDORID_KHRONOS || KHR_DFDVAL(bdb, DESCRIPTORTYPE) != KHR_DF_KHR_DESCRIPTORTYPE_BASICFORMAT || KHR_DFDVAL(bdb, VERSIONNUMBER) < KHR_DF_VERSIONNUMBER_1_3)
		...
		case KTX_SS_BASIS_LZ:
			...
			for (uint32_t sample = 0; sample < numSamples; sample++) {
				...
				if (channelID != KHR_DF_CHANNEL_ETC1S_RGB && channelID != KHR_DF_CHANNEL_ETC1S_RRR && channelID != KHR_DF_CHANNEL_ETC1S_GGG && channelID != KHR_DF_CHANNEL_ETC1S_AAA)
...
bool ktxValidator::validateMetadata(validationContext &ctx, const char *key,
...
#define CALL_MEMBER_FN(object, ptrToMember) ((object)->*(ptrToMember))
	...
	for (it = metadataValidators.begin(); it < metadataValidators.end(); it++) {
		if (!it->name.compare(key)) {
			...
			CALL_MEMBER_FN(this, it->validateFunc)
			(ctx, key, pValue, valueLen);
			break;
		}
	}
...
void ktxValidator::validateCubemapIncomplete(validationContext &ctx,
                                             const char        *key,
                                             const uint8_t *,
                                             uint32_t valueLen) {
...
void ktxValidator::validateOrientation(validationContext &ctx,
                                       const char        *key,
                                       const uint8_t     *value,
                                       uint32_t           valueLen) {
	if (valueLen == 0) {
...
void ktxValidator::validateGlFormat(validationContext & /*ctx*/,
                                    const char *key,
                                    const uint8_t * /*value*/,
                                    uint32_t valueLen) {
...
void ktxValidator::validateDxgiFormat(validationContext & /*ctx*/,
                                      const char *key,
                                      const uint8_t * /*value*/,
                                      uint32_t valueLen) {
...
void ktxValidator::validateMetalPixelFormat(validationContext & /*ctx*/,
                                            const char *key,
                                            const uint8_t * /*value*/,
                                            uint32_t valueLen) {
	if (valueLen != sizeof(uint32_t))
...
void ktxValidator::validateSwizzle(validationContext & /*ctx*/,
                                   const char    *key,
                                   const uint8_t *value,
                                   uint32_t       valueLen) {
	string      swizzle;
...
void ktxValidator::validateWriter(validationContext & /*ctx*/,
                                  const char    *key,
                                  const uint8_t *value,
                                  uint32_t       valueLen) {
	if (value[valueLen - 1] != '\0')
...
void ktxValidator::validateWriterScParams(validationContext & /*ctx*/,
                                          const char    *key,
                                          const uint8_t *value,
                                          uint32_t       valueLen) {
	if (value[valueLen - 1] != '\0')
...
void ktxValidator::validateAstcDecodeMode(validationContext &ctx,
                                          const char        *key,
                                          const uint8_t     *value,
                                          uint32_t           valueLen) {
	if (valueLen == 0) {
...
void ktxValidator::validateAnimData(validationContext &ctx,
                                    const char        *key,
                                    const uint8_t * /*value*/,
                                    uint32_t valueLen) {
...
void ktxValidator::validateSgd(validationContext &ctx) {
	...
	uint64_t expectedBgdByteLength = sizeof(ktxBasisLzGlobalHeader) + sizeof(ktxBasisLzEtc1sImageDesc) * imageCount + bgdh.endpointsByteLength + bgdh.selectorsByteLength + bgdh.tablesByteLength;
