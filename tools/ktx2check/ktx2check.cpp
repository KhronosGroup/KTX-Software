...
struct {
    ...
    issue UnknownFormat{WARNING | 0x0022,
                        "vkFormat, %#x is unknown, possibly an extension format."};
    issue WidthZero{ERROR | 0x0023, "pixelWidth is 0. Textures must have width."};
    issue DepthNoHeight{ERROR | 0x0024,
    ...
    issue TooManyMipLevels{ERROR | 0x0028,
                           "%d is too many levels for the largest image dimension %d."};
    issue VendorSupercompression{WARNING | 0x0029,
                                 "Using vendor supercompressionScheme. Can't validate."};
    issue InvalidSupercompression{ERROR | 0x002a, "Invalid supercompressionScheme: %#x"};
    issue InvalidOptionalIndexEntry{ERROR | 0x002b,
                                    "Invalid %s index entry. Only 1 of offset & length != 0."};
    issue InvalidRequiredIndexEntry{ERROR | 0x002c,
                                    "Index for required entry has offset or length == 0."};
    issue InvalidDFDOffset{ERROR | 0x002d,
                           "Invalid dfdByteOffset. DFD must immediately follow level index."};
    issue InvalidKVDOffset{ERROR | 0x002e,
                           "Invalid kvdByteOffset. KVD must immediately follow DFD."};
    issue InvalidSGDOffset{ERROR | 0x002f, "Invalid sgdByteOffset. SGD must follow KVD."};
    issue TypeSizeMismatch{ERROR | 0x0030,
                           "typeSize, %d, does not match data described by the DFD."};
    issue VkFormatAndBasis{
        ERROR | 0x0031,
        "VkFormat must be VK_FORMAT_UNDEFINED for supercompressionScheme BASIS_LZ."};
    issue TypeSizeNotOne{ERROR | 0x0032,
                         "typeSize for a block compressed or supercompressed format must be 1."};
    issue ZeroLevelCountForBC{ERROR | 0x0033,
                              "levelCount must be > 0 for block-compressed formats."};
} HeaderData;
...
struct {
    ...
    issue IncorrectModelForBlock{ERROR | 0x0052,
                                 "DFD color model is not that of a block-compressed texture."};
    issue MultiplePlanes{ERROR | 0x0053,
    ...
    issue UnsignedFloat{ERROR | 0x0055,
                        "DFD says data is unsigned float but there are no such texture formats."};
    issue FormatMismatch{ERROR | 0x0056,
                         "DFD does not match VK_FORMAT w.r.t. sign, float or normalization."};
    ...
    issue BytesPlane0Zero{ERROR | 0x005a,
                          "DFD bytesPlane0 must be non-zero for non-supercompressed %s texture."};
    ...
    issue IncorrectModelForBLZE{ERROR | 0x005d,
                                "DFD colorModel for BasisLZ/ETC1S must be KHR_DF_MODEL_ETC1S."};
    issue InvalidTexelBlockDimension{ERROR | 0x005e,
                                     "DFD texel block dimension must be %dx%d for %s textures."};
    ...
    issue InvalidBitOffsetForUASTC{ERROR | 0x0065,
    ...
    issue InvalidColorModel{ERROR | 0x0067,
                            "DFD colorModel for non block-compressed textures must be RGBSDA."};
    issue MixedChannels{ERROR | 0x0068,
...
struct {
    issue IncorrectByteLength{ERROR | 0x0070,
                              "Level %d byteLength %#x does not match expected value %#x."};
    issue ByteOffsetTooSmall{ERROR | 0x0071,
                             "Level %d byteOffset %#x is smaller than expected value %#x."};
    issue IncorrectByteOffset{ERROR | 0x0072,
                              "Level %d byteOffset %#x does not match expected value %#x."};
    issue IncorrectUncompressedByteLength{
        ERROR | 0x0073, "Level %d uncompressedByteLength %#x does not match expected value %#x."};
    issue UnequalByteLengths{ERROR | 0x0074,
                             "Level %d uncompressedByteLength does not match byteLength."};
    issue UnalignedOffset{ERROR | 0x0075,
                          "Level %d byteOffset is not aligned to required %d byte alignment."};
...
struct {
    ...
    issue InvalidStructure{
        ...
        " after %d KV pairs."};
    issue MissingFinalPadding{ERROR | 0x0084,
                              "Required valuePadding after last metadata value missing."};
    ...
    issue IllegalMetadata{ERROR | 0x0087,
                          "Unrecognized metadata \"%s\" found with KTX or ktx prefix found."};
...
struct {
    ...
    issue IncorrectGlobalDataSize{ERROR | 0x0093,
    ...
    issue DfdMismatchNoAlpha{ERROR | 0x0096,
                             "supercompressionGlobalData indicates an alpha channel but DFD "
                             "indicates no alpha channel."};
} SGD;
  ...
  protected:
    ...
    struct validationContext {
        ...
        size_t kvDataEndOffset() {
            return sizeof(KTX_header2) + levelIndexSize + header.dataFormatDescriptor.byteLength +
    ...
    bool validateMetadata(validationContext& ctx, const char* key, const uint8_t* value,
                          uint32_t valueLen);

    typedef void (ktxValidator::*validateMetadataFunc)(validationContext& ctx, const char* key,
                                                       const uint8_t* value, uint32_t valueLen);
    void validateCubemapIncomplete(validationContext& ctx, const char* key, const uint8_t* value,
                                   uint32_t valueLen);
    void validateOrientation(validationContext& ctx, const char* key, const uint8_t* value,
                             uint32_t valueLen);
    void validateGlFormat(validationContext& ctx, const char* key, const uint8_t* value,
                          uint32_t valueLen);
    void validateDxgiFormat(validationContext& ctx, const char* key, const uint8_t* value,
                            uint32_t valueLen);
    void validateMetalPixelFormat(validationContext& ctx, const char* key, const uint8_t* value,
                                  uint32_t valueLen);
    void validateSwizzle(validationContext& ctx, const char* key, const uint8_t* value,
                         uint32_t valueLen);
    void validateWriter(validationContext& ctx, const char* key, const uint8_t* value,
                        uint32_t valueLen);
    void validateWriterScParams(validationContext& ctx, const char* key, const uint8_t* value,
                                uint32_t valueLen);
    void validateAstcDecodeMode(validationContext& ctx, const char* key, const uint8_t* value,
                                uint32_t valueLen);
    void validateAnimData(validationContext& ctx, const char* key, const uint8_t* value,
                          uint32_t valueLen);
...
#define checkOptionalIndexEntry(index, issue, name) \
    ...
    checkRequiredIndexEntry(ctx.header.dataFormatDescriptor, HeaderData.InvalidRequiredIndexEntry,
                            "dfd");
    ...
    for (int32_t level = ctx.levelCount - 1; level >= 0; level--) {
        if (ctx.header.vkFormat != VK_FORMAT_UNDEFINED &&
            ...
            if (actualUBL != expectedUBL)
                addIssue(logger::eError, LevelIndex.IncorrectUncompressedByteLength, level,
                         actualUBL, expectedUBL);
            ...
            if (actualByteOffset != expectedByteOffset) {
                if (actualByteOffset % requiredLevelAlignment != 0)
                    addIssue(logger::eError, LevelIndex.UnalignedOffset, level,
                ...
                if (levelIndex[level].byteOffset > expectedByteOffset)
                    ...
                    addIssue(logger::eError, LevelIndex.ByteOffsetTooSmall, level, actualByteOffset,
                             expectedByteOffset);
            }
        } else {
            ...
            if (levelIndex[level].byteOffset != expectedOffset) {
                addIssue(logger::eError, LevelIndex.IncorrectByteOffset, level,
                         levelIndex[level].byteOffset, expectedOffset);
            }
        ...
        ctx.dataSizeFromLevelIndex +=
            padn(ctx.requiredLevelAlignment(), levelIndex[level].byteLength);
    }
    delete[] levelIndex;
}
        ...
        if (ctx.header.vkFormat != VK_FORMAT_UNDEFINED) {
            ...
            if (KHR_DFDVAL(bdb, MODEL) == KHR_DF_MODEL_UASTC) {
                ...
                if (KHR_DFDSVAL(bdb, 0, BITLENGTH) != 127)
                    addIssue(logger::eError, DFD.InvalidBitLength, "UASTC", 127);
                ...
                if (ctx.header.supercompressionScheme == KTX_SS_NONE) {
                    if (KHR_DFDVAL(bdb, BYTESPLANE0) == 0)
                        addIssue(logger::eError, DFD.BytesPlane0Zero, "VK_FORMAT_UNDEFINED");
                } else {
                    if (KHR_DFDVAL(bdb, BYTESPLANE0) != 0) {
                        addIssue(logger::eError, DFD.NotUnsized);
                    }
    ...
    case KTX_SS_BASIS_LZ:
        ...
        for (uint32_t sample = 0; sample < numSamples; sample++) {
            ...
            if (KHR_DFDSVAL(bdb, sample, BITLENGTH) != 63)
                addIssue(logger::eError, DFD.InvalidBitLength, "BasisLZ/ETC1S", 63);
...
bool ktxValidator::validateMetadata(validationContext& ctx, const char* key, const uint8_t* pValue,
                                    uint32_t valueLen) {
...
void ktxValidator::validateCubemapIncomplete(validationContext& ctx, const char* key,
                                             const uint8_t*, uint32_t valueLen) {
    ctx.cubemapIncompleteFound = true;
    if (valueLen != 1) addIssue(logger::eError, Metadata.InvalidValue, key);
}

void ktxValidator::validateOrientation(validationContext& ctx, const char* key,
                                       const uint8_t* value, uint32_t valueLen) {
    if (valueLen == 0) {
...
void ktxValidator::validateGlFormat(validationContext& /*ctx*/, const char* key,
                                    const uint8_t* /*value*/, uint32_t valueLen) {
    if (valueLen != sizeof(uint32_t) * 3) addIssue(logger::eError, Metadata.InvalidValue, key);
}

void ktxValidator::validateDxgiFormat(validationContext& /*ctx*/, const char* key,
                                      const uint8_t* /*value*/, uint32_t valueLen) {
    if (valueLen != sizeof(uint32_t)) addIssue(logger::eError, Metadata.InvalidValue, key);
}

void ktxValidator::validateMetalPixelFormat(validationContext& /*ctx*/, const char* key,
                                            const uint8_t* /*value*/, uint32_t valueLen) {
    if (valueLen != sizeof(uint32_t)) addIssue(logger::eError, Metadata.InvalidValue, key);
}

void ktxValidator::validateSwizzle(validationContext& /*ctx*/, const char* key,
                                   const uint8_t* value, uint32_t valueLen) {
    string swizzle;
...
void ktxValidator::validateWriter(validationContext& /*ctx*/, const char* key, const uint8_t* value,
                                  uint32_t valueLen) {
    if (value[valueLen - 1] != '\0')
        addIssue(logger::eWarning, Metadata.ValueNotNulTerminated, key);
}

void ktxValidator::validateWriterScParams(validationContext& /*ctx*/, const char* key,
                                          const uint8_t* value, uint32_t valueLen) {
    if (value[valueLen - 1] != '\0')
        addIssue(logger::eWarning, Metadata.ValueNotNulTerminated, key);
}

void ktxValidator::validateAstcDecodeMode(validationContext& ctx, const char* key,
                                          const uint8_t* value, uint32_t valueLen) {
    if (valueLen == 0) {
        addIssue(logger::eError, Metadata.MissingValue, key);
        return;
...
void ktxValidator::validateAnimData(validationContext& ctx, const char* key,
                                    const uint8_t* /*value*/, uint32_t valueLen) {
    if (ctx.cubemapIncompleteFound) {
        addIssue(logger::eError, Metadata.NotAllowed, key, "together with KTXcubemapIncomplete");
    }
    if (ctx.layerCount == 0)
        addIssue(logger::eError, Metadata.NotAllowed, key, "except with array textures");
...
void ktxValidator::validateSgd(validationContext& ctx) {
    ...
    for (uint32_t level = 1; level <= ctx.levelCount; level++) {
        ...
        firstImages[level] =
            firstImages[level - 1] + layersFaces * MAX(ctx.header.pixelDepth >> (level - 1), 1);
    }
...
bool ktxValidator::validateTranscode(validationContext& ctx) {
    ...
    ktx_error_code_e result = ktxTexture2_CreateFromStream(
        ktx2Stream.stream(), KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, texture2.pHandle());
    if (result != KTX_SUCCESS) {
