...
int ktxSupercompressor::main(int argc, char* argv[]) {
    ...
    for (it = options.infiles.begin(); it < options.infiles.end(); it++) {
        ...
        if (inf) {
            ...
            if (outf) {
                result = ktxTexture2_CreateFromStdioStream(
                ...
                if ((options.astc || options.etc1s || options.bopts.uastc) &&
                ...
                if (!options.outfile.length() && !options.useStdout) {
                    ...
                    if (!MoveFileExW(DecodeUTF8Path(tmpfile).c_str(),
                                     DecodeUTF8Path(infile).c_str(), MOVEFILE_REPLACE_EXISTING))
#else
                    if (rename(tmpfile.c_str(), infile.c_str()))
#endif
                    {
                        cerr << name << ": rename of \"" << tmpfile << "\" to \"" << infile
            ...
            cerr << name << " could not open input file \""
                 << (infile.compare("-") == 0 ? "stdin" : infile) << "\". " << strerror(errno)
                 << endl;
            exitCode = 2;
            goto cleanup;
        }
