...
ktxSupercompressor::ktxSupercompressor() :
...
int ktxSupercompressor::main(int argc, _TCHAR *argv[]) {
	FILE          *inf, *outf;
...
cleanup:
	if (pTmpFile)
		(void) _tunlink(pTmpFile);
	if (options.outfile.length())
	...
	switch (opt) {
		...
		case 'o':
			...
			if (!options.outfile.compare(_T("stdout"))) {
				...
				// a file extension.
				if (dot == _tstring::npos || (slash != _tstring::npos && dot < slash)) {
