...
class color_base {
  public:
	static uint32_t getComponentCount() {
		return componentCount;
	}
	static uint32_t getComponentSize() {
	...
	static uint32_t one() {
		return ((1 << sizeof(componentType) * 8) - 1);
	}
};
...
struct vec3_base {
	...
	vec3_base(float _r, float _g, float _b) :
	    r(_r), g(_g), b(_b) {
	}
	void base_normalize() {
...
struct vec3 : public vec3_base {
	...
	explicit vec3(float _r) :
	    vec3_base(_r, 0.0f, 0.0f) {
	}
	vec3(float _r, float _g) :
	    vec3_base(_r, _g, 0.0f) {
	}
...
class Image {
  public:
	...
	enum rescale_e {
		eNoRescale,
		eAlwaysRescaleTo8Bits,
		eRescaleTo8BitsIfLess
	};
	...
	colortype_e getColortype() {
		return this->colortype;
	}
	void setColortype(colortype_e t) {
		colortype = t;
	}
	khr_df_transfer_e getOetf() const {
		return oetf;
	}
	void setOetf(khr_df_transfer_e noetf) {
		this->oetf = noetf;
	}
	...
	virtual void     resample(Image &dst, bool srgb = false,
	                          ...
	                          basisu::Resampler::Boundary_Op wrapMode     = basisu::Resampler::Boundary_Op::BOUNDARY_CLAMP) = 0;
  ...
  protected:
	...
	Image(uint32_t w, uint32_t h) :
	    width(w), height(h), oetf(KHR_DF_TRANSFER_UNSPECIFIED), primaries(KHR_DF_PRIMARIES_BT709) {
	}
...
class ImageT : public Image {
  ...
  public:
	...
	virtual void resample(Image &abstract_dst, bool srgb, const char *pFilter,
		...
		if (::maximum(src_w, src_h) > BASISU_RESAMPLER_MAX_DIMENSION || ::maximum(dst_w, dst_h) > BASISU_RESAMPLER_MAX_DIMENSION) {
	...
	virtual ImageT &copyToR(Image &dst) {
		return copyTo((ImageT<componentType, 1> &) dst);
	}
	virtual ImageT &copyToRG(Image &dst) {
		return copyTo((ImageT<componentType, 2> &) dst);
	}
	virtual ImageT &copyToRGB(Image &dst) {
		return copyTo((ImageT<componentType, 3> &) dst);
	}
	virtual ImageT &copyToRGBA(Image &dst) {
		return copyTo((ImageT<componentType, 4> &) dst);
	}
...
class rg8image : public ImageT<uint8_t, 2> {
  public:
	...
	rg8image(uint32_t w, uint32_t h) :
	    MyImageT(w, h) {
	}
...
class rgb8image : public ImageT<uint8_t, 3> {
  public:
	...
	rgb8image(uint32_t w, uint32_t h) :
	    MyImageT(w, h) {
	}
...
class rgba8image : public ImageT<uint8_t, 4> {
  public:
	...
	rgba8image(uint32_t w, uint32_t h) :
	    MyImageT(w, h) {
	}
...
class r16image : public ImageT<uint16_t, 1> {
  public:
	...
	r16image(uint32_t w, uint32_t h) :
	    MyImageT(w, h) {
	}
...
class rg16image : public ImageT<uint16_t, 2> {
  public:
	...
	rg16image(uint32_t w, uint32_t h) :
	    ImageT(w, h) {
	}
...
class rgb16image : public ImageT<uint16_t, 3> {
  public:
	...
	rgb16image(uint32_t w, uint32_t h) :
	    MyImageT(w, h) {
	}
...
class rgba16image : public ImageT<uint16_t, 4> {
  public:
	...
	rgba16image(uint32_t w, uint32_t h) :
	    MyImageT(w, h) {
	}
