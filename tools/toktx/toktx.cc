...
#if IMAGE_DEBUG
static void dumpImage(char* name, int width, int height, int components, int componentSize,
...
| | Target type | R | RG | RGB | RGBA | RGB | RGBA | RGB | RGBA | | Input swizzle | - | - | - | - |
rrr1 | rrrg | - | - | | Swizzle | rrr1 | rrrg | - | - | - | - | - | - |
...
class toktxApp : public scApp {
  ...
  protected:
    ...
    void genMipmap(unique_ptr<Image> pImage, uint32_t layer, uint32_t faceSlice,
                   ktxTexture* texture);
    ...
    void determineTargetTypeBitLengthScale(const ImageInput& in, targetImageSpec& target,
                                           string& defaultSwizzle);

    void determineTargetImageSpec(const ImageInput& in, targetImageSpec& target,
                                  string& defaultSwizzle);
...
toktxApp::toktxApp() : scApp(myversion, mydefversion, options) {
    argparser::option my_option_list[] = {
        ...
        {"lower_left_maps_to_s0t0", argparser::option::no_argument,
         &options.lower_left_maps_to_s0t0, 1},
        {"upper_left_maps_to_s0t0", argparser::option::no_argument,
         &options.lower_left_maps_to_s0t0, 0},
        {"linear", argparser::option::no_argument, (int*)&options.assign_oetf,
...
int toktxApp::main(int argc, char* argv[]) {
        ...
        try {
            ...
            do {      // subimages
                do {  // miplevels
                    ...
                    ret = ktxTexture_SetImageFromMemory(texture, level, layer, faceSlice, *image,
                                                        image->getByteCount());
            ...
            cerr << name << ": could not create image from " << infile << "(" << subimage << ","
                 << miplevel << ")." << endl
                 << e.what() << endl;
            ...
            cerr << name << ": failed to create image from " << infile << "(" << subimage << ","
                 << miplevel << ")." << endl
                 << e.what() << endl;
            exitCode = 2;
            goto cleanup;
    ...
    if (layer != texture->numLayers) {
        cerr << name << ": too few input images for " << levelCount << " levels, "
             << texture->numLayers << " layers and " << texture->numFaces << " faces." << endl;
        exitCode = 1;
        goto cleanup;
    }
    ...
    if (options.metadata) {
        ...
        if (options.ktx2) {
            ...
            if (texture->numDimensions == 1) {
                ...
                snprintf(orientation, sizeof(orientation), KTX_ORIENTATION2_FMT, 'r',
                         options.lower_left_maps_to_s0t0 ? 'u' : 'd');
            } else
                snprintf(orientation, sizeof(orientation), KTX_ORIENTATION3_FMT, 'r',
                         options.lower_left_maps_to_s0t0 ? 'u' : 'd',
                         options.lower_left_maps_to_s0t0 ? 'o' : 'i');
        }
        ktxHashList_AddKVPair(ht, KTX_ORIENTATION_KEY, (unsigned int)strlen(orientation) + 1,
                              orientation);
    }
    if (options.ktx2) {
        ...
        ktxHashList_AddKVPair(&texture->kvDataHead, KTX_WRITER_KEY,
                              (ktx_uint32_t)writer.str().length() + 1, writer.str().c_str());
    ...
    if (f) {
        if (options.astc || options.etc1s || options.bopts.uastc || options.zcmp) {
            string& swizzle =
                options.inputSwizzle.size() == 0 && defaultSwizzle.size() && !options.normalMode
                    ? defaultSwizzle
                    : options.inputSwizzle;
            exitCode =
                encode((ktxTexture2*)texture, swizzle, f == stdout ? "stdout" : options.outfile);
    ...
    closefileandcleanup:
        ...
        cerr << name << ": "
             << "could not open output file \"" << options.outfile << "\". " << strerror(errno)
             << endl;
        exitCode = 2;
    }
...
unique_ptr<Image> toktxApp::createImage(const targetImageSpec& target, ImageInput& in) {
    ...
    if (inputFormat.channelBitLength() == 16) {
        ...
        message << "Unsupported format " << inSpec.format().channelBitLength() << "-bit " << dtq
                << " needed.";
        throw runtime_error(message.str());
        // TODO: uint32, uint64, float etc.
    }

    in.readImage(static_cast<uint8_t*>(*image), image->getByteCount(), 0 /*subimage*/,
                 0 /*miplevel*/, inputFormat);
    /* Sanity check. */
    ...
    if (target.dstTransferFunction != nullptr) {
        ...
        if (target.dstColorPrimaries != nullptr) {
            ...
            auto primaryTransform =
                target.srcColorPrimaries->transformTo(*target.dstColorPrimaries);
    ...
    if (options.scale != 1.0f) {
        ...
        image = scaleImage(std::move(image), static_cast<ktx_uint32_t>(scaledWidth),
                           static_cast<ktx_uint32_t>(scaledHeight));
    } else if (options.resize &&
               (image->getWidth() != target.width() || image->getHeight() != target.height())) {
...
unique_ptr<Image> toktxApp::scaleImage(unique_ptr<Image> pImage, ktx_uint32_t width,
                                       ktx_uint32_t height) {
    try {
        pImage = pImage->resample(width, height, options.gmopts.filter.c_str(),
...
void toktxApp::genMipmap(unique_ptr<Image> pImage, uint32_t layer, uint32_t faceSlice,
                         ktxTexture* texture) {
    ...
    for (uint32_t glevel = 1; glevel < texture->numLevels; glevel++) {
        ...
        try {
            levelImage = pImage->resample(levelWidth, levelHeight, options.gmopts.filter.c_str(),
                                          options.gmopts.filterScale, options.gmopts.wrapMode);
        } catch (runtime_error& e) {
            stringstream message;
            message << "Image::resample() failed! " << e.what();
        ...
        ret = ktxTexture_SetImageFromMemory(texture, glevel, layer, faceSlice, *levelImage,
...
ktxTexture* toktxApp::createTexture(const targetImageSpec& target) {
    ...
    if (options.ktx2) {
        ret = ktxTexture2_Create(&createInfo, KTX_TEXTURE_CREATE_ALLOC_STORAGE,
                                 (ktxTexture2**)&texture);
    } else {
        ret = ktxTexture1_Create(&createInfo, KTX_TEXTURE_CREATE_ALLOC_STORAGE,
                                 (ktxTexture1**)&texture);
    }
    if (KTX_SUCCESS != ret) {
        stringstream message;
        ...
        if (spec.format().transfer() != KHR_DF_TRANSFER_UNSPECIFIED) {
            ...
            if (spec.format().oeGamma() > .45450f && spec.format().oeGamma() < .45460f) {
                ...
                if (options.convert_oetf == KHR_DF_TRANSFER_UNSPECIFIED) {
                    stringstream message;
                    message << "Its encoding gamma, " << spec.format().oeGamma()
...
void toktxApp::determineTargetTypeBitLengthScale(const ImageInput& in, targetImageSpec& target,
                                                 string& defaultSwizzle) {
    ...
    if (bitLength != format.largestChannelBitLength()) {
        warning("Rescaling %d-bit image in %s to %d bits.", format.channelBitLength(),
                in.filename().c_str(), targetFormat.channelBitLength());
    }
    ...
    if (targetFormat.anyChannelBitLengthNotEqual(bitLength) ||
        maxValue != targetFormat.channelUpper() || channelCount != targetFormat.channelCount()) {
        targetFormat.updateSampleInfo(channelCount, bitLength, 0, maxValue,
                                      targetFormat.channelDataType());
    }
}

void toktxApp::determineTargetImageSpec(const ImageInput& in, targetImageSpec& target,
                                        string& defaultSwizzle) {
    target = in.spec();
    if (options.scale != 1.0f) {
...
khr_df_primaries_e toktxApp::parseColorPrimaries(string& argValue) {
    ...
    if (argValue.length()) {
        ...
        if (it != values.end()) {
            ...
            cerr << name << "Invalid or unsupported transfer function specified: " << argValue
                 << endl;
            exit(1);
        }
    }
...
bool toktxApp::processOption(argparser& parser, int opt) {
    ...
    case 1101:
        ...
        break;
    case 1102:
        for_each(parser.optarg.begin(), parser.optarg.end(),
                 [](char& c) { c = (char)::toupper(c); });
...
static void dumpImage(char* name, int width, int height, int components, int componentSize,
    ...
    sprintf(formatstr, "%s%d w=%%d h=%%d t=\'%s %s%d\' %%p", imagefmt, bitsPerComponent, name,
            fmtname, bitsPerComponent);
