...
[[nodiscard]] constexpr inline bool starts_with(std::string_view string,
...
[[nodiscard]] constexpr inline bool is_sorted(const Range& range, Comp&& comp = {},
                                              Proj&& proj = {}) {
...
inline void replace_all_inplace(std::string& string, std::string_view search,
...
[[nodiscard]] inline std::string replace_all_copy(std::string string, std::string_view search,
                                                  std::string_view replace) {
...
/**
 * @internal
 * @brief Safely checks and advances a UTF-8 sequence iterator to the start of the next unicode
 * codepoint
 * @param[in] it iterator to be advanced
 * @param[in] end iterator pointing to the end of the range
 * @return True if the advance operation was successful and the advanced codepoint was a valid UTF-8
 * sequence */
 ...
 * @return nullopt if the sequence is valid otherwise the first index where an invalid UTF-8
 * character was found */
[[nodiscard]] constexpr inline std::optional<std::size_t> validateUTF8(
