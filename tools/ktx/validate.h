...
struct BDFD {
    ...
    [[nodiscard]] bool matchTexelBlockDimensions(uint8_t dim0, uint8_t dim1, uint8_t dim2, uint8_t dim3) const {
        return texelBlockDimension0 == dim0 && texelBlockDimension1 == dim1 && texelBlockDimension2 == dim2 &&
               texelBlockDimension3 == dim3;
    }

    [[nodiscard]] bool hasNonZeroBytePlane() const {
        return bytesPlanes[0] != 0 || bytesPlanes[1] != 0 || bytesPlanes[2] != 0 || bytesPlanes[3] != 0 || bytesPlanes[4] != 0 ||
               bytesPlanes[5] != 0 || bytesPlanes[6] != 0 || bytesPlanes[7] != 0;
    }
};
