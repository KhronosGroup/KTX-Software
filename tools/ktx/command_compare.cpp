...
struct Diff : public DiffBase<T> {
    ...
    Diff(const std::string_view textHeaderIn, const std::string_view jsonPathIn, const T& value1,
         const T& value2)
        : DiffBase<T>(textHeaderIn, jsonPathIn, value1, value2) {}
...
struct DiffIdentifier : public DiffBase<std::array<uint8_t, sizeof(KTX_header2::identifier)>> {
    DiffIdentifier(const std::string_view textHeaderIn, const std::string_view jsonPathIn,
        ...
        : DiffBase<std::array<uint8_t, sizeof(KTX_header2::identifier)>>(
              textHeaderIn, jsonPathIn, to_array(value1), to_array(value2)) {}
  ...
  private:
    static std::array<uint8_t, sizeof(KTX_header2::identifier)> to_array(
        const KTX_header2& header) {
...
struct DiffHex : public DiffBase<T> {
    ...
    DiffHex(const std::string_view textHeaderIn, const std::string_view jsonPathIn, const T& value1,
            const T& value2)
        : DiffBase<T>(textHeaderIn, jsonPathIn, value1, value2) {}
...
struct DiffHexFixedWidth : public DiffBase<T> {
    ...
    virtual std::string value(std::size_t index, OutputFormat format) const override {
        if (format == OutputFormat::text) {
            return fmt::format(fmt::format("0x{{:0{}x}}", sizeof(T) << 1),
                               DiffBase<T>::rawValue(index));
        } else {
            return fmt::format("{}", DiffBase<T>::rawValue(index));
        }
    }
...
struct DiffEnum<ktxSupercmpScheme> : public DiffBase<ktxSupercmpScheme> {
    DiffEnum(const std::string_view textHeaderIn, const std::string_view jsonPathIn,
        ...
        : DiffBase<ktxSupercmpScheme>(textHeaderIn, jsonPathIn, ktxSupercmpScheme(value1),
                                      ktxSupercmpScheme(value2)) {
        ...
        if (format == OutputFormat::text) {
            if (enumNames[index]) {
                if (invalidScheme)
                    ...
                    return fmt::format("Vendor or reserved scheme (0x{:x})",
                                       uint32_t(rawValue(index)));
                else
                    return enumNames[index];
            } else {
...
struct DiffFlags : DiffBase<uint32_t> {
    ...
    virtual std::string value(std::size_t index, OutputFormat format) const override {
        ...
        if (format == OutputFormat::text) {
            return fmt::format("0x{:x} ({})", DiffBase<uint32_t>::rawValue(index),
                               formattedValue.str());
        } else {
...
struct DiffArray : public DiffBase<std::array<T, N>> {
    DiffArray(const std::string_view textHeaderIn, const std::string_view jsonPathIn,
              const std::optional<std::array<T, N>>& value1,
    ...
    virtual std::string value(std::size_t index, OutputFormat format) const override {
        ...
        for (std::size_t arrayIndex = 0; arrayIndex < N; ++arrayIndex) {
            ...
            fmt::print(formattedValue, "{}{}", comma,
                       DiffBase<std::array<T, N>>::rawValue(index)[arrayIndex]);
        }
...
struct DiffRawBytes : public DiffBase<std::vector<uint8_t>> {
    DiffRawBytes(const std::string_view textHeaderIn, const std::string_view jsonPathIn,
                 const std::optional<std::vector<uint8_t>>& value1,
                 const std::optional<std::vector<uint8_t>>& value2)
...
struct DiffComplex {
    ...
    DiffComplex(const std::string_view textHeaderIn, const std::string_view jsonPathIn,
        ...
        : textHeader(textHeaderIn),
          jsonPath(jsonPathIn),
          values(),
          different(value1.different(value2)) {
        values[0] = value1;
        values[1] = value2;
    }
    ...
    void printJson(std::size_t index, PrintIndent& out, int indent, const char* space,
                   const char* nl) const {
...
struct DiffImage {
    using TexelBlockPairList =
        std::vector<std::pair<ImageSpan::TexelBlockPtr<>, ImageSpan::TexelBlockPtr<>>>;

    DiffImage(const std::string_view textHeaderIn, const std::string_view fragmentUriIn,
              const std::optional<std::size_t>& fileOffset1,
              const std::optional<std::size_t>& fileOffset2,
              const TexelBlockPairList& texelBlockPairListIn)
        : textHeader(textHeaderIn),
          fragmentUri(fragmentUriIn),
          texelBlockPairList(texelBlockPairListIn) {
...
class PrintDiff {
  ...
  public:
    ...
    void operator<<(const DiffImage& diff) {
        ...
        auto formatOptionalFileOffset = [](const std::optional<std::size_t>& fileOffset,
                                           std::size_t imageOffset, bool json) {
        ...
        auto formatPacked = [=](const ImageSpan::TexelBlockPtr<>& texelBlock, bool json) {
            ...
            std::stringstream formattedValue;
            bool first = true;
            for (uint32_t elementIdx = 0; elementIdx < texelBlock.getPackedElementCount();
                 ++elementIdx) {
                const uint32_t element = texelBlock.getPackedElement(elementIdx);
                const auto comma = std::exchange(first, false) ? "" : fmt::format(",{}", space);
                fmt::print(formattedValue, "{}{}0x{:0{}x}{}", comma, quote, element, hexDigits,
                           quote);
            }
            return formattedValue.str();
        };
        ...
        auto formatChannels = [=](const ImageSpan::TexelBlockPtr<>& texelBlock, bool json) {
            // If decodable channels are not available (e.g. block compressed), then this should not
            // be called
            assert(texelBlock.getChannelCount() != 0);
            const auto quote = json ? "\"" : "";
            std::stringstream formattedValue;
            bool first = true;
            // Prefer to decode to integer (e.g. UNORM will be output as list of integer values
            ...
            if (texelBlock.canDecodeUINT()) {
                ...
                for (uint32_t channelIdx = 0; channelIdx < texelBlock.getChannelCount();
                     ++channelIdx) {
                    const auto comma = std::exchange(first, false) ? "" : fmt::format(",{}", space);
                ...
                for (uint32_t channelIdx = 0; channelIdx < texelBlock.getChannelCount();
                    ...
                    if (std::isinf(channel)) {
                        ...
                        // Output signed value (explicitly handle sign bit to differentiate between
                        // +0.0 and -0.0) However, we only output sign for negative values because
        ...
        if (outputFormat == OutputFormat::text) {
            ...
            for (const auto& texelBlockPair : diff.texelBlockPairList) {
                ...
                auto printDiff = [=](const char* textHeader, const std::string firstValue,
                ...
                printDiff("Packed", formatPacked(texelBlockPair.first, false),
                          formatPacked(texelBlockPair.second, false));
            ...
            for (const auto& texelBlockPair : diff.texelBlockPairList) {
                ...
                printIndent(4, "\"fileByteOffset\":{}[{}{},{}{}{}],{}", space, space,
                            formatOptionalFileOffset(diff.fileOffsets[0], imageByteOffset, true),
                            space,
                            formatOptionalFileOffset(diff.fileOffsets[1], imageByteOffset, true),
                            space, nl);

                printIndent(4, "\"packed\":{}[{}", space, nl);
                printIndent(5, "[{}{}{}],{}", space, formatPacked(texelBlockPair.first, true),
                            space, nl);
                printIndent(5, "[{}{}{}]{}", space, formatPacked(texelBlockPair.second, true),
                            space, nl);
                printIndent(4, "]");
                ...
                if (!texelBlockPair.first.isBlockCompressed()) {
                    ...
                    printIndent(5, "[{}{}{}],{}", space, formatChannels(texelBlockPair.first, true),
                                space, nl);
                    printIndent(5, "[{}{}{}]{}", space, formatChannels(texelBlockPair.second, true),
                                space, nl);
                    printIndent(4, "]");
                }
...
/>
        ...
        <dd>Ignore the vkFormat and typeSize fields in the file header. <br />
            Note: useful when comparing textures with and without BasisLZ encoding,
respectively.</dd> <dt>\--ignore-supercomp</dt> <dd>Ignore supercompression scheme in the file
header. <br /> Note: use the --ignore-sgd option to also ignore the SGD section, if needed.</dd>
...
/>
        ...
        <dd>Controls the comparison of the SGD section. Possible options are: <br />
            ...
            @b payload - Ignore any unrecognized SGD section and the payload of any known SGD
section. <br />
...
class CommandCompare : public Command {
    ...
    struct OptionsCompare {
        ...
        void init(cxxopts::Options& opts) {
            opts.add_options()("input-file1", "The first input file to compare.",
                               cxxopts::value<std::string>(),
                               "filepath")("input-file2", "The second input file to compare.",
                ...
                "Note: When the mode is set to @b image, the effective image data of individual "
                "mip "
                "levels, layers, and faces is compared texel block by texel block after applying "
                ...
                "For block compressed textures and textures encoded using UASTC the texel blocks "
                "are "
                "the individual compressed blocks, while for other formats the texel blocks are "
                "the "
                ...
                "files with non-supercompressed ones, as bytesPlane is set to zero for "
                ...
                "  payload: Ignore any unrecognized SGD section and the payload of any known SGD "
                "section\n"
        ...
        void process(cxxopts::Options&, cxxopts::ParseResult& args, Reporter& report) {
            ...
            if (args[kPerPixelOutput].count()) {
                if (contentMode != ContentMode::image)
                    report.fatal_usage(
                        "--per-pixel-output is specified but --content was not set to \"image\".");
                const auto perPixelOutputStr =
                    to_lower_copy(args[kPerPixelOutput].as<std::string>());
                if (perPixelOutputStr == "all") {
                    ...
                    try {
                        perPixelOutputLimit = static_cast<std::size_t>(
                            std::stoull(perPixelOutputStr, &parsedCharacters));
                    } catch (std::exception&) {
                    }
                    if (parsedCharacters != perPixelOutputStr.length())
                        report.fatal_usage("Invalid --per-pixel-output arugment: \"{}\".",
                                           perPixelOutputStr);
                }
            }
            ...
            if (args[kIgnoreMetadata].count()) {
                static std::unordered_map<std::string, bool> ignoreMetadataMapping{{"all", true},
                                                                                   {"none", false}};
                const auto ignoreMetadataStr =
                    to_lower_copy(args[kIgnoreMetadata].as<std::string>());
  ...
  private:
    ...
    void compareKVEntry(PrintDiff& diff, const std::string_view& key, ktxHashListEntry* entry1,
    ...
    void read(InputStream& stream, std::size_t offset, void* readDst, std::size_t readSize,
              std::string_view what) {
        stream->seekg(offset);
        if (stream->fail())
            fatal(rc::IO_FAILURE, "Failed to seek file to {} \"{}\": {}.", what, stream.str(),
                  errnoMessage());

        stream->read(reinterpret_cast<char*>(readDst), readSize);
        if (stream->eof()) {
            fatal(rc::IO_FAILURE, "Unexpected end of file reading {} from file \"{}\".", what,
                  stream.str());
        } else if (stream->fail()) {
            fatal(rc::IO_FAILURE, "Failed to read {} from file \"{}\": {}.", what, stream.str(),
                  errnoMessage());
        }
    }
    ...
    static bool compareTexelBlocksPacked(ImageSpan::TexelBlockPtr<> texelBlocks[2]) {
        ...
        assert(texelBlocks[0].imageCodec().getPackedElementCount() ==
               texelBlocks[1].imageCodec().getPackedElementCount());
        assert(texelBlocks[0].imageCodec().getPackedElementByteSize() ==
               texelBlocks[1].imageCodec().getPackedElementByteSize());
        for (uint32_t elementIdx = 0;
             elementIdx < texelBlocks[0].imageCodec().getPackedElementCount(); ++elementIdx) {
            if (texelBlocks[0].getPackedElement(elementIdx) !=
                texelBlocks[1].getPackedElement(elementIdx)) {
                return false;
            }
        }
        return true;
    ...
    case OutputFormat::text: {
        for (std::size_t i = 0; i < inputStreams.size(); ++i) {
            ...
            validationResults.emplace_back(
                validateIOStream(inputStreams[i], fmtInFile(options.inputFilepaths[i]), false,
                                 false, [&](const ValidationReport& issue) {
    ...
    case OutputFormat::json_mini: {
        ...
        for (std::size_t i = 0; i < inputStreams.size(); ++i) {
            ...
            validationResults.emplace_back(validateIOStream(
                inputStreams[i], fmtInFile(options.inputFilepaths[i]), false, false,
...
#define OPT_BITFIELD(struct, member)                                                       \
    ((struct).has_value() ? std::optional<int32_t>(static_cast<int32_t>((struct)->member)) \
...
#define OPT_FIELDS(structArr, member) \
    OPT_FIELD((structArr)[0], member), OPT_FIELD((structArr)[1], member)
#define OPT_BITFIELDS(structArr, member) \
    OPT_BITFIELD((structArr)[0], member), OPT_BITFIELD((structArr)[1], member)
#define OPT_UINT4S(structArr, memberPrefix) \
    OPT_UINT4((structArr)[0], memberPrefix), OPT_UINT4((structArr)[1], memberPrefix)
...
void CommandCompare::compareHeader(PrintDiff& diff, InputStreams& streams) {
    ...
    if (!options.ignoreFormatHeader) {
        diff << DiffEnum<VkFormat>(
            "vkFormat", "/header/vkFormat", headers[0].vkFormat, headers[1].vkFormat,
    ...
    diff << Diff("pixelHeight", "/header/pixelHeight", headers[0].pixelHeight,
                 headers[1].pixelHeight);
    ...
    if (!options.ignoreSupercomp)
        diff << DiffEnum<ktxSupercmpScheme>(
    ...
    if (options.ignoreIndex != IgnoreIndex::all) {
        diff << DiffHex("dataFormatDescriptor.byteOffset", "/index/dataFormatDescriptor/byteOffset",
                        headers[0].dataFormatDescriptor.byteOffset,
                        headers[1].dataFormatDescriptor.byteOffset);
        diff << Diff("dataFormatDescriptor.byteLength", "/index/dataFormatDescriptor/byteLength",
                     headers[0].dataFormatDescriptor.byteLength,
                     headers[1].dataFormatDescriptor.byteLength);
        ...
        diff << DiffHex("supercompressionGlobalData.byteOffset",
                        "/index/supercompressionGlobalData/byteOffset",
                        headers[0].supercompressionGlobalData.byteOffset,
                        headers[1].supercompressionGlobalData.byteOffset);
        diff << Diff("supercompressionGlobalData.byteLength",
                     "/index/supercompressionGlobalData/byteLength",
                     headers[0].supercompressionGlobalData.byteLength,
...
void CommandCompare::compareLevelIndex(PrintDiff& diff, InputStreams& streams) {
    ...
    const uint32_t numLevels[] = {std::max(1u, headers[0].levelCount),
...
void CommandCompare::compareDFD(PrintDiff& diff, InputStreams& streams) {
    ...
    if (options.ignoreDFD != IgnoreDFD::all_except_color_space)
        if (dfdTotalSize[0] != dfdTotalSize[1])
            diff << Diff("DFD total bytes", "/dataFormatDescriptor/totalSize", dfdTotalSize[0],
                         dfdTotalSize[1]);
        ...
        if (compareDFDs) {
            if (options.ignoreDFD != IgnoreDFD::all_except_color_space) {
                diff << DiffEnum<khr_df_vendorid_e>(
                    ...
                    OPT_BITFIELDS(blockHeaders, vendorId), [&](auto i) {
                        return dfdToStringVendorID(khr_df_vendorid_e(blockHeaders[i]->vendorId));
                    });
                diff << DiffEnum<khr_df_khr_descriptortype_e>(
                    ...
                    OPT_BITFIELDS(blockHeaders, descriptorType), [&](auto i) {
                        return (blockHeaders[i]->vendorId == KHR_DF_VENDORID_KHRONOS)
                                   ? dfdToStringDescriptorType(khr_df_khr_descriptortype_e(
                                         blockHeaders[i]->descriptorType))
                                   : nullptr;
                    });
                diff << DiffEnum<khr_df_versionnumber_e>(
                    "Version",
                    fmt::format("/dataFormatDescriptor/blocks/{}/versionNumber", blockIndex),
                    OPT_BITFIELDS(blockHeaders, versionNumber), [&](auto i) {
                        return dfdToStringVersionNumber(
                            khr_df_versionnumber_e(blockHeaders[i]->versionNumber));
                    });
                diff << Diff(
                ...
                for (std::size_t i = 0; i < streams.size(); ++i)
                    if (blockHeaders[i].has_value() &&
                        ...
                        std::vector<SampleType> samples(
                            std::min(MAX_NUM_BDFD_SAMPLES,
                                     (blockHeaders[i]->descriptorBlockSize - 24u) / 16u));
                        std::memcpy(samples.data(), ptrDFDIt[i] + sizeof(BDFD),
            ...
            if (options.ignoreDFD != IgnoreDFD::all_except_color_space)
                if (!dfdKnown[0] || !dfdKnown[1]) {
                    ...
                    for (std::size_t i = 0; i < streams.size(); ++i) {
                        if (blockHeaders[i].has_value()) {
                            rawPayloads[i] = std::vector<uint8_t>(
                                blockHeaders[i]->descriptorBlockSize - sizeof(DFDHeader));
                            std::memcpy(rawPayloads[i]->data(), ptrDFDIt[i],
                                        rawPayloads[i]->size());

                            diff << DiffRawBytes(
                                "Raw payload",
                                fmt::format("/dataFormatDescriptor/blocks/{}/rawPayload",
                                            blockIndex),
...
void CommandCompare::compareDFDBasic(PrintDiff& diff, uint32_t blockIndex,
    ...
    if (options.ignoreDFD != IgnoreDFD::all_except_color_space)
        diff << DiffFlags("Flags", fmt::format("/dataFormatDescriptor/blocks/{}/flags", blockIndex),
                          OPT_BITFIELDS(bdfds, flags), dfdToStringFlagsBit);

    diff << DiffEnum<khr_df_transfer_e>(
        "Transfer", fmt::format("/dataFormatDescriptor/blocks/{}/transferFunction", blockIndex),
    ...
    diff << DiffEnum<khr_df_primaries_e>(
        "Primaries", fmt::format("/dataFormatDescriptor/blocks/{}/colorPrimaries", blockIndex),
    ...
    diff << DiffEnum<khr_df_model_e>(
        "Model", fmt::format("/dataFormatDescriptor/blocks/{}/colorModel", blockIndex),
    ...
    std::size_t maxNumSamples = std::max(bdfdSamples[0].has_value() ? bdfdSamples[0]->size() : 0,
    ...
    for (std::size_t sampleIndex = 0; sampleIndex < maxNumSamples; ++sampleIndex) {
        ...
        diff << DiffFlags("    Qualifiers",
                          fmt::format("/dataFormatDescriptor/blocks/{}/samples/{}/qualifiers",
                                      blockIndex, sampleIndex),
                          qualifierFlags[0], qualifierFlags[1],
                          dfdToStringSampleDatatypeQualifiersBit);

        diff << DiffEnum<khr_df_model_channels_e>(
                    "    Channel Type",
                    fmt::format("/dataFormatDescriptor/blocks/{}/samples/{}/channelType",
                                blockIndex, sampleIndex),
                    OPT_BITFIELDS(samples, channelType),
                    [&](auto i) {
                        return dfdToStringChannelId(
                            khr_df_model_e(bdfds[i]->model),
        ...
        if (options.format == OutputFormat::text) {
            ...
            diff << Diff({},
                         fmt::format("/dataFormatDescriptor/blocks/{}/samples/{}/bitLength",
                                     blockIndex, sampleIndex),
                         OPT_BITFIELDS(samples, bitLength));
            diff << Diff({},
                         fmt::format("/dataFormatDescriptor/blocks/{}/samples/{}/bitOffset",
                                     blockIndex, sampleIndex),
                         OPT_BITFIELDS(samples, bitOffset));
        }

        diff << DiffArray("    Position",
                          fmt::format("/dataFormatDescriptor/blocks/{}/samples/{}/samplePosition",
                                      blockIndex, sampleIndex),
                          OPT_UINT4S(samples, samplePosition));
        diff << DiffHexFixedWidth(
            "    Lower",
            fmt::format("/dataFormatDescriptor/blocks/{}/samples/{}/sampleLower", blockIndex,
                        sampleIndex),
            OPT_FIELDS(samples, lower));
        diff << DiffHexFixedWidth(
            "    Upper",
            fmt::format("/dataFormatDescriptor/blocks/{}/samples/{}/sampleUpper", blockIndex,
                        sampleIndex),
...
void CommandCompare::compareKVD(PrintDiff& diff, InputStreams& streams) {
    ...
    for (std::size_t i = 0; i < streams.size(); ++i) {
        ...
        KTX_error_code result = ktxHashList_Deserialize(
            &kvDataHead, headers[i].keyValueData.byteLength, keyValueStores[i].data());
        ...
        for (; entry != NULL && entryIndex < MAX_NUM_KV_ENTRIES;
             entry = ktxHashList_Next(entry), ++entryIndex) {
            char* key;
            ktx_uint32_t keyLen;
    ...
    std::map<std::string_view, ktxHashListEntry*>::iterator it[2] = {keys[0].begin(),
                                                                     keys[1].begin()};
    while (it[0] != keys[0].end() || it[1] != keys[1].end()) {
        bool ignoreKey = false;
        for (std::size_t i = 0; i < streams.size(); ++i)
            if ((it[i] != keys[i].end()) &&
                (options.ignoreMetadataKeys.find(std::string(it[i]->first)) !=
                 options.ignoreMetadataKeys.end())) {
...
struct KVEntry {
    ...
    static std::optional<T> load(ktxHashListEntry* entry) {
        if (entry) {
            T newEntry;
            ktxHashListEntry_GetValue(entry, &newEntry.valueLen,
                                      reinterpret_cast<void**>(&newEntry.value));
    ...
    T extract(std::size_t offset = 0) const {
        return (valueLen >= offset + sizeof(T)) ? T(*reinterpret_cast<const T*>(value + offset))
                                                : T();
    }
...
void CommandCompare::compareKVEntry(PrintDiff& diff, const std::string_view& key,
    ...
    const std::unordered_set<std::string_view> keysWithUint32Values = {"KTXdxgiFormat__",
                                                                       "KTXmetalPixelFormat"};
    const std::unordered_set<std::string_view> keysWithStringValues = {
        "KTXorientation", "KTXswizzle", "KTXwriter", "KTXwriterScParams", "KTXastcDecodeMode"};
    ...
    if (keysWithUint32Values.find(key) != keysWithUint32Values.end()) {
        ...
        diff << DiffComplex(key, fmt::format("/keyValueData/{}", key),
                            KVEntry::load<KVEntryUint32>(entry1),
        ...
        diff << DiffComplex(key, fmt::format("/keyValueData/{}", key),
                            KVEntry::load<KVEntryString>(entry1),
        ...
        struct KTXglFormat : public KVEntry {
            ...
            void printJson(PrintIndent& out, int indent, const char* space, const char* nl) const {
                if (isValid()) {
                    out(indent, "{{{}", nl);
                    out(indent + 1, "\"glInternalformat\":{}{},{}", space, extract<ktx_uint32_t>(0),
                        nl);
        ...
        diff << DiffComplex("KTXcubemapIncomplete", "/keyValueData/KTXcubemapIncomplete",
                            KVEntry::load<KTXcubemapIncomplete>(entry1),
                            KVEntry::load<KTXcubemapIncomplete>(entry2));
    } else {
        ...
        diff << DiffComplex(key, fmt::format("/keyValueData/{}", key),
                            KVEntry::load<KVEntryUnknown>(entry1),
...
void CommandCompare::compareSGD(PrintDiff& diff, InputStreams& streams) {
    ...
    auto sgdTypeBasisLZ = [&](std::size_t i) {
        return headers[i].supercompressionScheme == KTX_SS_BASIS_LZ;
    };
    ...
    for (std::size_t i = 0; i < streams.size(); ++i) {
        ...
        read(streams[i], headers[i].supercompressionGlobalData.byteOffset, buffers[i].get(),
             sgdByteLength, "the SGD");

        if (sgdTypeBasisLZ(i)) {
            uint32_t imageCount = 0;
            for (uint32_t level = 0; level < std::max(headers[i].levelCount, 1u); ++level)
                // numFaces * depth is only reasonable because they can't both be > 1. There are no
                // 3D cubemaps
                imageCount += std::max(headers[i].layerCount, 1u) * headers[i].faceCount *
                              std::max(headers[i].pixelDepth >> level, 1u);

            if (sgdByteLength < sizeof(ktxBasisLzGlobalHeader)) continue;

            const ktxBasisLzGlobalHeader& bgh =
                *reinterpret_cast<const ktxBasisLzGlobalHeader*>(buffers[i].get());
            ...
            for (uint32_t level = 0; level < std::max(headers[i].levelCount, 1u); ++level) {
                for (uint32_t layer = 0; layer < std::max(headers[i].layerCount, 1u); ++layer) {
                    for (uint32_t face = 0; face < headers[i].faceCount; ++face) {
                        for (uint32_t zSlice = 0;
                             zSlice < std::max(headers[i].pixelDepth >> level, 1u); ++zSlice) {
                            ...
                            basisLZ[i].alphaSliceByteLength.push_back(
                                imageDesc->alphaSliceByteLength);
                            basisLZ[i].alphaSliceByteOffset.push_back(
            ...
            basisLZ[i].endpointsByteOffset =
                sizeof(ktxBasisLzGlobalHeader) + sizeof(ktxBasisLzEtc1sImageDesc) * imageCount;
            basisLZ[i].selectorsByteOffset =
                *basisLZ[i].endpointsByteOffset + *basisLZ[i].endpointsByteLength;
            basisLZ[i].tablesByteOffset =
                *basisLZ[i].selectorsByteOffset + *basisLZ[i].selectorsByteLength;
            basisLZ[i].extendedByteOffset =
                *basisLZ[i].tablesByteOffset + *basisLZ[i].tablesByteLength;
        }
    }
    ...
    auto compareSGDPayload = [&](const char* textName, const char* jsonPath,
        ...
        if (!offset1.has_value() || !length1.has_value() || !offset2.has_value() ||
            !length2.has_value())
            mismatch = true;
        ...
        if (!mismatch) {
            if (length1 != length2) {
                ...
                mismatch = (memcmp(buffers[0].get() + *offset1, buffers[1].get() + *offset2,
                                   *length1) != 0);
            }
        }
    ...
    if (sgdTypeBasisLZ(0) || sgdTypeBasisLZ(1)) {
        ...
        if (options.format != OutputFormat::text)
            diff << DiffEnum<ktxSupercmpScheme>(
                "supercompressionScheme", "/supercompressionGlobalData/type",
                headers[0].supercompressionScheme, headers[1].supercompressionScheme);
        ...
        std::size_t maxImageCount =
            std::max(basisLZ[0].imageFlags.size(), basisLZ[1].imageFlags.size());
        for (std::size_t i = 0; i < streams.size(); ++i) {
        ...
        for (std::size_t imageIndex = 0; imageIndex < maxImageCount; ++imageIndex) {
            diff << DiffFlags(
                fmt::format("Image{}.imageFlags", imageIndex),
                fmt::format("/supercompressionGlobalData/images/{}/imageFlags", imageIndex),
                basisLZ[0].imageFlags[imageIndex], basisLZ[1].imageFlags[imageIndex],
                ktxBUImageFlagsBitString);

            diff << Diff(
                fmt::format("Image{}.rgbSliceByteLength", imageIndex),
                fmt::format("/supercompressionGlobalData/images/{}/rgbSliceByteLength", imageIndex),
                basisLZ[0].rgbSliceByteLength[imageIndex],
                basisLZ[1].rgbSliceByteLength[imageIndex]);

            diff << Diff(
                fmt::format("Image{}.rgbSliceByteOffset", imageIndex),
                fmt::format("/supercompressionGlobalData/images/{}/rgbSliceByteOffset", imageIndex),
                basisLZ[0].rgbSliceByteOffset[imageIndex],
                basisLZ[1].rgbSliceByteOffset[imageIndex]);

            diff << Diff(fmt::format("Image{}.alphaSliceByteLength", imageIndex),
                         fmt::format("/supercompressionGlobalData/images/{}/alphaSliceByteLength",
                                     imageIndex),
                         basisLZ[0].alphaSliceByteLength[imageIndex],
                         basisLZ[1].alphaSliceByteLength[imageIndex]);

            diff << Diff(fmt::format("Image{}.alphaSliceByteOffset", imageIndex),
                         fmt::format("/supercompressionGlobalData/images/{}/alphaSliceByteOffset",
                                     imageIndex),
                         basisLZ[0].alphaSliceByteOffset[imageIndex],
                         basisLZ[1].alphaSliceByteOffset[imageIndex]);
        }
        ...
        compareSGDPayload("SGD", "/supercompressionGlobalData/rawPayload", 0,
                          headers[0].supercompressionGlobalData.byteLength, 0,
                          headers[1].supercompressionGlobalData.byteLength);
    }
}
...
void CommandCompare::compareImagesRaw(PrintDiff& diff, InputStreams& streams) {
    ...
    for (uint32_t level = 0; level < maxNumLevels; ++level) {
        ...
        if (!mismatch) {
            ...
            if (std::memcmp(buffers[0].get(), buffers[1].get(), levelIndexEntry[0]->byteLength) !=
                0)
                mismatch = true;
        }

        if (mismatch)
            diff << DiffMismatch(fmt::format("Mismatch in level {} data", level),
                                 fmt::format("m={}", level));
    }
}
...
void CommandCompare::compareImagesPerPixel(PrintDiff& diff, InputStreams& streams) {
    ...
    for (std::size_t i = 0; i < streams.size(); ++i) {
        auto ret = ktxTexture2_CreateFromStream(
            ktx2Streams[i].stream(), KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, textures[i].pHandle());
        if (ret != KTX_SUCCESS)
            fatal(rc::INVALID_FILE, "Failed to create KTX2 texture from file \"{}\": {}",
                  streams[i].str(), ktxErrorString(ret));

        formatDesc[i] = createFormatDescriptor(textures[i]->pDfd);
        imageCodecs[i] = ImageCodec(static_cast<VkFormat>(headers[i].vkFormat), headers[i].typeSize,
                                    textures[i]->pDfd);
        ...
        if (formatDesc[i].model() == KHR_DF_MODEL_ETC1S) {
            ...
            if (ret != KTX_SUCCESS)
                fatal(rc::INVALID_FILE, "Failed to transcode KTX2 texture from file \"{}\": {}",
                      streams[i].str(), ktxErrorString(ret));
        ...
        // If the image data was supercompressed then file offsets of texel blocks cannot be
        // calculated
    ...
    if (textures[0]->numDimensions != textures[1]->numDimensions ||
        ...
        fatal(rc::INVALID_ARGUMENTS,
              "Comparison requires matching texture and texel block dimensions.");
    ...
    if (textures[0]->vkFormat != textures[1]->vkFormat)
        fatal(
            ...
            "Comparison requires matching texture formats (BasisLZ is treated as R8G8B8A8_UNORM).");
    ...
    for (uint32_t level = 0; level < maxNumLevels; ++level) {
        ...
        const auto texelBlockDims = imageCodecs[0].pixelToTexelBlockSize(
            glm::uvec4(imageWidth, imageHeight, imageDepth, 1));
        ...
        for (uint32_t layer = 0; layer < maxNumLayers; ++layer) {
            for (uint32_t face = 0; face < maxNumFaces; ++face) {
                ...
                for (std::size_t i = 0; i < streams.size(); ++i)
                    if (level >= textures[i]->numLevels || face >= textures[i]->numFaces ||
                        layer >= textures[i]->numLayers)
                        missingImage = true;

                if (missingImage) {
                    diff << DiffImage(
                        fmt::format("Mismatch in level {}, layer {}, face {}", level, layer, face),
                ...
                for (std::size_t i = 0; i < streams.size(); ++i)
                    if (levelFileOffsets[i].has_value())
                        imageFileOffsets[i] =
                            *levelFileOffsets[i] +
                            (face + layer * textures[i]->numFaces) * imageSizes[i];
                        ...
                        for (uint32_t blockX = 0; blockX < texelBlockDims.x; ++blockX) {
                            ...
                            if (!compareFunc(texelBlocks)) {
                                ...
                                if (texelBlockDifferences <= options.perPixelOutputLimit) {
                                    mismatchingBlocks.push_back(
                                        std::make_pair(texelBlocks[0], texelBlocks[1]));
                                }
                            }
                        }
                    }
                ...
                if (imageMismatch) {
                    diff << DiffImage(
                        fmt::format("Mismatch in level {} layer {} face {}", level, layer, face),
                        fmt::format("m={},a={},f={}", level, layer, face), imageFileOffsets[0],
                        imageFileOffsets[1], mismatchingBlocks);
                }
            }
        }
    }
