...
struct OptionsEncodeASTC : public ktxAstcParams {
    ...
    ClampedOption<ktx_uint32_t> qualityLevel{ktxAstcParams::qualityLevel, 0,
    ...
    void process(cxxopts::Options&, cxxopts::ParseResult& args, Reporter& report) {
        if (args[kAstcQuality].count()) {
            static std::unordered_map<std::string, ktx_pack_astc_quality_levels_e>
                astc_quality_mapping{{"fastest", KTX_PACK_ASTC_QUALITY_LEVEL_FASTEST},
            ...
            const auto qualityLevelStr =
                to_lower_copy(captureASTCOption<std::string>(args, kAstcQuality));
