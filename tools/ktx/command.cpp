...
void Command::parseCommandLine(const std::string& name, const std::string& desc, int argc,
                               char* argv[]) {
    commandName = name;
...
#endif
        if (!file)
            report.fatal(rc::IO_FAILURE, "Could not open output file \"{}\": {}.", filepath,
                         errnoMessage());
    }

    // TODO: Investigate and resolve the portability issue with the C++ streams. The issue will most
    // likely
    //       be in StreambufStream's position reporting and seeking. Currently a fallback is
    //       implemented in C above.
    // if (filepath == "-") {
    //     #if defined(_WIN32)
    //     // Set "stdout" to binary mode
    //     const auto setmodeResult = _setmode(_fileno(stdout), _O_BINARY);
    //     if (setmodeResult == -1)
    //         report.fatal(rc::IO_FAILURE, "Failed to set stdout mode to binary: {}",
    //         setmodeResult);
    //     // #else
    //     // std::freopen(nullptr, "wb", stdout);
    //     #endif
...
void OutputStream::write(const char* data, std::size_t size, Reporter& report) {
    ...
    if (written != size)
        report.fatal(rc::IO_FAILURE, "Failed to write output file \"{}\": {}.",
                     fmtOutFile(filepath), errnoMessage());
}
...
void OutputStream::writeKTX2(ktxTexture* texture, Reporter& report) {
    ...
    if (KTX_SUCCESS != ret) {
        ...
        report.fatal(rc::IO_FAILURE, "Failed to write KTX file \"{}\": KTX error: {}.", filepath,
                     ktxErrorString(ret));
    }
    ...
    //       be in StreambufStream's position reporting and seeking. Currently a fallback is
    //       implemented in C above.
    // StreambufStream<std::streambuf*> stream(activeStream->rdbuf(), std::ios::in |
    // std::ios::binary); const auto ret = ktxTexture_WriteToStream(texture, stream.stream());
    //
    // if (KTX_SUCCESS != ret) {
    //     if (activeStream != &std::cout)
    //         std::filesystem::remove(DecodeUTF8Path(filepath).c_str());
    //     report.fatal(rc::IO_FAILURE, "Failed to write KTX file \"{}\": {}.",
    //     fmtOutFile(filepath), ktxErrorString(ret));
    // }
}
