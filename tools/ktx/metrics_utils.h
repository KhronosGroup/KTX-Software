...
<dl>
    <dt>\--compare-ssim</dt>
    <dd>Calculate encoding structural similarity index measure (SSIM) and print it to stdout. Requires Basis-LZ or UASTC
encoding.</dd> <dt>\--compare-psnr</dt> <dd>Calculate encoding peak signal-to-noise ratio (PSNR) and print it to stdout. Requires
Basis-LZ or UASTC encoding.</dd>
</dl>
//! [command options_metrics]
*/
struct OptionsMetrics {
    ...
    void init(cxxopts::Options& opts) {
        opts.add_options()("compare-ssim",
                           "Calculate encoding structural similarity index measure (SSIM) and print it to stdout. Requires "
                    ...
                    for (uint32_t depthSliceIndex = 0; depthSliceIndex < imageDepth; ++depthSliceIndex) {
                        ...
                        for (uint32_t y = 0; y < imageHeight; ++y) {
                            for (uint32_t x = 0; x < imageWidth; ++x) {
                                ...
                                for (uint32_t c = 0; c < referenceNumChannels; ++c) basisuImage(x, y)[c] = imageView(x, y)[c];
                            }
                        }
                        ...
                        if (opts.compare_psnr) {
                            ...
                            if (referenceImages.size() != 1) fmt::print("    PSNR: {:9.6f}\n", im.m_psnr);
        ...
        if (opts.compare_ssim) {
            ...
            if (referenceNumChannels > 3)
                fmt::print("    SSIM Avg R: {:+7.6f}, G: {:+7.6f}, B: {:+7.6f}, A: {:+7.6f}\n", overallSSIM[0] / numIf,
                           overallSSIM[1] / numIf, overallSSIM[2] / numIf, overallSSIM[3] / numIf);
            else if (referenceNumChannels > 2)
