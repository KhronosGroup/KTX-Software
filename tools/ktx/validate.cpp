...
class FatalValidationError : public std::runtime_error {
  ...
  public:
    explicit FatalValidationError(ValidationReport report)
        : std::runtime_error(report.details), report(std::move(report)) {}
};
...
struct ValidationContext {
  ...
  private:
    ...
    uint32_t numLayers =
        0;  // The actual number of layers, After header parsing always at least one
    uint32_t numLevels =
        0;  // The actual number of levels, After header parsing always at least one
  ...
  public:
    ValidationContext(bool warningsAsErrors, bool GLTFBasisU,
                      std::function<void(const ValidationReport&)> callback)
  ...
  protected:
    ...
    void fatal(const IssueFatal& issue, Args&&... args) {
        ...
        returnCode = +rc::INVALID_FILE;
        const auto report =
            ValidationReport{issue.type, issue.id, std::string{issue.message},
                             fmt::format(issue.detailsFmt, std::forward<Args>(args)...)};
        callback(report);
  ...
  private:
    virtual void read(std::size_t offset, void* readDst, std::size_t readSize,
                      std::string_view name) = 0;
    virtual KTX_error_code createKTXTexture(ktxTextureCreateFlags createFlags,
                                            ktxTexture2** newTex) = 0;

  private:
    template <typename... Args>
    void validateAlignmentPaddingZeros(const void* ptr, const void* bufferEnd,
                                       std::size_t alignment, const IssueError& issue,
                                       Args&&... args) {
  ...
  private:
    ...
    void validateDFDBasic(uint32_t blockIndex, const uint32_t* dfd, const BDFD& block,
...
class ValidationContextIOStream : public ValidationContext {
  ...
  public:
    ValidationContextIOStream(bool warningsAsErrors, bool GLTFBasisU,
                              std::function<void(const ValidationReport&)> callback,
...
class ValidationContextStdioStream : public ValidationContext {
  ...
  public:
    ValidationContextStdioStream(bool warningsAsErrors, bool GLTFBasisU,
                                 std::function<void(const ValidationReport&)> callback, FILE* file,
                                 const std::string& filepath)
...
class ValidationContextMemory : public ValidationContext {
  ...
  public:
    ValidationContextMemory(bool warningsAsErrors, bool GLTFBasisU,
                            std::function<void(const ValidationReport&)> callback,
  ...
  private:
    ...
    virtual KTX_error_code createKTXTexture(ktxTextureCreateFlags createFlags,
                                            ktxTexture2** newTex) override {
        return ktxTexture2_CreateFromMemory(reinterpret_cast<const ktx_uint8_t*>(memoryData),
                                            memorySize, createFlags, newTex);
    }
};
...
void validateToolInput(std::istream& stream, const std::string& filepath, Reporter& report) {
    if (!stream)
        report.fatal(rc::IO_FAILURE, "Could not open input file \"{}\": {}", filepath,
                     errnoMessage());
    ...
    stream.seekg(0);
    if (!stream)
        report.fatal(rc::IO_FAILURE, "Could not rewind the input file \"{}\": {}", filepath,
                     errnoMessage());
}

int validateIOStream(std::istream& stream, const std::string& filepath, bool warningsAsErrors,
                     bool GLTFBasisU, std::function<void(const ValidationReport&)> callback) {
    try {
...
int validateStdioStream(FILE* file, const std::string& filepath, bool warningsAsErrors,
                        bool GLTFBasisU, std::function<void(const ValidationReport&)> callback) {
    try {
...
void ValidationContext::validateHeader() {
    ...
    // Additional block-compressed formats (like UASTC) are detected after the DFD is parsed to
    ...
    const auto max_dim =
        std::max(std::max(header.pixelWidth, header.pixelHeight), header.pixelDepth);
    ...
    if (KTX_SS_BEGIN_VENDOR_RANGE <= header.supercompressionScheme &&
        header.supercompressionScheme <= KTX_SS_END_VENDOR_RANGE)
        warning(HeaderData::VendorSupercompression, toString(supercompressionScheme));
    else if (header.supercompressionScheme < KTX_SS_BEGIN_RANGE ||
             KTX_SS_END_RANGE < header.supercompressionScheme)
...
void ValidationContext::validateIndices() {
    ...
    if (header.dataFormatDescriptor.byteOffset == 0 || header.dataFormatDescriptor.byteLength == 0)
        error(HeaderData::IndexDFDMissing, header.dataFormatDescriptor.byteOffset,
    ...
    if (expectedOffset != header.dataFormatDescriptor.byteOffset)
        error(HeaderData::IndexDFDInvalidOffset, header.dataFormatDescriptor.byteOffset,
              expectedOffset);
    expectedOffset += header.dataFormatDescriptor.byteLength;

    if (header.dataFormatDescriptor.byteOffset != 0 && header.dataFormatDescriptor.byteLength != 0)
        if (header.keyValueData.byteOffset != 0)
            if (header.dataFormatDescriptor.byteLength !=
    ...
    if (header.keyValueData.byteLength != 0) {
        ...
        if (expectedOffset != header.keyValueData.byteOffset)
            error(HeaderData::IndexKVDInvalidOffset, header.keyValueData.byteOffset,
                  expectedOffset);
    ...
    if (header.supercompressionGlobalData.byteLength != 0) {
        ...
        if (expectedOffset != header.supercompressionGlobalData.byteOffset)
            error(HeaderData::IndexSGDInvalidOffset, header.supercompressionGlobalData.byteOffset,
                  expectedOffset);
        expectedOffset += header.supercompressionGlobalData.byteLength;
    } else {
        if (header.supercompressionGlobalData.byteOffset != 0)
            error(HeaderData::IndexSGDOffsetWithoutLength,
                  header.supercompressionGlobalData.byteOffset);
    }
...
size_t ValidationContext::calcImageSize(uint32_t level) {
    ...
    const auto blockDimensionX =
        1u + expectedBlockDimension0.value_or(parsedBlockDimension0.value_or(0u));
    const auto blockDimensionY =
        1u + expectedBlockDimension1.value_or(parsedBlockDimension1.value_or(0u));
    ...
    const auto blockSize =
        expectedBytePlanes ? expectedBytePlanes->at(0) : parsedBlockByteLength.value_or(0u);
...
size_t ValidationContext::calcLayerSize(uint32_t level) {
    ...
    const auto blockDimensionZ =
        1u + expectedBlockDimension2.value_or(parsedBlockDimension2.value_or(0u));
...
size_t ValidationContext::calcLevelOffset(size_t firstLevelOffset, size_t alignment,
                                          uint32_t level) {
...
void ValidationContext::validateLevelIndex() {
    ...
    std::size_t requiredLevelAlignment =
        calcLevelAlignment(ktxSupercmpScheme(header.supercompressionScheme), blockByteLength);

    std::size_t expectedFirstLevelOffset = 0;
    if (header.supercompressionGlobalData.byteLength != 0)
        expectedFirstLevelOffset = header.supercompressionGlobalData.byteLength +
                                   header.supercompressionGlobalData.byteOffset;
        ...
        expectedFirstLevelOffset =
            header.dataFormatDescriptor.byteLength + header.dataFormatDescriptor.byteOffset;
    else
    ...
    for (std::size_t i = 1; i < levelIndices.size(); ++i) {
        if (levelIndices[i].uncompressedByteLength > levelIndices[i - 1].uncompressedByteLength)
            error(LevelIndex::IncorrectIndexOrder, i - 1,
                  levelIndices[i - 1].uncompressedByteLength, i,
                  levelIndices[i].uncompressedByteLength);

        if (levelIndices[i].byteOffset > levelIndices[i - 1].byteOffset)
            error(LevelIndex::IncorrectLevelOrder, i - 1, levelIndices[i - 1].byteOffset, i,
                  levelIndices[i].byteOffset);
    }

    std::size_t lastByteOffset =
        expectedFirstLevelOffset;  // Reuse lastByteOffset to inject first offset to expectedOffset
    std::size_t lastByteLength = 0;

    // Count down, so we can check the distance between levels for the UNDEFINED and
    // SUPERCOMPRESSION cases.
    for (auto it = static_cast<uint32_t>(levelIndices.size()); it != 0; --it) {
        const auto index = it - 1;
        const auto& level = levelIndices[index];

        // Validate byteOffset
        const auto knownLevelOffset =
            header.vkFormat != VK_FORMAT_UNDEFINED && header.supercompressionScheme == KTX_SS_NONE;
        ...
        if (level.byteOffset % requiredLevelAlignment != 0)
            error(LevelIndex::IncorrectByteOffsetUnaligned, index, level.byteOffset,
        ...
        // Workaround: Disable ByteLength validations for the 3D ASTC encoder which currently
        // ignores partial Z blocks in our test files
        const auto disableByteLengthValidation =
            isFormat3DBlockCompressed(VkFormat(header.vkFormat)) &&
            ...
            if (header.supercompressionScheme == KTX_SS_BASIS_LZ) {
                ...
                if (header.supercompressionScheme == KTX_SS_NONE) {
                    ...
                    if (level.uncompressedByteLength != expectedUncompressedLength)
                        error(LevelIndex::IncorrectUncompressedByteLength, index,
                              level.uncompressedByteLength, expectedUncompressedLength);
                }
            } else {
                if (level.uncompressedByteLength == 0)
                    error(LevelIndex::ZeroUncompressedLength, index);
                else if (level.uncompressedByteLength %
                             std::max(header.faceCount * numLayers, 1u) !=
                         0)
                    ...
                    error(LevelIndex::InvalidUncompressedLength, index,
...
void ValidationContext::validateDFD() {
    ...
    while (ptrDFDIt < ptrDFDEnd) {
        ...
        if (blockHeader.descriptorBlockSize < sizeof(DFDHeader)) {
            ...
            error(DFD::DescriptorBlockSizeTooBig, numBlocks, +blockHeader.descriptorBlockSize,
                  remainingDFDBytes);
            ...
            if (blockHeader.vendorId == KHR_DF_VENDORID_KHRONOS &&
                ...
                if (std::exchange(foundBDFD, true)) {
                    ...
                    error(DFD::BasicDescriptorBlockSizeTooSmall, numBlocks,
                          +blockHeader.descriptorBlockSize);
                    ...
                    if ((block.descriptorBlockSize - 24) % 16 != 0)
                        error(DFD::BasicDescriptorBlockSizeInvalid, numBlocks,
                              +block.descriptorBlockSize);

                    const auto numSamplesStored = (block.descriptorBlockSize - 24u) / 16u;
                    const auto numSamplesValidating =
                        std::min(MAX_NUM_BDFD_SAMPLES, numSamplesStored);

                    if (numSamplesStored > MAX_NUM_BDFD_SAMPLES)
                        warning(DFD::TooManySample, numBlocks, numSamplesStored,
                                MAX_NUM_BDFD_SAMPLES, numSamplesStored - numSamplesValidating,
                                block.descriptorBlockSize - sizeof(BDFD) -
                                    numSamplesValidating * sizeof(SampleType));

                    // Samples are located at the end of the block
                    std::vector<SampleType> samples(numSamplesValidating);
                    std::memcpy(samples.data(), ptrDFDIt + sizeof(BDFD),
                                numSamplesValidating * sizeof(SampleType));

                    validateDFDBasic(numBlocks, reinterpret_cast<uint32_t*>(buffer.get()), block,
                                     samples);
                }

            } else if (blockHeader.vendorId == KHR_DF_VENDORID_KHRONOS &&
                       blockHeader.descriptorType ==
                ...
                warning(DFD::UnknownDFDBlock, numBlocks,
                        toString(khr_df_vendorid_e(blockHeader.vendorId)),
                        toString(khr_df_vendorid_e(blockHeader.vendorId),
                                 khr_df_khr_descriptortype_e(blockHeader.descriptorType)));
            }
        }
...
void ValidationContext::validateDFDBasic(uint32_t blockIndex, const uint32_t* dfd,
                                         const BDFD& block,
                                         const std::vector<SampleType>& samples) {
    ...
    if (block.versionNumber != KHR_DF_VERSIONNUMBER_1_3)
        error(DFD::BasicVersionNotSupported, blockIndex,
              toString(khr_df_versionnumber_e(block.versionNumber)));

    // Validate transferFunction
    if (block.transfer != KHR_DF_TRANSFER_SRGB && block.transfer != KHR_DF_TRANSFER_LINEAR)
        error(DFD::BasicInvalidTransferFunction, blockIndex,
              toString(khr_df_transfer_e(block.transfer)));

    if (isFormatSRGB(VkFormat(header.vkFormat)) && block.transfer != KHR_DF_TRANSFER_SRGB)
        error(DFD::BasicSRGBMismatch, blockIndex, toString(khr_df_transfer_e(block.transfer)),
              toString(VkFormat(header.vkFormat)));

    if (isFormatNotSRGBButHasSRGBVariant(VkFormat(header.vkFormat)) &&
        block.transfer == KHR_DF_TRANSFER_SRGB)
    ...
    if (isFormat422(VkFormat(header.vkFormat))) {
        ...
        const auto expectedBCColorModel =
            getColorModelForBlockCompressedFormat(VkFormat(header.vkFormat));
    ...
    if (checkGLTFBasisU) {
                ...
                case KHR_DF_CHANNEL_ETC1S_RGB:
                    if (samples.size() > 1 && samples[1].channelType != KHR_DF_CHANNEL_ETC1S_AAA)
                        error(DFD::InvalidChannelGLTFBU, blockIndex, "KHR_DF_MODEL_ETC1S", 2,
                              toString(KHR_DF_MODEL_ETC1S,
                                       khr_df_model_channels_e(samples[1].channelType)),
                              "KHR_DF_CHANNEL_ETC1S_AAA when sample #0 channelType is "
                ...
                case KHR_DF_CHANNEL_ETC1S_RRR:
                    if (samples.size() > 1 && samples[1].channelType != KHR_DF_CHANNEL_ETC1S_GGG)
                        error(DFD::InvalidChannelGLTFBU, blockIndex, "KHR_DF_MODEL_ETC1S", 2,
                              toString(KHR_DF_MODEL_ETC1S,
                                       khr_df_model_channels_e(samples[1].channelType)),
                              "KHR_DF_CHANNEL_ETC1S_GGG when sample #0 channelType is "
                              "KHR_DF_CHANNEL_ETC1S_RRR");
                    break;
                default:
                    error(DFD::InvalidChannelGLTFBU, blockIndex, "KHR_DF_MODEL_ETC1S", 1,
                          toString(KHR_DF_MODEL_ETC1S,
                                   khr_df_model_channels_e(samples[0].channelType)),
            ...
            if (samples.size() > 0) {
                ...
                default:
                    error(DFD::InvalidChannelGLTFBU, blockIndex, "KHR_DF_MODEL_UASTC", 0,
                          toString(KHR_DF_MODEL_UASTC,
                                   khr_df_model_channels_e(samples[0].channelType)),
                          "KHR_DF_CHANNEL_UASTC_RGB, KHR_DF_CHANNEL_UASTC_RGBA, "
                          "KHR_DF_CHANNEL_UASTC_RRR, or KHR_DF_CHANNEL_UASTC_RG");
                    break;
                }
            }
            break;
        ...
        if ((block.primaries != KHR_DF_PRIMARIES_BT709 || block.transfer != KHR_DF_TRANSFER_SRGB) &&
            (block.primaries != KHR_DF_PRIMARIES_UNSPECIFIED ||
             block.transfer != KHR_DF_TRANSFER_LINEAR))
            error(DFD::InvalidColorSpaceGLTFBU, blockIndex,
                  toString(khr_df_primaries_e(block.primaries)),
                  toString(khr_df_transfer_e(block.transfer)));
    }
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        ...
        if (!isChannelTypeValid(khr_df_model_e(block.model),
                                khr_df_model_channels_e(sample.channelType)))
            error(
                DFD::InvalidChannelForModel, blockIndex, i + 1,
        ...
        if (header.vkFormat != VK_FORMAT_UNDEFINED) {
            if (expectedSamples) {
                if (samples.size() != expectedSamples->size())
                    error(DFD::SampleCountMismatch, blockIndex, samples.size(),
                          toString(VkFormat(header.vkFormat)), expectedSamples->size());

                for (std::size_t i = 0; i < std::min(samples.size(), expectedSamples->size());
                     ++i) {
                    ...
                    if (parsed.channelType != expected.channelType)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "channelType",
                              toString(khr_df_model_e(block.model),
                                       khr_df_model_channels_e(parsed.channelType)),
                    ...
                    if (parsed.qualifierLinear != expected.qualifierLinear)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "qualifierLinear",
                              parsed.qualifierLinear, expected.qualifierLinear,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.qualifierExponent != expected.qualifierExponent)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "qualifierExponent",
                              parsed.qualifierExponent, expected.qualifierExponent,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.qualifierSigned != expected.qualifierSigned)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "qualifierSigned",
                              parsed.qualifierSigned, expected.qualifierSigned,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.qualifierFloat != expected.qualifierFloat)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "qualifierFloat",
                              parsed.qualifierFloat, expected.qualifierFloat,
                              toString(VkFormat(header.vkFormat)));
                    // For 4:2:2 formats the X sample positions can vary
                    if (!isFormat422(VkFormat(header.vkFormat)) &&
                        parsed.samplePosition0 != expected.samplePosition0)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "samplePosition0",
                              parsed.samplePosition0, expected.samplePosition0,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.samplePosition1 != expected.samplePosition1)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "samplePosition1",
                              parsed.samplePosition1, expected.samplePosition1,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.samplePosition2 != expected.samplePosition2)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "samplePosition2",
                              parsed.samplePosition2, expected.samplePosition2,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.samplePosition3 != expected.samplePosition3)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "samplePosition3",
                              parsed.samplePosition3, expected.samplePosition3,
                              toString(VkFormat(header.vkFormat)));
            ...
            if (header.supercompressionScheme == KTX_SS_NONE) {
                if (expectedBytePlanes &&
                    !std::equal(expectedBytePlanes->begin(), expectedBytePlanes->end(),
                                block.bytesPlanes.begin(), block.bytesPlanes.end())) {
                    error(DFD::BytesPlanesMismatch, blockIndex, block.bytesPlanes[0],
                          block.bytesPlanes[1], block.bytesPlanes[2], block.bytesPlanes[3],
                          block.bytesPlanes[4], block.bytesPlanes[5], block.bytesPlanes[6],
                          block.bytesPlanes[7], toString(VkFormat(header.vkFormat)),
            ...
            if (!isFormatBlockCompressed(VkFormat(header.vkFormat)) &&
                ...
                // Reset the "false" positive error in interpretDFD with
                // VK_FORMAT_E5B9G9R9_UFLOAT_PACK32
                if (header.vkFormat == VK_FORMAT_E5B9G9R9_UFLOAT_PACK32 &&
                    result == i_UNSUPPORTED_NONTRIVIAL_ENDIANNESS)
                    result = InterpretDFDResult(0);

                if (result >= i_UNSUPPORTED_ERROR_BIT) {
                    switch (result) {
                    case i_UNSUPPORTED_CHANNEL_TYPES:
                        // We already checked channel types elsewhere with more detailed error
                        // message
                        break;
                    case i_UNSUPPORTED_MULTIPLE_PLANES:
                        error(DFD::MultiplaneFormatsNotSupported, blockIndex, block.bytesPlanes[0],
                              block.bytesPlanes[1], block.bytesPlanes[2], block.bytesPlanes[3],
                              block.bytesPlanes[4], block.bytesPlanes[5], block.bytesPlanes[6],
                              block.bytesPlanes[7]);
                        break;
            ...
            if (block.model == KHR_DF_MODEL_UASTC) {
                ...
                if (!block.matchTexelBlockDimensions(3, 3, 0, 0))
                    error(DFD::InvalidTexelBlockDimension, blockIndex,
                          block.texelBlockDimension0 + 1, block.texelBlockDimension1 + 1,
                          block.texelBlockDimension2 + 1, block.texelBlockDimension3 + 1, 4, 4, 1,
                          1, "UASTC");
                ...
                if (header.supercompressionScheme == KTX_SS_NONE) {
                    if (block.bytesPlanes[0] != 16 || block.bytesPlanes[1] != 0 ||
                        ...
                        error(DFD::BytesPlanesMismatch, blockIndex, block.bytesPlanes[0],
                              block.bytesPlanes[1], block.bytesPlanes[2], block.bytesPlanes[3],
                              block.bytesPlanes[4], block.bytesPlanes[5], block.bytesPlanes[6],
                              block.bytesPlanes[7], "UASTC", 16, 0, 0, 0, 0, 0, 0, 0);
                    }
                } else {
                    if (block.hasNonZeroBytePlane())
                        error(DFD::BytesPlanesNotUnsized, blockIndex, block.bytesPlanes[0],
                              block.bytesPlanes[1], block.bytesPlanes[2], block.bytesPlanes[3],
                              block.bytesPlanes[4], block.bytesPlanes[5], block.bytesPlanes[6],
                              block.bytesPlanes[7], "UASTC");
                }
                ...
                if (samples.size() > 0) {
                    ...
                    if (samples[0].upper != UINT32_MAX)
                        error(DFD::InvalidUpper, blockIndex, 1, samples[0].upper, "UASTC",
                ...
                if (header.supercompressionScheme == KTX_SS_NONE) {
                    ...
                    if (block.bytesPlanes[1] != 0 || block.bytesPlanes[2] != 0 ||
                        block.bytesPlanes[3] != 0 || block.bytesPlanes[4] != 0 ||
                        block.bytesPlanes[5] != 0 || block.bytesPlanes[6] != 0 ||
    ...
    case KTX_SS_BASIS_LZ:
        ...
        if (!block.matchTexelBlockDimensions(3, 3, 0, 0))
            error(DFD::InvalidTexelBlockDimension, blockIndex, block.texelBlockDimension0 + 1,
                  block.texelBlockDimension1 + 1, block.texelBlockDimension2 + 1,
                  block.texelBlockDimension3 + 1, 4, 4, 1, 1, "BasisLZ/ETC1S");

        if (block.hasNonZeroBytePlane())
            error(DFD::BytesPlanesNotUnsized, blockIndex, block.bytesPlanes[0],
                  block.bytesPlanes[1], block.bytesPlanes[2], block.bytesPlanes[3],
                  block.bytesPlanes[4], block.bytesPlanes[5], block.bytesPlanes[6],
                  block.bytesPlanes[7], "BasisLZ/ETC1S");

        for (uint32_t i = 0; i < std::min(static_cast<uint32_t>(samples.size()), 2u); ++i) {
            if (samples[i].bitOffset != (i == 0 ? 0u : 64u))
                error(DFD::InvalidBitOffsetForBLZE, blockIndex, i + 1, samples[i].bitOffset,
                      (i == 0 ? 0u : 64u));
            ...
            if (samples[i].upper != UINT32_MAX)
                error(DFD::InvalidUpper, blockIndex, i + 1, samples[i].upper, "BasisLZ/ETC1S",
                      "UINT32_MAX (0xFFFFFFFF)");
        }
        break;
    ...
    if (header.vkFormat != VK_FORMAT_UNDEFINED &&
        !isFormatBlockCompressed(static_cast<VkFormat>(header.vkFormat))) {
        ...
        if (isColorModelBlockCompressed(khr_df_model_e(block.model))) {
            ...
            if (expectedTypeSize && header.typeSize != expectedTypeSize)
                error(HeaderData::TypeSizeMismatch, header.typeSize,
                      toString(VkFormat(header.vkFormat)), *expectedTypeSize);
        }
    }
}
    ...
    while (ptrEntry < ptrKVDEnd) {
        ...
        if (sizeKeyValuePair < 2) {
            ...
            if (ptrKeyValuePair + sizeKeyValuePair > ptrKVDEnd) {
                ...
                sizeKeyValuePair = static_cast<uint32_t>(
                    bytesLeft);  // Attempt recovery to read out at least the key
            }
        ...
        validateAlignmentPaddingZeros(ptrEntry, ptrKVDEnd, 4, Metadata::PaddingNotZero,
                                      "after a Key-Value entry");
        ptrEntry = align(ptrEntry, 4);
    }

    if (ptrEntry != ptrKVDEnd)
        // Being super explicit about the specs. This check might be overkill as other checks often
        // cover this case
        error(Metadata::SizesDontAddUp, ptrEntry - ptrKVD, kvdByteLength);

    if (header.supercompressionGlobalData.byteLength != 0)
        validateAlignmentPaddingZeros(ptrEntry, ptrKVDEnd, 8, Metadata::PaddingNotZero,
                                      "after the last KVD entry");
...
void ValidationContext::validateKTXorientation(const uint8_t* data, uint32_t size) {
    ...
    const auto value =
        std::string_view(reinterpret_cast<const char*>(data), hasNull ? size - 1 : size);
...
void ValidationContext::validateSGD() {
    ...
    for (uint32_t level = 0; level < numLevels; ++level)
        // numFaces * depth is only reasonable because they can't both be > 1. There are no 3D
        // cubemaps
    ...
    const ktxBasisLzGlobalHeader& bgh =
        *reinterpret_cast<const ktxBasisLzGlobalHeader*>(buffer.get());

    const uint64_t expectedBgdByteLength = sizeof(ktxBasisLzGlobalHeader) +
                                           sizeof(ktxBasisLzEtc1sImageDesc) * imageCount +
                                           bgh.endpointsByteLength + bgh.selectorsByteLength +
                                           bgh.tablesByteLength + bgh.extendedByteLength;
    ...
    // Validate ImageDesc
    if (sgdByteLength <
        sizeof(ktxBasisLzGlobalHeader) + sizeof(ktxBasisLzEtc1sImageDesc) * imageCount)
        return;
    ...
    for (uint32_t level = 0; level < numLevels; ++level) {
        for (uint32_t layer = 0; layer < numLayers; ++layer) {
            for (uint32_t face = 0; face < header.faceCount; ++face) {
                for (uint32_t zSlice = 0; zSlice < std::max(header.pixelDepth >> level, 1u);
                     ++zSlice) {
                    ...
                    if (image.imageFlags & ~ETC1S_P_FRAME)
                        error(SGD::BLZEInvalidImageFlagBit, level, layer, face, zSlice,
                              image.imageFlags);

                    if (image.rgbSliceByteLength == 0)
                        error(SGD::BLZEZeroRGBLength, level, layer, face, zSlice,
                              image.rgbSliceByteLength);

                    if (image.rgbSliceByteOffset + image.rgbSliceByteLength >
                        levelIndices[level].byteLength)
                        error(SGD::BLZEInvalidRGBSlice, level, layer, face, zSlice,
                              image.rgbSliceByteOffset, image.rgbSliceByteLength,
                              levelIndices[level].byteLength);
                    if (image.alphaSliceByteOffset + image.alphaSliceByteLength >
                        levelIndices[level].byteLength)
                        error(SGD::BLZEInvalidAlphaSlice, level, layer, face, zSlice,
                              image.alphaSliceByteOffset, image.alphaSliceByteLength,
                              levelIndices[level].byteLength);
                    ...
                    if (image.alphaSliceByteLength != 0 && numSamples == 1)
                        error(SGD::BLZEDFDMismatchNoAlpha, level, layer, face, zSlice,
...
void ValidationContext::validatePaddings() {
    ...
    const auto check = [&](size_t offset, size_t size, std::string name) {
        ...
        for (size_t i = 0; i < paddingSize; ++i)
            if (buffer[i] != 0) {
                error(Metadata::PaddingNotZero, buffer[i],
                      fmt::format("before {} at offset {}", name, position + i));
    ...
    if (numError == 0 && numWarning == 0) {
        ...
        case KTX_SUCCESS:
            ...
            if (result != KTX_SUCCESS)
                error(Validator::TranscodeFailure, toString(*parsedColorModel),
