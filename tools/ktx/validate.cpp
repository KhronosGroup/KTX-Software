...
class FatalValidationError : public std::runtime_error {
  ...
  public:
    explicit FatalValidationError(ValidationReport report) : std::runtime_error(report.details), report(std::move(report)) {}
};
...
struct ValidationContext {
  ...
  public:
    ValidationContext(bool warningsAsErrors, bool GLTFBasisU, std::function<void(const ValidationReport&)> callback)
        : callback(std::move(callback)), treatWarningsAsError(warningsAsErrors), checkGLTFBasisU(GLTFBasisU) {
  ...
  protected:
    ...
    void warning(const IssueWarning& issue, Args&&... args) {
        ...
        if (treatWarningsAsError) {
            ...
            callback(ValidationReport{IssueType::error, issue.id, std::string{issue.message},
                                      fmt::format(issue.detailsFmt, std::forward<Args>(args)...)});

        } else {
            callback(ValidationReport{issue.type, issue.id, std::string{issue.message},
                                      fmt::format(issue.detailsFmt, std::forward<Args>(args)...)});
        }
    }
    ...
    void fatal(const IssueFatal& issue, Args&&... args) {
        ++numError;
        returnCode = +rc::INVALID_FILE;
        const auto report = ValidationReport{issue.type, issue.id, std::string{issue.message},
                                             fmt::format(issue.detailsFmt, std::forward<Args>(args)...)};
        callback(report);
  ...
  private:
    template <typename... Args>
    void validateAlignmentPaddingZeros(const void* ptr, const void* bufferEnd, std::size_t alignment, const IssueError& issue,
                                       Args&&... args) {
        const auto* begin = static_cast<const char*>(ptr);
        const auto* end = std::min(bufferEnd, align(ptr, alignment));

        for (auto it = begin; it < end; ++it)
            if (*it != 0) error(issue, static_cast<uint8_t>(*it), std::forward<Args>(args)...);
    }
...
class ValidationContextIOStream : public ValidationContext {
  ...
  public:
    ValidationContextIOStream(bool warningsAsErrors, bool GLTFBasisU, std::function<void(const ValidationReport&)> callback,
                              std::istream& stream, const std::string& filepath)
        : ValidationContext(warningsAsErrors, GLTFBasisU, std::move(callback)), stream(stream) {
        if (!stream) fatal(IOError::FileOpen, filepath, errnoMessage());
    }
    ...
    ValidationContextIOStream(bool warningsAsErrors, bool GLTFBasisU, std::function<void(const ValidationReport&)> callback,
        ...
        : ValidationContext(warningsAsErrors, GLTFBasisU, std::move(callback)),
          ...
          stream(file) {
        if (!file) fatal(IOError::FileOpen, filepath, errnoMessage());
    }

  private:
    virtual void read(std::size_t offset, void* readDst, std::size_t readSize, std::string_view name) override {
        stream.seekg(offset);
        if (!stream) fatal(IOError::FileSeekFailure, offset, name, errnoMessage());
    ...
    virtual KTX_error_code createKTXTexture(ktxTextureCreateFlags createFlags, ktxTexture2** newTex) override {
        stream.seekg(0);
        if (!stream) fatal(IOError::RewindFailure, errnoMessage());
...
class ValidationContextStdioStream : public ValidationContext {
  ...
  public:
    ValidationContextStdioStream(bool warningsAsErrors, bool GLTFBasisU, std::function<void(const ValidationReport&)> callback,
                                 FILE* file, const std::string& filepath)
        : ValidationContext(warningsAsErrors, GLTFBasisU, std::move(callback)), file(file) {
        if (!file) fatal(IOError::FileOpen, filepath, errnoMessage());
    }
  ...
  private:
    virtual void read(std::size_t offset, void* readDst, std::size_t readSize, std::string_view name) override {
        ...
        if (seekResult != 0) fatal(IOError::FileSeekFailure, offset, name, seekResult);
    ...
    virtual KTX_error_code createKTXTexture(ktxTextureCreateFlags createFlags, ktxTexture2** newTex) override {
        ...
        if (seekResult != 0) fatal(IOError::RewindFailure, seekResult);
...
class ValidationContextMemory : public ValidationContext {
  ...
  public:
    ValidationContextMemory(bool warningsAsErrors, bool GLTFBasisU, std::function<void(const ValidationReport&)> callback,
                            const char* memoryData, std::size_t memorySize)
        : ValidationContext(warningsAsErrors, GLTFBasisU, std::move(callback)), memoryData(memoryData), memorySize(memorySize) {}

  private:
    virtual void read(std::size_t offset, void* readDst, std::size_t readSize, std::string_view name) override {
        const auto bytesAvailable = offset > memorySize ? 0 : memorySize - offset;

        if (bytesAvailable < readSize) fatal(IOError::UnexpectedEOF, readSize, offset, name, bytesAvailable);
...
void validateToolInput(std::istream& stream, const std::string& filepath, Reporter& report) {
    if (!stream) report.fatal(rc::IO_FAILURE, "Could not open input file \"{}\": {}", filepath, errnoMessage());
    ...
    if (validationResult != +rc::SUCCESS) throw FatalError(ReturnCode{validationResult});

    stream.seekg(0);
    if (!stream) report.fatal(rc::IO_FAILURE, "Could not rewind the input file \"{}\": {}", filepath, errnoMessage());
}

int validateIOStream(std::istream& stream, const std::string& filepath, bool warningsAsErrors, bool GLTFBasisU,
                     std::function<void(const ValidationReport&)> callback) {
    try {
...
int validateMemory(const char* data, std::size_t size, bool warningsAsErrors, bool GLTFBasisU,
                   std::function<void(const ValidationReport&)> callback) {
    try {
...
int validateNamedFile(const std::string& filepath, bool warningsAsErrors, bool GLTFBasisU,
                      std::function<void(const ValidationReport&)> callback) {
    try {
...
int validateStdioStream(FILE* file, const std::string& filepath, bool warningsAsErrors, bool GLTFBasisU,
                        std::function<void(const ValidationReport&)> callback) {
    try {
...
int ValidationContext::validate(bool doCreateAndTranscodeChecks) {
    ...
    if (doCreateAndTranscodeChecks) call(&ValidationContext::validateCreateAndTranscode, "Create and Transcode");
...
void ValidationContext::validateHeader() {
    ...
    if (std::memcmp(&header.identifier, ktx2_identifier_reference, 12) != 0) fatal(FileError::NotKTX2);
    ...
    if (isProhibitedFormat(vkFormat)) {
        ...
        if (vkFormat <= VK_FORMAT_MAX_STANDARD_ENUM) error(HeaderData::InvalidFormat, toString(vkFormat));
        if (VK_FORMAT_MAX_STANDARD_ENUM < vkFormat && vkFormat < 1000001000) error(HeaderData::InvalidFormat, toString(vkFormat));
        if (1000001000 <= vkFormat && !isFormatKnown(vkFormat)) warning(HeaderData::UnknownFormat, toString(vkFormat));
    }

    if (header.supercompressionScheme == KTX_SS_BASIS_LZ) {
        if (header.vkFormat != VK_FORMAT_UNDEFINED) error(HeaderData::VkFormatAndBasis, toString(vkFormat));
    }

    // Validate typeSize
    if (header.vkFormat == VK_FORMAT_UNDEFINED) {
        if (header.typeSize != 1) error(HeaderData::TypeSizeNotOne, header.typeSize, toString(vkFormat));
    ...
    if (header.pixelWidth == 0) error(HeaderData::WidthZero);

    if (isFormatBlockCompressed(vkFormat))
        if (header.pixelHeight == 0) error(HeaderData::BlockCompressedNoHeight, toString(vkFormat));
    if (isSupercompressionBlockCompressed(supercompressionScheme))
        if (header.pixelHeight == 0) error(HeaderData::BlockCompressedNoHeight, toString(supercompressionScheme));
    ...
    if (header.pixelDepth != 0 && header.pixelHeight == 0) error(HeaderData::DepthNoHeight, header.pixelDepth);

    if (isFormat3DBlockCompressed(vkFormat))
        if (header.pixelDepth == 0) error(HeaderData::DepthBlockCompressedNoDepth, toString(vkFormat));

    if (isFormatDepth(vkFormat) || isFormatStencil(vkFormat))
        if (header.pixelDepth != 0) error(HeaderData::DepthStencilFormatWithDepth, header.pixelDepth, toString(vkFormat));

    if (header.faceCount == 6)
        if (header.pixelDepth != 0) error(HeaderData::CubeWithDepth, header.pixelDepth);

    // Detect dimension counts
    if (header.pixelDepth != 0) {
        dimensionCount = 3;
        if (header.layerCount != 0) warning(HeaderData::ThreeDArray);  // Warning on 3D Array textures
    ...
    // Validate faceCount
    if (header.faceCount != 6 && header.faceCount != 1) error(HeaderData::InvalidFaceCount, header.faceCount);

    // 2D Cube map faces were validated by CubeHeightWidthMismatch and CubeWithDepth

    // Validate levelCount
    if (isFormatBlockCompressed(vkFormat))
        if (header.levelCount == 0) error(HeaderData::BlockCompressedNoLevel, toString(vkFormat));
    if (isSupercompressionBlockCompressed(supercompressionScheme))
        if (header.levelCount == 0) error(HeaderData::BlockCompressedNoLevel, toString(supercompressionScheme));
    ...
    if (checkGLTFBasisU) {
        ...
        if (header.pixelDepth != 0) error(HeaderData::InvalidTextureTypeGLTFBU, "pixelDepth", header.pixelDepth, 0);
        if (header.layerCount != 0) error(HeaderData::InvalidTextureTypeGLTFBU, "layerCount", header.layerCount, 0);
        if (header.faceCount != 1) error(HeaderData::InvalidTextureTypeGLTFBU, "faceCount", header.faceCount, 1);

        // Check that width and height are multiples of 4
        if (header.pixelWidth % 4 != 0) error(HeaderData::InvalidPixelWidthHeightGLTFBU, "pixelWidth", header.pixelWidth);
        if (header.pixelHeight % 4 != 0) error(HeaderData::InvalidPixelWidthHeightGLTFBU, "pixelHeight", header.pixelHeight);
...
void ValidationContext::validateIndices() {
    ...
    if (header.dataFormatDescriptor.byteOffset != 0 && header.dataFormatDescriptor.byteLength != 0)
        if (header.keyValueData.byteOffset != 0)
            if (header.dataFormatDescriptor.byteLength != header.keyValueData.byteOffset - header.dataFormatDescriptor.byteOffset)
                error(HeaderData::IndexDFDInvalidLength, header.dataFormatDescriptor.byteLength,
                      header.keyValueData.byteOffset - header.dataFormatDescriptor.byteOffset);
    ...
    if (header.keyValueData.byteLength != 0) {
        ...
        if (header.keyValueData.byteOffset != 0) error(HeaderData::IndexKVDOffsetWithoutLength, header.keyValueData.byteOffset);
    }
    ...
    if (isSupercompressionWithGlobalData(supercompressionScheme)) {
        if (header.supercompressionGlobalData.byteLength == 0) error(HeaderData::IndexSGDMissing, toString(supercompressionScheme));
    } else if (isSupercompressionWithNoGlobalData(supercompressionScheme)) {
        if (header.supercompressionGlobalData.byteLength != 0)
            error(HeaderData::IndexSGDNotApplicable, header.supercompressionGlobalData.byteLength,
                  toString(supercompressionScheme));
    }
}
...
size_t ValidationContext::calcImageSize(uint32_t level) {
    ...
    const auto blockSize = expectedBytePlanes ? expectedBytePlanes->at(0) : parsedBlockByteLength.value_or(0u);
...
size_t ValidationContext::calcLevelSize(uint32_t level) { return calcLayerSize(level) * numLayers; }
...
void ValidationContext::validateLevelIndex() {
    ...
    const auto blockByteLength =
        (expectedBytePlanes ? expectedBytePlanes->at(0)
                            : (parsedBlockByteLength == 0 ? uint8_t(1) : parsedBlockByteLength.value_or(0)));
    ...
    for (std::size_t i = 1; i < levelIndices.size(); ++i) {
        if (levelIndices[i].uncompressedByteLength > levelIndices[i - 1].uncompressedByteLength)
            error(LevelIndex::IncorrectIndexOrder, i - 1, levelIndices[i - 1].uncompressedByteLength, i,
                  levelIndices[i].uncompressedByteLength);
    ...
    for (auto it = static_cast<uint32_t>(levelIndices.size()); it != 0; --it) {
        ...
        // Validate byteOffset
        const auto knownLevelOffset = header.vkFormat != VK_FORMAT_UNDEFINED && header.supercompressionScheme == KTX_SS_NONE;
        ...
        const auto expectedOffset = knownLevelOffset ? calcLevelOffset(expectedFirstLevelOffset, requiredLevelAlignment, index)
        ...
        // Workaround: Disable ByteLength validations for the 3D ASTC encoder which currently ignores partial Z blocks in our test
        // files
        ...
        if (!disableByteLengthValidation) {
            ...
            if (header.supercompressionScheme == KTX_SS_BASIS_LZ) {
                if (level.uncompressedByteLength != 0) error(LevelIndex::NonZeroUBLForBLZE, index, level.uncompressedByteLength);
                ...
                if (header.supercompressionScheme == KTX_SS_NONE) {
                    ...
                    if (level.uncompressedByteLength != expectedUncompressedLength)
                        error(LevelIndex::IncorrectUncompressedByteLength, index, level.uncompressedByteLength,
...
void ValidationContext::calculateExpectedDFD(VkFormat format) {
    if (format == VK_FORMAT_UNDEFINED || !isFormatValid(format) || isProhibitedFormat(format)) return;
    ...
    std::memcpy(expectedSamples->data(), reinterpret_cast<const char*>(bdfd) + sizeof(BDFD),
                expectedSampleCount * sizeof(SampleType));
...
void ValidationContext::validateDFD() {
    ...
    if (dfdByteOffset == 0 || dfdByteLength == 0) return;  // There is no DFD block
    ...
    if (dfdByteLength != dfdTotalSize) error(DFD::SizeMismatch, dfdByteLength, dfdTotalSize);
    ...
    while (ptrDFDIt < ptrDFDEnd) {
        ...
        if (blockHeader.descriptorBlockSize < sizeof(DFDHeader)) {
            ...
            if (blockHeader.vendorId == KHR_DF_VENDORID_KHRONOS &&
                blockHeader.descriptorType == KHR_DF_KHR_DESCRIPTORTYPE_BASICFORMAT) {
            ...
            } else if (blockHeader.vendorId == KHR_DF_VENDORID_KHRONOS &&
                       blockHeader.descriptorType == KHR_DF_KHR_DESCRIPTORTYPE_ADDITIONAL_DIMENSIONS) {
                // TODO: Implement DFD validation for ADDITIONAL_DIMENSIONS

            } else if (blockHeader.vendorId == KHR_DF_VENDORID_KHRONOS &&
                       blockHeader.descriptorType == KHR_DF_KHR_DESCRIPTORTYPE_ADDITIONAL_PLANES) {
                // TODO: Implement DFD validation for ADDITIONAL_PLANES

            } else {
                warning(DFD::UnknownDFDBlock, numBlocks, toString(khr_df_vendorid_e(blockHeader.vendorId)),
    ...
    if (!foundBDFD) error(DFD::MissingBDFD);
}

void ValidationContext::validateDFDBasic(uint32_t blockIndex, const uint32_t* dfd, const BDFD& block,
    ...
    if (isFormat422(VkFormat(header.vkFormat))) {
        if (!isProhibitedFormat(VkFormat(header.vkFormat)))
            if (block.model != KHR_DF_MODEL_YUVSDA)
                error(DFD::IncorrectModelFor422, blockIndex, toString(khr_df_model_e(block.model)),
        ...
        if (khr_df_model_e(block.model) != expectedBCColorModel)
            error(DFD::IncorrectModelForBlock, blockIndex, toString(khr_df_model_e(block.model)),
                  toString(VkFormat(header.vkFormat)), toString(expectedBCColorModel));

    } else if (header.vkFormat != VK_FORMAT_UNDEFINED) {
        if (block.model != KHR_DF_MODEL_RGBSDA)
            error(DFD::IncorrectModelForRGB, blockIndex, toString(khr_df_model_e(block.model)),
                  toString(VkFormat(header.vkFormat)));
    }

    if (header.supercompressionScheme == KTX_SS_BASIS_LZ)
        if (block.model != KHR_DF_MODEL_ETC1S) error(DFD::IncorrectModelForBLZE, blockIndex, toString(khr_df_model_e(block.model)));
    ...
    if (checkGLTFBasisU) {
        ...
        case KHR_DF_MODEL_UASTC:
            if (header.supercompressionScheme != KTX_SS_NONE && header.supercompressionScheme != KTX_SS_ZSTD)
                error(DFD::IncompatibleModelGLTFBU, blockIndex, "KHR_DF_MODEL_UASTC",
                      toString(ktxSupercmpScheme(header.supercompressionScheme)), "KTX_SS_NONE or KTX_SS_ZSTD");
            ...
            if (samples.size() > 0) {
                ...
                default:
                    error(DFD::InvalidChannelGLTFBU, blockIndex, "KHR_DF_MODEL_UASTC", 0,
                          ...
                          "KHR_DF_CHANNEL_UASTC_RGB, KHR_DF_CHANNEL_UASTC_RGBA, KHR_DF_CHANNEL_UASTC_RRR, or "
        ...
        if ((block.primaries != KHR_DF_PRIMARIES_BT709 || block.transfer != KHR_DF_TRANSFER_SRGB) &&
            ...
            error(DFD::InvalidColorSpaceGLTFBU, blockIndex, toString(khr_df_primaries_e(block.primaries)),
                  toString(khr_df_transfer_e(block.transfer)));
    }

    // Validate colorPrimaries
    if (!isColorPrimariesValid(khr_df_primaries_e(block.primaries))) error(DFD::InvalidColorPrimaries, blockIndex, block.primaries);
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        ...
        if (!isChannelTypeValid(khr_df_model_e(block.model), khr_df_model_channels_e(sample.channelType)))
            error(DFD::InvalidChannelForModel, blockIndex, i + 1,
        ...
        if (header.vkFormat != VK_FORMAT_UNDEFINED) {
            if (expectedSamples) {
                if (samples.size() != expectedSamples->size())
                    error(DFD::SampleCountMismatch, blockIndex, samples.size(), toString(VkFormat(header.vkFormat)),
                ...
                for (std::size_t i = 0; i < std::min(samples.size(), expectedSamples->size()); ++i) {
                    ...
                    if (parsed.bitOffset != expected.bitOffset)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "bitOffset", parsed.bitOffset, expected.bitOffset,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.bitLength != expected.bitLength)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "bitLength", parsed.bitLength, expected.bitLength,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.channelType != expected.channelType)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "channelType",
                              ...
                              toString(expectedColorModel.value_or(KHR_DF_MODEL_UNSPECIFIED),
                                       khr_df_model_channels_e(expected.channelType)),
                              toString(VkFormat(header.vkFormat)));
                    ...
                    if (parsed.lower != expected.lower)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "sampleLower", parsed.lower, expected.lower,
                              toString(VkFormat(header.vkFormat)));
                    if (parsed.upper != expected.upper)
                        error(DFD::FormatMismatch, blockIndex, i + 1, "sampleUpper", parsed.upper, expected.upper,
                              toString(VkFormat(header.vkFormat)));
                }
            }

            if (header.supercompressionScheme == KTX_SS_NONE) {
                if (expectedBytePlanes && !std::equal(expectedBytePlanes->begin(), expectedBytePlanes->end(),
                                                      block.bytesPlanes.begin(), block.bytesPlanes.end())) {
                    error(DFD::BytesPlanesMismatch, blockIndex, block.bytesPlanes[0], block.bytesPlanes[1], block.bytesPlanes[2],
                          block.bytesPlanes[3], block.bytesPlanes[4], block.bytesPlanes[5], block.bytesPlanes[6],
                          block.bytesPlanes[7], toString(VkFormat(header.vkFormat)), (*expectedBytePlanes)[0],
                          (*expectedBytePlanes)[1], (*expectedBytePlanes)[2], (*expectedBytePlanes)[3], (*expectedBytePlanes)[4],
                          (*expectedBytePlanes)[5], (*expectedBytePlanes)[6], (*expectedBytePlanes)[7]);
                }
            }

            if (!isFormatBlockCompressed(VkFormat(header.vkFormat)) && !isProhibitedFormat(VkFormat(header.vkFormat))) {
                ...
                if (result >= i_UNSUPPORTED_ERROR_BIT) {
                    ...
                    case i_UNSUPPORTED_MULTIPLE_PLANES:
                        error(DFD::MultiplaneFormatsNotSupported, blockIndex, block.bytesPlanes[0], block.bytesPlanes[1],
                              block.bytesPlanes[2], block.bytesPlanes[3], block.bytesPlanes[4], block.bytesPlanes[5],
            ...
            if (block.model == KHR_DF_MODEL_UASTC) {
                // Validate UASTC
                if (samples.size() != 1) error(DFD::InvalidSampleCount, blockIndex, samples.size(), "UASTC", "1");

                if (!block.matchTexelBlockDimensions(3, 3, 0, 0))
                    error(DFD::InvalidTexelBlockDimension, blockIndex, block.texelBlockDimension0 + 1,
                          block.texelBlockDimension1 + 1, block.texelBlockDimension2 + 1, block.texelBlockDimension3 + 1, 4, 4, 1,
                          1, "UASTC");

                if (header.supercompressionScheme == KTX_SS_NONE) {
                    if (block.bytesPlanes[0] != 16 || block.bytesPlanes[1] != 0 || block.bytesPlanes[2] != 0 ||
                        block.bytesPlanes[3] != 0 || block.bytesPlanes[4] != 0 || block.bytesPlanes[5] != 0 ||
                        block.bytesPlanes[6] != 0 || block.bytesPlanes[7] != 0) {
                        error(DFD::BytesPlanesMismatch, blockIndex, block.bytesPlanes[0], block.bytesPlanes[1],
                              block.bytesPlanes[2], block.bytesPlanes[3], block.bytesPlanes[4], block.bytesPlanes[5],
                              block.bytesPlanes[6], block.bytesPlanes[7], "UASTC", 16, 0, 0, 0, 0, 0, 0, 0);
                    }
                } else {
                    if (block.hasNonZeroBytePlane())
                        error(DFD::BytesPlanesNotUnsized, blockIndex, block.bytesPlanes[0], block.bytesPlanes[1],
                              block.bytesPlanes[2], block.bytesPlanes[3], block.bytesPlanes[4], block.bytesPlanes[5],
                              block.bytesPlanes[6], block.bytesPlanes[7], "UASTC");
                }

                if (samples.size() > 0) {
                    if (samples[0].bitOffset != 0) error(DFD::InvalidBitOffsetForUASTC, blockIndex, 1, samples[0].bitOffset);

                    if (samples[0].bitLength != 127) error(DFD::InvalidBitLengthForUASTC, blockIndex, 1, samples[0].bitLength);

                    if (samples[0].lower != 0) error(DFD::InvalidLower, blockIndex, 1, samples[0].lower, "UASTC", 0);
                ...
                if (samples.size() == 0) error(DFD::ZeroSamples, blockIndex);

                if (header.supercompressionScheme == KTX_SS_NONE) {
                    if (block.bytesPlanes[0] == 0) error(DFD::BytesPlane0Zero, blockIndex, block.bytesPlanes[0]);
                    if (block.bytesPlanes[1] != 0 || block.bytesPlanes[2] != 0 || block.bytesPlanes[3] != 0 ||
                        block.bytesPlanes[4] != 0 || block.bytesPlanes[5] != 0 || block.bytesPlanes[6] != 0 ||
                        block.bytesPlanes[7] != 0)
                        error(DFD::MultiplaneFormatsNotSupported, blockIndex, block.bytesPlanes[0], block.bytesPlanes[1],
                              block.bytesPlanes[2], block.bytesPlanes[3], block.bytesPlanes[4], block.bytesPlanes[5],
                              block.bytesPlanes[6], block.bytesPlanes[7]);
                }
    ...
    case KTX_SS_BASIS_LZ:
        ...
        if (!block.matchTexelBlockDimensions(3, 3, 0, 0))
            error(DFD::InvalidTexelBlockDimension, blockIndex, block.texelBlockDimension0 + 1, block.texelBlockDimension1 + 1,
                  block.texelBlockDimension2 + 1, block.texelBlockDimension3 + 1, 4, 4, 1, 1, "BasisLZ/ETC1S");

        if (block.hasNonZeroBytePlane())
            error(DFD::BytesPlanesNotUnsized, blockIndex, block.bytesPlanes[0], block.bytesPlanes[1], block.bytesPlanes[2],
                  block.bytesPlanes[3], block.bytesPlanes[4], block.bytesPlanes[5], block.bytesPlanes[6], block.bytesPlanes[7],
                  "BasisLZ/ETC1S");
        ...
        for (uint32_t i = 0; i < std::min(static_cast<uint32_t>(samples.size()), 2u); ++i) {
            ...
            if (samples[i].bitLength != 63u) error(DFD::InvalidBitLengthForBLZE, blockIndex, i + 1, samples[i].bitLength);

            if (samples[i].lower != 0) error(DFD::InvalidLower, blockIndex, i + 1, samples[i].lower, "BasisLZ/ETC1S", 0);
    ...
    if (header.vkFormat == VK_FORMAT_UNDEFINED &&
        !isSupercompressionBlockCompressed(ktxSupercmpScheme(header.supercompressionScheme))) {
        ...
        if (isColorModelBlockCompressed(khr_df_model_e(block.model))) {
            if (header.pixelHeight == 0) error(HeaderData::BlockCompressedNoHeight, toString(khr_df_model_e(block.model)));

            if (header.levelCount == 0) error(HeaderData::BlockCompressedNoLevel, toString(khr_df_model_e(block.model)));
        }
    }
    ...
    if (header.vkFormat != VK_FORMAT_UNDEFINED && !isFormatBlockCompressed(static_cast<VkFormat>(header.vkFormat))) {
        ...
        if (isColorModelBlockCompressed(khr_df_model_e(block.model))) {
            if (header.typeSize != 1) error(HeaderData::TypeSizeNotOne, toString(khr_df_model_e(block.model)));
        } else {
...
void ValidationContext::validateKVD() {
    ...
    if (kvdByteOffset == 0 || kvdByteLength == 0) return;  // There is no KVD block
    ...
    struct KeyValueEntry {
        ...
        KeyValueEntry(std::string_view key, const uint8_t* data, uint32_t size) : key(key), data(data), size(size) {}
    };
    ...
    while (ptrEntry < ptrKVDEnd) {
        ...
        if (sizeKeyValuePair < 2) {
            ...
            uint32_t sizeKey = 0;
            while (sizeKey < sizeKeyValuePair && ptrKey[sizeKey] != '\0') ++sizeKey;
            ...
            if (sizeKey == 0) {
                ...
                if (auto invalidIndex = validateUTF8(key)) error(Metadata::KeyInvalidUTF8, key, *invalidIndex);

                if (!keyHasNullTerminator) error(Metadata::KeyMissingNullTerminator, key);

                entries.emplace_back(key, ptrValue, sizeValue);

                if (!keys.emplace(key).second) error(Metadata::DuplicateKey, key);
            }
        }
    ...
    if (foundKTXanimData && foundKTXcubemapIncomplete) error(Metadata::KTXanimDataWithCubeIncomplete);
...
void ValidationContext::validateKTXcubemapIncomplete(const uint8_t* data, uint32_t size) {
    ...
    if (size != 1) error(Metadata::KTXcubemapIncompleteInvalidSize, size);

    if (size == 0) return;

    uint8_t value = *data;

    if (size > 0 && (value & 0b11000000u) != 0) error(Metadata::KTXcubemapIncompleteInvalidBitSet, value);
    value = value & 0b00111111u;  // Error recovery

    const auto popCount = popcount(value);
    if (popCount == 6) warning(Metadata::KTXcubemapIncompleteAllBitsSet);

    if (popCount == 0) error(Metadata::KTXcubemapIncompleteNoBitSet);
    ...
    if (header.faceCount != 1) error(Metadata::KTXcubemapIncompleteWithFaceCountNot1, header.faceCount);

    if (header.pixelHeight != header.pixelWidth) error(HeaderData::CubeHeightWidthMismatch, header.pixelWidth, header.pixelHeight);
...
void ValidationContext::validateKTXorientation(const uint8_t* data, uint32_t size) {
    ...
    if (!hasNull) error(Metadata::KTXorientationMissingNull);

    const auto value = std::string_view(reinterpret_cast<const char*>(data), hasNull ? size - 1 : size);

    if (value.size() != dimensionCount) error(Metadata::KTXorientationIncorrectDimension, value.size(), dimensionCount);
    ...
    if (checkGLTFBasisU && value != "rd") error(Metadata::KTXorientationInvalidGLTFBU, value);
}

void ValidationContext::validateKTXglFormat(const uint8_t* data, uint32_t size) {
    foundKTXglFormat = true;

    if (header.vkFormat != VK_FORMAT_UNDEFINED) error(Metadata::KTXglFormatWithVkFormat, toString(VkFormat(header.vkFormat)));
...
void ValidationContext::validateKTXdxgiFormat(const uint8_t* data, uint32_t size) {
    ...
    if (header.vkFormat != VK_FORMAT_UNDEFINED) error(Metadata::KTXdxgiFormatWithVkFormat, toString(VkFormat(header.vkFormat)));

    if (size != 4) error(Metadata::KTXdxgiFormatInvalidSize, size);
}
...
void ValidationContext::validateKTXmetalPixelFormat(const uint8_t* data, uint32_t size) {
    ...
    if (size != 4) error(Metadata::KTXmetalPixelFormatInvalidSize, size);
}
...
void ValidationContext::validateKTXswizzle(const uint8_t* data, uint32_t size) {
    ...
    const auto hasNull = size > 0 && data[size - 1] == '\0';
    if (!hasNull) error(Metadata::KTXswizzleMissingNull);

    const auto value = std::string_view(reinterpret_cast<const char*>(data), hasNull ? size - 1 : size);

    if (value.size() != 4) error(Metadata::KTXswizzleInvalidSize, size);

    for (std::size_t i = 0; i < std::min(std::size_t{4}, value.size()); ++i) {
        if (value.size() > i && !contains("rgba01", value[i])) error(Metadata::KTXswizzleInvalidValue, i, value[i]);
    }
    ...
    if (checkGLTFBasisU && value != "rgba") error(Metadata::KTXswizzleInvalidGLTFBU, value);
}
...
void ValidationContext::validateKTXwriter(const uint8_t* data, uint32_t size) {
    ...
    const auto hasNull = size > 0 && data[size - 1] == '\0';
    if (!hasNull) error(Metadata::KTXwriterMissingNull);

    const auto value = std::string_view(reinterpret_cast<const char*>(data), hasNull ? size - 1 : size);
    if (auto invalidIndex = validateUTF8(value)) error(Metadata::KTXwriterInvalidUTF8, *invalidIndex);
}
...
void ValidationContext::validateKTXwriterScParams(const uint8_t* data, uint32_t size) {
    ...
    const auto hasNull = size > 0 && data[size - 1] == '\0';
    if (!hasNull) error(Metadata::KTXwriterScParamsMissingNull);

    const auto value = std::string_view(reinterpret_cast<const char*>(data), hasNull ? size - 1 : size);
    if (auto invalidIndex = validateUTF8(value)) error(Metadata::KTXwriterScParamsInvalidUTF8, *invalidIndex);
}
...
void ValidationContext::validateKTXastcDecodeMode(const uint8_t* data, uint32_t size) {
    ...
    const auto hasNull = size > 0 && data[size - 1] == '\0';
    if (!hasNull) error(Metadata::KTXastcDecodeModeMissingNull);

    const auto value = std::string_view(reinterpret_cast<const char*>(data), hasNull ? size - 1 : size);

    if (value != "rgb9e5" && value != "unorm8") error(Metadata::KTXastcDecodeModeInvalidValue, value);
...
void ValidationContext::validateKTXanimData(const uint8_t* data, uint32_t size) {
    ...
    if (size != 12) error(Metadata::KTXanimDataInvalidSize, size);

    if (header.layerCount == 0) error(Metadata::KTXanimDataNotArray, header.layerCount);
}
...
void ValidationContext::validateSGD() {
    ...
    if (sgdByteOffset == 0 || sgdByteLength == 0) return;  // There is no SGD block

    const auto buffer = std::make_unique<uint8_t[]>(sgdByteLength);
    read(sgdByteOffset, buffer.get(), sgdByteLength, "the SGD");

    if (header.supercompressionScheme != KTX_SS_BASIS_LZ) return;
    ...
    const uint64_t expectedBgdByteLength = sizeof(ktxBasisLzGlobalHeader) + sizeof(ktxBasisLzEtc1sImageDesc) * imageCount +
                                           bgh.endpointsByteLength + bgh.selectorsByteLength + bgh.tablesByteLength +
                                           bgh.extendedByteLength;
    if (sgdByteLength != expectedBgdByteLength) error(SGD::BLZESizeIncorrect, sgdByteLength, imageCount, expectedBgdByteLength);
    ...
    // Validate ImageDesc
    if (sgdByteLength < sizeof(ktxBasisLzGlobalHeader) + sizeof(ktxBasisLzEtc1sImageDesc) * imageCount) return;
        ...
        for (uint32_t layer = 0; layer < numLayers; ++layer) {
            for (uint32_t face = 0; face < header.faceCount; ++face) {
                for (uint32_t zSlice = 0; zSlice < std::max(header.pixelDepth >> level, 1u); ++zSlice) {
                    ...
                    if (image.imageFlags & ETC1S_P_FRAME) foundPFrame = true;
                    ...
                    if (image.rgbSliceByteOffset + image.rgbSliceByteLength > levelIndices[level].byteLength)
                        error(SGD::BLZEInvalidRGBSlice, level, layer, face, zSlice, image.rgbSliceByteOffset,
                              image.rgbSliceByteLength, levelIndices[level].byteLength);
                    if (image.alphaSliceByteOffset + image.alphaSliceByteLength > levelIndices[level].byteLength)
                        error(SGD::BLZEInvalidAlphaSlice, level, layer, face, zSlice, image.alphaSliceByteOffset,
                              image.alphaSliceByteLength, levelIndices[level].byteLength);
    ...
    if (foundPFrame)
        if (!foundKTXanimData) error(SGD::BLZENoAnimationSequencesPFrame);
}
...
void ValidationContext::validatePaddings() {
    ...
    const auto check = [&](size_t offset, size_t size, std::string name) {
        if (offset == 0 || size == 0) return;  // Block is missing, skip
...
void ValidationContext::validateCreateAndTranscode() {
    ...
    if (checkGLTFBasisU) flags |= KTX_TEXTURE_CREATE_CHECK_GLTF_BASISU_BIT;
    ...
    if (numError == 0 && numWarning == 0) {
        ...
        case KTX_DECOMPRESS_LENGTH_ERROR:
            error(LevelIndex::UncompressedByteLengthMismatch, toString(ktxSupercmpScheme(header.supercompressionScheme)));
            break;

        case KTX_DECOMPRESS_CHECKSUM_ERROR:
            error(Validator::DecompressChecksumError, toString(ktxSupercmpScheme(header.supercompressionScheme)));
            break;
        ...
        case KTX_SUCCESS:
            ...
            if (result != KTX_SUCCESS) error(Validator::TranscodeFailure, toString(*parsedColorModel), ktxErrorString(result));
            break;
