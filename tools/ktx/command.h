...
struct OptionsGeneric {
    ...
    void init(cxxopts::Options& opts) {
        opts.add_options()("h,help", "Print this usage message and exit")(
...
struct OptionsSingleIn {
    ...
    void init(cxxopts::Options& opts) {
        opts.add_options()("stdin",
                           "Use stdin as the input file. (Using a single dash '-' as the input "
...
struct OptionsSingleInSingleOut {
    ...
    void init(cxxopts::Options& opts) {
        opts.add_options()("stdin",
                           ...
                           "file has the same effect)")(
            "stdout",
            "Use stdout as the output file. (Using a single dash '-' as the output file has the "
            "same effect)")(
            "i,input-file",
            "The input file. Using a single dash '-' as the input file will use stdin.",
            cxxopts::value<std::string>(), "filepath")(
            "o,output-file",
            "The output file. Using a single dash '-' as the output file will use stdout.",
            cxxopts::value<std::string>(), "filepath");
    ...
    void process(cxxopts::Options&, cxxopts::ParseResult& args, Reporter& report) {
        ...
        if (args.count("stdout") + args.count("output-file") == 0)
            report.fatal_usage(
                "Missing output file. Either <output-file> or --stdout must be specified.");
        if (args.count("stdout") + args.count("output-file") > 1)
            report.fatal_usage(
                "Conflicting options: Only one can be specified from <output-file> and --stdout.");
...
struct OptionsMultiInSingleOut {
    ...
    void init(cxxopts::Options& opts) {
        opts.add_options()("stdin",
                           "Use stdin as the first input file. (Using a single dash '-' as the "
    ...
    void process(cxxopts::Options&, cxxopts::ParseResult& args, Reporter& report) {
        ...
        if (files.size() < 2)
            report.fatal_usage("{} file must be specified.",
                               args.count("stdout") == 0 ? "Output" : "Input");
