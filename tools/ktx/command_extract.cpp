...
@section ktx_extract_description DESCRIPTION
    ...
    - Where the @e _face{} part is only present if the source texture is cubemap or cubemap array
(Cubemap)
...
class CommandExtract : public Command {
  ...
  private:
    void executeExtract();
    void saveRawFile(std::string filepath, bool appendExtension, const char* data,
                     std::size_t size);
    ...
    void savePNG(std::string filepath, bool appendExtension, VkFormat vkFormat,
                 const FormatDescriptor& format, uint32_t width, uint32_t height, const char* data,
                 std::size_t size);
    void saveEXR(std::string filepath, bool appendExtension, VkFormat vkFormat,
                 const FormatDescriptor& format, uint32_t width, uint32_t height, int pixelType,
    ...
    void decodeAndSaveASTC(std::string filepath, bool appendExtension, VkFormat vkFormat,
                           const FormatDescriptor& format, uint32_t width, uint32_t height,
                           const char* data, std::size_t size);
    void unpackAndSave422(std::string filepath, bool appendExtension, VkFormat vkFormat,
                          const FormatDescriptor& format, uint32_t width, uint32_t height,
                          const char* data, std::size_t size);
};
...
int CommandExtract::main(int argc, char* argv[]) {
    try {
        parseCommandLine("ktx extract",
                         "Extract one or multiple images from the KTX2 file specified as the "
                         "input-file argument\n"
                         "    and, based on the format, save them as Raw, EXR or PNG image files "
                         "to the output-path.",
                         argc, argv);
        executeExtract();
        return +rc::SUCCESS;
...
void CommandExtract::OptionsExtract::init(cxxopts::Options& opts) {
    opts.add_options()(kOutput,
                       "Output filepath for single, output directory for multiple image export.",
                       cxxopts::value<std::string>(), "<filepath>")(
        kStdout,
        "Use stdout as the output file. (Using a single dash '-' as the output file has the same "
        "effect)")(kTranscode,
                   "Transcode the texture to the target format before executing the extract steps."
                   ...
                   "<target>")(kUri, "KTX Fragment URI.", cxxopts::value<std::string>(), "<uri>")(
        kLevel, "Level to extract. When 'all' is used every level is exported. Defaults to 0.",
        cxxopts::value<std::string>(), "[0-9]+ | all")(
        kLayer, "Layer to extract. When 'all' is used every layer is exported. Defaults to 0.",
        cxxopts::value<std::string>(), "[0-9]+ | all")(
        kFace, "Face to extract. When 'all' is used every face is exported. Defaults to 0.",
        cxxopts::value<std::string>(), "[0-5] | all")(
        kDepth,
        "Depth slice to extract. When 'all' is used every depth is exported. Defaults to 0.",
        cxxopts::value<std::string>(),
        "[0-9]+ | all")(kAll, "Extract every image slice from the texture.")(
        kRaw, "Extract the raw image data without any conversion.");
}

void CommandExtract::OptionsExtract::process(cxxopts::Options&, cxxopts::ParseResult& args,
                                             Reporter& report) {
    ...
    const auto parseSelector = [&](const std::string& name,
                                   bool& found) -> std::optional<SelectorRange> {
        if (!args[name].count()) return std::nullopt;
        ...
        try {
            ...
            report.fatal_usage(
                "Invalid {} value \"{}\". The value must be a either a number or \"all\".", name,
                str);
    ...
    if (globalAll) {
        if (level) report.fatal_usage("Conflicting options: --level cannot be used with --all.");
        if (layer) report.fatal_usage("Conflicting options: --layer cannot be used with --all.");
        if (face) report.fatal_usage("Conflicting options: --face cannot be used with --all.");
        if (depth_) report.fatal_usage("Conflicting options: --depth cannot be used with --all.");
    ...
    if (args[kUri].count()) {
        ...
        if (globalAll) report.fatal_usage("Conflicting options: --all cannot be used with --uri.");
        if (levelFlagUsed)
...
void CommandExtract::processOptions(cxxopts::Options& opts, cxxopts::ParseResult& args) {
    ...
    if (!options.raw && options.transcodeTarget) {
        if (options.transcodeTarget != KTX_TTF_RGBA32)
            fatal_usage(
                "Transcode to \"{}\" for non-raw extract is not supported. "
                "For PNG/EXR output only r8, rg8, rgb8 and rgba8 are supported.",
...
void CommandExtract::executeExtract() {
    ...
    if (!options.fragmentURI.mip.validate(texture->numLevels))
        fatal(rc::INVALID_FILE,
              "Requested level index {} is missing. The input file only has {} level(s).",
              options.fragmentURI.mip, texture->numLevels);

    if (((options.uriFlagUsed && !options.fragmentURI.stratal.is_undefined()) ||
         options.layerFlagUsed) &&
        !texture->isArray) {
        if (options.fragmentURI.stratal == all)
            fatal(rc::INVALID_FILE, "Requested all layers from a non-array texture.");
        else
            fatal(rc::INVALID_FILE, "Requested layer index {} from a non-array texture.",
                  options.fragmentURI.stratal);
    }

    if (!options.fragmentURI.stratal.validate(texture->numLayers))
        fatal(rc::INVALID_FILE,
              "Requested layer index {} is missing. The input file only has {} layer(s).",
              options.fragmentURI.stratal, texture->numLayers);

    if (((options.uriFlagUsed && !options.fragmentURI.facial.is_undefined()) ||
         options.faceFlagUsed) &&
        !texture->isCubemap) {
        if (options.fragmentURI.facial == all)
            fatal(rc::INVALID_FILE, "Requested all faces from a non-cubemap texture.");
        else
            fatal(rc::INVALID_FILE, "Requested face index {} from a non-cubemap texture.",
                  options.fragmentURI.facial);
    }

    if (!options.fragmentURI.facial.validate(texture->numFaces))
        fatal(rc::INVALID_FILE,
              "Requested face index {} is missing. The input file only has {} face(s).",
    ...
    if (!options.globalAll && options.depthFlagUsed && texture->numDimensions != 3) {
        if (options.depth == all)
            ...
            fatal(rc::INVALID_FILE, "Requested depth slice index {} from a non-3D texture.",
                  options.depth);
    }
    ...
    const auto lastExportedLevel =
        options.fragmentURI.mip == all ? texture->numLevels - 1 : options.fragmentURI.mip.last();
    const auto lastExportedLevelDepthCount =
        std::max(1u, ceil_div(texture->baseDepth, blockSizeZ) >> lastExportedLevel);
    if (options.depthFlagUsed && options.depth != all &&
        options.depth.last() > lastExportedLevelDepthCount)
        fatal(rc::INVALID_FILE,
              "Requested depth slice index {} is missing. The input file only has {} depth "
              "slice(s) in level {}.",
    ...
    try {
        ...
        if (isMultiOutput) {
            if (std::filesystem::exists(outputPath) && !std::filesystem::is_directory(outputPath))
                fatal_usage(
                    "Specified output path must be a directory for multi-output extract: \"{}\".",
                    options.outputPath);
        ...
        fatal(rc::IO_FAILURE, "Failed to create the output directory \"{}\": {}.",
              e.path1().generic_string(), e.what());
    }

    // Iterate
    for (uint32_t levelIndex = 0; levelIndex < texture->numLevels; ++levelIndex) {
        if (options.fragmentURI.mip.is_undefined() ? levelIndex != 0
        ...
        for (uint32_t layerIndex = 0; layerIndex < texture->numLayers; ++layerIndex) {
            ...
            for (uint32_t faceIndex = 0; faceIndex < texture->numFaces; ++faceIndex) {
                ...
                if (imageDepth > 1 && !options.globalAll && !options.depthFlagUsed && options.raw) {
                    // If the texture type is 3D / 3D Array and the "all" or "depth" option is not
                    // set, the whole 3D block of pixel data is selected according to the "level"
                    // and "layer" option. This extraction path requires the "raw" option to be
                    // enabled.
                    ...
                    const auto outputFilepath =
                        !isMultiOutput
                            ...
                            : fmt::format(
                                  ...
                                  // Depth is not part of the name as the whole 3D image is raw
                                  // exported
                              );
                    ...
                    for (uint32_t depthIndex = 0; depthIndex < imageDepth; ++depthIndex) {
                        ...
                        const char* imageData =
                            reinterpret_cast<const char*>(texture->pData) + imageOffset;
                ...
                for (uint32_t depthIndex = 0; depthIndex < ceil_div(imageDepth, blockSizeZ);
                     ++depthIndex) {
                    if (!options.depth.contains(depthIndex)) continue;  // Skip
                    ...
                    if (options.raw) {
                        ...
                        saveImageFile(outputFilepath, isMultiOutput, depthSliceData, imageSize,
                                      static_cast<VkFormat>(texture->vkFormat), format, imageWidth,
                                      imageHeight);
                    }
                }
            }
...
void CommandExtract::decodeAndSaveASTC(std::string filepath, bool appendExtension,
    ...
    if (ec != ASTCENC_SUCCESS)
        fatal(rc::RUNTIME_ERROR, "ASTC Codec context alloc failed: {}",
    ...
    const auto uncompressedVkFormat =
        isFormatSRGB(vkFormat) ? VK_FORMAT_R8G8B8A8_SRGB : VK_FORMAT_R8G8B8A8_UNORM;
...
void CommandExtract::unpackAndSave422(std::string filepath, bool appendExtension, VkFormat vkFormat,
                                      const FormatDescriptor& format, uint32_t width,
                                      uint32_t height, const char* data, std::size_t size) {
    (void)vkFormat;
    ...
    // Reuse similar 4 channel VkFormats and drop the last channel (There is no RGB variant of 10X6
    // and 12X4)
    const auto precision = format.find(KHR_DF_CHANNEL_YUVSDA_Y)->bitLength + 1u;
    auto unpackedFormat =
        createFormatDescriptor(precision == 8    ? VK_FORMAT_R8G8B8A8_UNORM
    ...
    // 1 pixel (block) with 4 channel is unpacked to 2 pixel with 3 channels: Y0,Y1,U,V ->
    // R0,G0,B0,R1,G1,B1
    ...
    for (uint32_t y = 0; y < height; y++) {
        for (uint32_t x = 0; x < blockCountX; x++) {
            const char* rawYUVBlockLeft =
                data + (y * blockCountX + (x == 0 ? 0 : x - 1)) * blockYUVBytes;
            const char* rawYUVBlock = data + (y * blockCountX + x) * blockYUVBytes;
            const char* rawYUVBlockRight =
                data + (y * blockCountX + (x == blockCountX - 1 ? blockCountX - 1 : x + 1)) *
                           blockYUVBytes;

            char* pixel0 = unpackedData.data() + (y * width + x * 2) * pixelBytes;
            char* pixel1 = unpackedData.data() + (y * width + x * 2 + 1) * pixelBytes;

            const float valueLeftY1 = convertUNORMToFloat(
                extract_bits<uint32_t>(rawYUVBlockLeft, y1Offset, y1Bits), y1Bits);
            const float valueLeftU =
                convertUNORMToFloat(extract_bits<uint32_t>(rawYUVBlockLeft, uOffset, uBits), uBits);
            const float valueLeftV =
                convertUNORMToFloat(extract_bits<uint32_t>(rawYUVBlockLeft, vOffset, vBits), vBits);
            const float valueY0 =
                convertUNORMToFloat(extract_bits<uint32_t>(rawYUVBlock, y0Offset, y0Bits), y0Bits);
            const float valueY1 =
                convertUNORMToFloat(extract_bits<uint32_t>(rawYUVBlock, y1Offset, y1Bits), y1Bits);
            const float valueU =
                convertUNORMToFloat(extract_bits<uint32_t>(rawYUVBlock, uOffset, uBits), uBits);
            const float valueV =
                convertUNORMToFloat(extract_bits<uint32_t>(rawYUVBlock, vOffset, vBits), vBits);
            const float valueRightY0 = convertUNORMToFloat(
                extract_bits<uint32_t>(rawYUVBlockRight, y0Offset, y0Bits), y0Bits);
            const float valueRightU = convertUNORMToFloat(
                extract_bits<uint32_t>(rawYUVBlockRight, uOffset, uBits), uBits);
            const float valueRightV = convertUNORMToFloat(
                extract_bits<uint32_t>(rawYUVBlockRight, vOffset, vBits), vBits);

            const auto interpolateY = [](float pos, float pos0, float value0, float pos1,
                                         float value1, float pos2, float value2, float pos3,
                                         float value3) {
                if (pos < pos1)
            ...
            const auto interpolateUV = [](float pos, float pos0, float value0, float pos1,
                                          float value1, float pos2, float value2) {
                if (pos < pos1)
                    return remap(pos, pos0, pos1, value0, value1);
                else
                    return remap(pos, pos1, pos2, value1, value2);
            ...
            const auto setPixel = [&](auto* pixel, float pos) {
                const auto r = convertFloatToUNORM(
                    interpolateUV(pos, positionV - blockSize, valueLeftV, positionV, valueV,
                                  positionV + blockSize, valueRightV),
                    uBits);
                const auto g = convertFloatToUNORM(
                    interpolateY(pos, positionY1 - blockSize, valueLeftY1, positionY0, valueY0,
                                 positionY1, valueY1, positionY0 + blockSize, valueRightY0),
                    y0Bits);
                const auto b = convertFloatToUNORM(
                    interpolateUV(pos, positionU - blockSize, valueLeftU, positionU, valueU,
                                  positionU + blockSize, valueRightU),
                    uBits);
...
void CommandExtract::saveRawFile(std::string filepath, bool appendExtension, const char* data,
                                 std::size_t size) {
    if (appendExtension && filepath != "-") filepath += ".raw";
    OutputStream file(filepath, *this);
    file.write(data, size, *this);
}

void CommandExtract::savePNG(std::string filepath, bool appendExtension, VkFormat vkFormat,
                             const FormatDescriptor& format, uint32_t width, uint32_t height,
                             const char* data, std::size_t size) {
    if (appendExtension && filepath != "-") filepath += ".png";
    ...
    if (format.model() != KHR_DF_MODEL_RGBSDA)
        fatal(rc::NOT_SUPPORTED, "PNG saving is not supported for {} with {}.",
              toString(format.model()), toString(vkFormat));
    ...
    if (const auto sample = format.find(KHR_DF_CHANNEL_RGBSDA_D)) {
        if (!depthFormat)
            fatal(rc::NOT_SUPPORTED,
                  "PNG saving encountered unexpected depth channel in non-depth format {}.",
                  toString(vkFormat));
    ...
    const auto includeSBits = (srcBits[0] != 0 && srcBits[0] != bitDepth) ||
    ...
    for (uint32_t y = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++) {
            ...
            for (uint32_t packedChannelIndex = 0; packedChannelIndex < packedChannelCount;
                 ++packedChannelIndex) {
                ...
                const uint32_t value =
                    convertUNORM(extract_bits<uint32_t>(rawPixel, offset, bits), bits, bitDepth);
                if (byteDepth == 1) {
                    uint8_t temp = static_cast<uint8_t>(value);
                    std::memcpy(unpackedImage.data() + (y * width * unpackedChannelCount +
                                                        x * unpackedChannelCount + dstChannel) *
                                                           byteDepth,
                                &temp, byteDepth);
                } else if (byteDepth == 2) {
                    uint16_t temp = static_cast<uint16_t>(value);
                    if (!is_big_endian) temp = byteswap(temp);  // LodePNG Uses big endian input
                    std::memcpy(unpackedImage.data() + (y * width * unpackedChannelCount +
                                                        x * unpackedChannelCount + dstChannel) *
                                                           byteDepth,
                                &temp, byteDepth);
...
void CommandExtract::saveEXR(std::string filepath, bool appendExtension, VkFormat vkFormat,
                             const FormatDescriptor& format, uint32_t width, uint32_t height,
                             int pixelType, const char* data, std::size_t size) {
    saveEXR(
        std::move(filepath), appendExtension, vkFormat, format, width, height,
        std::vector<int>(vkFormat == VK_FORMAT_E5B9G9R9_UFLOAT_PACK32 ? 3 : format.channelCount(),
                         pixelType),
        data, size);
}
...
void CommandExtract::saveEXR(std::string filepath, bool appendExtension, VkFormat vkFormat,
                             const FormatDescriptor& format, uint32_t width, uint32_t height,
                             const std::vector<int>& pixelTypes, const char* data,
                             std::size_t size) {
    assert(!format.samples.empty());

    if (appendExtension && filepath != "-") filepath += ".exr";
    ...
    if (format.model() == KHR_DF_MODEL_RGBSDA) {
        const auto addChannel = [&](auto channelType, const char* channelName) {
            if (const auto sample = format.find(channelType))
                channels.push_back(
                    {sample->bitOffset, sample->bitLength + 1u, channelName,
                     sample->qualifierFloat != 0, sample->qualifierSigned != 0,
                     sample->upper !=
                         (sample->qualifierFloat != 0 ? bit_cast<uint32_t>(1.0f) : 1u)});
        };
        ...
        fatal(rc::NOT_SUPPORTED, "EXR saving is unsupported for {} with {}.",
              toString(format.model()), toString(vkFormat));
    }

    const auto pixelBytes = format.pixelByteCount();
    const auto numChannels =
        vkFormat == VK_FORMAT_E5B9G9R9_UFLOAT_PACK32 ? 3 : format.channelCount();
    assert(size == width * height * pixelBytes);
    (void)size;
    ...
    for (uint32_t i = 0; i < numChannels; ++i) images[i].resize(width * height);
        ...
        for (uint32_t x = 0; x < width; x++) {
            ...
            if (vkFormat == VK_FORMAT_E5B9G9R9_UFLOAT_PACK32) {
                ...
                for (uint32_t c = 0; c < numChannels; c++) {
                    ...
                    if (pixelTypes[c] == TINYEXR_PIXELTYPE_FLOAT ||
                        pixelTypes[c] == TINYEXR_PIXELTYPE_HALF) {
                        if (channel.isFloat) {
                            if (channel.isSigned)
    ...
    struct EXRStruct {
        ...
        void addAttributesToHeader() {
            ...
            header.custom_attributes =
                (EXRAttribute*)malloc(sizeof(EXRAttribute) * attributes.size());
    ...
    for (int i = 0; i < exr.header.num_channels; i++) {
        ...
        exr.header.pixel_types[i] = (pixelTypes[i] == TINYEXR_PIXELTYPE_UINT)
    ...
    std::size_t fileDataSize =
        SaveEXRImageToMemory(&exr.image, &exr.header, &exr.fileData, &exr.err);
    if (fileDataSize == 0) fatal(rc::IO_FAILURE, "EXR Encoder error: {}.", exr.err);

    OutputStream file(filepath, *this);
    file.write(reinterpret_cast<const char*>(exr.fileData), fileDataSize, *this);
}

void CommandExtract::saveImageFile(std::string filepath, bool appendExtension, const char* data,
                                   std::size_t size, VkFormat vkFormat,
                                   const FormatDescriptor& format, uint32_t width,
                                   uint32_t height) {
    switch (vkFormat) {
    ...
    case VK_FORMAT_B16G16R16G16_422_UNORM:
        ...
        unpackAndSave422(std::move(filepath), appendExtension, vkFormat, format, width, height,
                         data, size);
        break;
    ...
    case VK_FORMAT_R8_SINT:
        saveEXR(std::move(filepath), appendExtension, vkFormat, format, width, height,
                TINYEXR_PIXELTYPE_HALF, data, size);
        break;
    case VK_FORMAT_R16_UINT:
        [[fallthrough]];
    case VK_FORMAT_R16_SINT:
        saveEXR(std::move(filepath), appendExtension, vkFormat, format, width, height,
                TINYEXR_PIXELTYPE_FLOAT, data, size);
        break;
    case VK_FORMAT_R32_UINT:
        saveEXR(std::move(filepath), appendExtension, vkFormat, format, width, height,
                TINYEXR_PIXELTYPE_UINT, data, size);
        break;
    ...
    case VK_FORMAT_D32_SFLOAT_S8_UINT:
        saveEXR(std::move(filepath), appendExtension, vkFormat, format, width, height,
                {TINYEXR_PIXELTYPE_FLOAT, TINYEXR_PIXELTYPE_HALF}, data, size);
        break;
