...
class CommandExtract : public Command {
  ...
  private:
    ...
    void saveImageFile(std::string filepath, bool appendExtension, const char* data, std::size_t size, VkFormat vkFormat,
                       const FormatDescriptor& format, uint32_t width, uint32_t height);

    void savePNG(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format, uint32_t width,
                 uint32_t height, const char* data, std::size_t size);
    void saveEXR(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format, uint32_t width,
                 uint32_t height, int pixelType, const char* data, std::size_t size);
    void saveEXR(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format, uint32_t width,
                 uint32_t height, const std::vector<int>& pixelTypes, const char* data, std::size_t size);
    void decodeAndSaveASTC(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format,
                           uint32_t width, uint32_t height, const char* data, std::size_t size);
    void unpackAndSave422(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format,
                          uint32_t width, uint32_t height, const char* data, std::size_t size);
};
...
void CommandExtract::OptionsExtract::init(cxxopts::Options& opts) {
    opts.add_options()(kOutput, "Output filepath for single, output directory for multiple image export.",
        ...
        " r8 | rg8 | rgb8 | rgba8."
        "\netc-rgb is ETC1; etc-rgba, eac-r11 and eac-rg11 are ETC2.",
        cxxopts::value<std::string>(), "<target>")(kUri, "KTX Fragment URI.", cxxopts::value<std::string>(), "<uri>")(
        kLevel, "Level to extract. When 'all' is used every level is exported. Defaults to 0.", cxxopts::value<std::string>(),
        "[0-9]+ | all")(kLayer, "Layer to extract. When 'all' is used every layer is exported. Defaults to 0.",
                        cxxopts::value<std::string>(), "[0-9]+ | all")(
        kFace, "Face to extract. When 'all' is used every face is exported. Defaults to 0.", cxxopts::value<std::string>(),
        "[0-5] | all")(kDepth, "Depth slice to extract. When 'all' is used every depth is exported. Defaults to 0.",
                       cxxopts::value<std::string>(), "[0-9]+ | all")(kAll, "Extract every image slice from the texture.")(
...
void CommandExtract::OptionsExtract::process(cxxopts::Options&, cxxopts::ParseResult& args, Reporter& report) {
    ...
    if (globalAll && outputPath == "-") report.fatal_usage("stdout cannot be used with multi-output '--all' extract.");
    if (level == all && outputPath == "-") report.fatal_usage("stdout cannot be used with multi-output '--level all' extract.");
    if (layer == all && outputPath == "-") report.fatal_usage("stdout cannot be used with multi-output '--layer all' extract.");
    if (face == all && outputPath == "-") report.fatal_usage("stdout cannot be used with multi-output '--face all' extract.");
    if (depth_ == all && outputPath == "-") report.fatal_usage("stdout cannot be used with multi-output '--depth all' extract.");

    if (args[kUri].count()) {
        uriFlagUsed = true;

        if (globalAll) report.fatal_usage("Conflicting options: --all cannot be used with --uri.");
        if (levelFlagUsed) report.fatal_usage("Conflicting options: --level cannot be used with --uri.");
        if (layerFlagUsed) report.fatal_usage("Conflicting options: --layer cannot be used with --uri.");
        if (faceFlagUsed) report.fatal_usage("Conflicting options: --face cannot be used with --uri.");
        ...
        const auto isMultiOutputFragmentURI = (!fragmentURI.mip.is_undefined() && fragmentURI.mip.is_multi()) ||
...
void CommandExtract::executeExtract() {
    ...
    if (ktxTexture2_NeedsTranscoding(texture)) {
        ...
        fatal(rc::INVALID_FILE, "Requested transcode \"{}\" but the KTX file is not transcodable.", options.transcodeTargetName);
    }
    ...
    const auto isMultiOutput = (!options.fragmentURI.mip.is_undefined() && options.fragmentURI.mip.is_multi()) ||
    ...
    for (uint32_t levelIndex = 0; levelIndex < texture->numLevels; ++levelIndex) {
        if (options.fragmentURI.mip.is_undefined() ? levelIndex != 0 : !options.fragmentURI.mip.contains(levelIndex)) continue;
        ...
        for (uint32_t layerIndex = 0; layerIndex < texture->numLayers; ++layerIndex) {
            if (options.fragmentURI.stratal.is_undefined() ? layerIndex != 0 : !options.fragmentURI.stratal.contains(layerIndex))
                continue;

            for (uint32_t faceIndex = 0; faceIndex < texture->numFaces; ++faceIndex) {
                if (options.fragmentURI.facial.is_undefined() ? faceIndex != 0 : !options.fragmentURI.facial.contains(faceIndex))
                    continue;
                ...
                if (imageDepth > 1 && !options.globalAll && !options.depthFlagUsed && options.raw) {
                    ...
                    const auto outputFilepath =
                        !isMultiOutput ? options.outputPath
                                       : fmt::format("{}/output{}{}{}.raw", options.outputPath,
                ...
                for (uint32_t depthIndex = 0; depthIndex < ceil_div(imageDepth, blockSizeZ); ++depthIndex) {
                    ...
                    const auto outputFilepath =
                        !isMultiOutput ? options.outputPath
                                       : fmt::format("{}/output{}{}{}{}", options.outputPath,
                                                     texture->numLevels > 1 ? fmt::format("_level{}", levelIndex) : "",
                                                     texture->isCubemap ? fmt::format("_face{}", faceIndex) : "",
                                                     texture->isArray ? fmt::format("_layer{}", layerIndex) : "",
    ...
    if (ret != KTX_SUCCESS) fatal(rc::INVALID_FILE, "Failed to iterate KTX2 texture: {}", ktxErrorString(ret));
}

void CommandExtract::decodeAndSaveASTC(std::string filepath, bool appendExtension, VkFormat vkFormat,
                                       const FormatDescriptor& format, uint32_t width, uint32_t height, const char* compressedData,
    ...
    if (ec != ASTCENC_SUCCESS) fatal(rc::RUNTIME_ERROR, "ASTC Codec config init failed: {}", astcenc_get_error_string(ec));
    ...
    if (ec != ASTCENC_SUCCESS) fatal(rc::RUNTIME_ERROR, "ASTC Codec context alloc failed: {}", astcenc_get_error_string(ec));
    ...
    if (ec != ASTCENC_SUCCESS) fatal(rc::RUNTIME_ERROR, "ASTC Codec decompress failed: {}", astcenc_get_error_string(ec));
    astcenc_decompress_reset(context);

    const auto uncompressedVkFormat = isFormatSRGB(vkFormat) ? VK_FORMAT_R8G8B8A8_SRGB : VK_FORMAT_R8G8B8A8_UNORM;
    saveImageFile(std::move(filepath), appendExtension, reinterpret_cast<const char*>(uncompressedBuffer.get()), uncompressedSize,
                  uncompressedVkFormat, createFormatDescriptor(uncompressedVkFormat, *this), width, height);
}

using namespace imageio;

void CommandExtract::unpackAndSave422(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format,
                                      uint32_t width, uint32_t height, const char* data, std::size_t size) {
    (void)vkFormat;
    ...
    auto unpackedFormat = createFormatDescriptor(precision == 8    ? VK_FORMAT_R8G8B8A8_UNORM
    ...
    for (uint32_t y = 0; y < height; y++) {
        for (uint32_t x = 0; x < blockCountX; x++) {
            ...
            const char* rawYUVBlockRight =
                data + (y * blockCountX + (x == blockCountX - 1 ? blockCountX - 1 : x + 1)) * blockYUVBytes;
            ...
            const auto interpolateY = [](float pos, float pos0, float value0, float pos1, float value1, float pos2, float value2,
                                         float pos3, float value3) {
                if (pos < pos1)
            ...
            const auto setPixel = [&](auto* pixel, float pos) {
                const auto r = convertFloatToUNORM(
                    interpolateUV(pos, positionV - blockSize, valueLeftV, positionV, valueV, positionV + blockSize, valueRightV),
                    uBits);
                const auto g = convertFloatToUNORM(interpolateY(pos, positionY1 - blockSize, valueLeftY1, positionY0, valueY0,
                                                                positionY1, valueY1, positionY0 + blockSize, valueRightY0),
                                                   y0Bits);
                const auto b = convertFloatToUNORM(
                    interpolateUV(pos, positionU - blockSize, valueLeftU, positionU, valueU, positionU + blockSize, valueRightU),
                    uBits);
    ...
    savePNG(filepath, appendExtension, VK_FORMAT_UNDEFINED, unpackedFormat, width, height,
            reinterpret_cast<const char*>(unpackedData.data()), unpackedData.size() * sizeof(decltype(unpackedData)::value_type));
}
...
void CommandExtract::savePNG(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format,
                             uint32_t width, uint32_t height, const char* data, std::size_t size) {
    ...
    const auto includeSBits = (srcBits[0] != 0 && srcBits[0] != bitDepth) || (srcBits[1] != 0 && srcBits[1] != bitDepth) ||
                              (srcBits[2] != 0 && srcBits[2] != bitDepth) || (srcBits[3] != 0 && srcBits[3] != bitDepth);
    if (includeSBits) {
        ...
        for (uint32_t x = 0; x < width; x++) {
            ...
            for (uint32_t packedChannelIndex = 0; packedChannelIndex < packedChannelCount; ++packedChannelIndex) {
                ...
                if (byteDepth == 1) {
                    ...
                    std::memcpy(unpackedImage.data() +
                                    (y * width * unpackedChannelCount + x * unpackedChannelCount + dstChannel) * byteDepth,
                                &temp, byteDepth);
                } else if (byteDepth == 2) {
    ...
    if (error) fatal(rc::INVALID_FILE, "PNG Encoder error {}: {}.", error, lodepng_error_text(error));

    OutputStream file(filepath, *this);
    file.write(reinterpret_cast<const char*>(png.data()), png.size(), *this);
}

void CommandExtract::saveEXR(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format,
                             uint32_t width, uint32_t height, int pixelType, const char* data, std::size_t size) {
    saveEXR(std::move(filepath), appendExtension, vkFormat, format, width, height,
            std::vector<int>(vkFormat == VK_FORMAT_E5B9G9R9_UFLOAT_PACK32 ? 3 : format.channelCount(), pixelType), data, size);
}

void CommandExtract::saveEXR(std::string filepath, bool appendExtension, VkFormat vkFormat, const FormatDescriptor& format,
                             uint32_t width, uint32_t height, const std::vector<int>& pixelTypes, const char* data,
                             std::size_t size) {
    ...
    if (format.model() == KHR_DF_MODEL_RGBSDA) {
        const auto addChannel = [&](auto channelType, const char* channelName) {
            if (const auto sample = format.find(channelType))
                channels.push_back({sample->bitOffset, sample->bitLength + 1u, channelName, sample->qualifierFloat != 0,
...
void CommandExtract::saveImageFile(std::string filepath, bool appendExtension, const char* data, std::size_t size,
