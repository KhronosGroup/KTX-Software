...
int ktxUpgrader::main(int argc, char* argv[]) {
    ...
    for (it = options.infiles.begin(); it < options.infiles.end(); it++) {
        ...
        if (inf) {
            ...
            if (outf) {
                result = ktxTexture1_CreateFromStdioStream(
                    inf, KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &texture);
                if (result != KTX_SUCCESS) {
                    if (result == KTX_UNKNOWN_FILE_FORMAT) {
                        cerr << infile << " is not a KTX v1 file." << endl;
                    } else if (result != KTX_SUCCESS) {
                        cerr << name << " failed to create ktxTexture from " << infile << ": "
                ...
                for (pEntry = texture->kvDataHead; pEntry != NULL;
                ...
                (void)fclose(outf);
                if (result != KTX_SUCCESS) {
                    cerr << name << " failed to write KTX2 file; " << ktxErrorString(result)
                         << endl;
                    (void)unlinkUTF8(options.outfile.c_str());
                    exitCode = 2;
                    goto cleanup;
                ...
                cerr << name << " could not open output file \""
                     << (options.outfile.length() ? options.outfile.c_str() : "stdout") << "\". "
                     << strerror(errno) << endl;
                exitCode = 2;
                goto cleanup;
            }
        } else {
            cerr << name << " could not open input file \""
                 << (infile.compare("-") ? infile : "stdin") << "\". " << strerror(errno) << endl;
            exitCode = 2;
            goto cleanup;
        }
    }
