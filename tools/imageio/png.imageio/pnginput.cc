...
class PngInput final : public ImageInput {
  public:
    ...
    virtual void readImage(void* bufferOut, size_t bufferByteCount, uint32_t subimage,
...
void PngInput::readHeader() {
    ...
    lodepngError = lodepng_decode_chunks(&pIdat, &idatsize, &w, &h, &state,
                                         (const uint8_t*)pngBuffer.data(), pngBuffer.size());
    if (lodepngError) {
        throw std::runtime_error(
            fmt::format("PNG decode chunks error: {}.", lodepng_error_text(lodepngError)));
    }
    ...
    case LCT_PALETTE: {
        ...
        pTrnsChunk = lodepng_chunk_find_const(
            pFirstChunk, (unsigned char*)&pngBuffer[pngBuffer.size() - 1], "tRNS");
        if (pTrnsChunk) {
            state.info_raw.colortype = LCT_RGBA;
            componentCount = 4;
            colorModel = KHR_DF_MODEL_RGBSDA;
    ...
    images.emplace_back(
        ImageSpec(w, h, 1, componentCount, bitDepth,
                  static_cast<khr_df_sample_datatype_qualifiers_e>(0), KHR_DF_TRANSFER_UNSPECIFIED,
    ...
    if (state.info_png.iccp_defined) {
        ...
        format.extended.iccProfile.profile.insert(
            format.extended.iccProfile.profile.begin(), state.info_png.iccp_profile,
    ...
    if (state.info_png.chrm_defined && !state.info_png.srgb_defined &&
        !state.info_png.iccp_defined) {
        Primaries primaries;
...
/// accordingly.
void PngInput::readImage(void* bufferOut, size_t bufferOutByteCount, uint32_t /*subimage*/,
    ...
    if (requestBits != 8 && requestBits != 16)
        throw std::runtime_error(
            fmt::format("PNG decode error: Requested decode into {}-bit format is not supported.",
                        requestBits));
    ...
    state.info_raw.colortype = [&] {
        ...
        throw std::runtime_error(
            fmt::format("PNG decode error: Requested decode into {} channels is not supported.",
                        targetFormat.channelCount()));
    }();
    auto lodepngError = lodepng_finish_decode((unsigned char*)bufferOut, bufferOutByteCount, width,
                                              height, &state, pIdat, idatsize);

    if (lodepngError)
        throw std::runtime_error(
            fmt::format("PNG decode error: {}.", lodepng_error_text(lodepngError)));
                ...
                for (uint32_t c = 0; c < channelCount; ++c) {
                    ...
                    if (requestBits == 8) {
                        ...
                        value = static_cast<uint8_t>(
                            imageio::convertUNORM(value >> (8 - sBits[c]), sBits[c], 8));
                    } else {  // requestBits == 16
                        auto& value = *(reinterpret_cast<uint16_t*>(bufferOut) + index);
                        value = static_cast<uint16_t>(
                            imageio::convertUNORM(value >> (16 - sBits[c]), sBits[c], 16));
                    }
                }
            }
        }
