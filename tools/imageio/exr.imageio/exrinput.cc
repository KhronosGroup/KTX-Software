...
ImageInput* exrInputCreate() { return new ExrInput; }
...
void ExrInput::open(ImageSpec& newspec) {
...

    {
        ...
        if (ec == TINYEXR_ERROR_INVALID_MAGIC_NUMBER) throw different_format();
    ...
    for (int i = 0; i < header.num_channels; i++) {
        ...
        default:
            throw std::runtime_error(
                fmt::format("EXR header decode error: Not supported pixel type: {}.", type));
        }
    }
        ...
        if (attributeName == "chromaticities") {
            ...
            if (header.custom_attributes[i].size != expectedSize) {
                throw std::runtime_error(fmt::format(
                    "EXR chromaticities attribute decode error: Expected size {} but got {}.",
    ...
    images.emplace_back(ImageSpec(width, height, 1, header.num_channels, bitDepth,
                                  static_cast<khr_df_sample_datatype_qualifiers_e>(qualifiers),
                                  KHR_DF_TRANSFER_LINEAR, colorPrimaries, KHR_DF_MODEL_RGBSDA),
                        formatType);
...
void ExrInput::readImage(void* outputBuffer, size_t bufferByteCount, uint32_t subimage,
                         uint32_t miplevel, const FormatDescriptor& requestFormat) {
    assert(subimage == 0);
    (void)subimage;
    assert(miplevel == 0);
    (void)miplevel;
    ...
    if (bufferByteCount != expectedBufferByteCount)
        throw std::runtime_error(
            fmt::format("EXR load error: "
                        "Provided target buffer size is {} does not match the expected value: {}.",
                        bufferByteCount, expectedBufferByteCount));
    ...
    for (uint32_t i = 0; i < numSourceChannels; ++i) {
        if (std::strcmp(header.channels[i].name, "R") == 0)
            ...
            warning(fmt::format("EXR load warning: Unrecognized channel \"{}\" is ignored.",
    ...
    const auto copyData = [&](unsigned char* ptr, uint32_t dataSize, const void* defaultColor) {
        const auto sourcePtr = [&](uint32_t channel, uint32_t x, uint32_t y) {
            return reinterpret_cast<const unsigned char*>(image.images[channel] +
                                                          (y * width + x) * dataSize);
        };
        ...
        for (uint32_t y = 0; y < height; ++y) {
            for (uint32_t x = 0; x < width; ++x) {
                auto* targetPixel =
                    ptr + (y * width * numTargetChannels + x * numTargetChannels) * dataSize;
                for (uint32_t c = 0; c < numTargetChannels; ++c) {
                    if (channels[c].has_value()) {
                        std::memcpy(targetPixel + c * dataSize, sourcePtr(*channels[c], x, y),
                                    dataSize);
                    } else {
                        std::memcpy(targetPixel + c * dataSize,
                                    static_cast<const uint8_t*>(defaultColor) + c * dataSize,
                                    dataSize);
                    }
                }
            }
    ...
    case TINYEXR_PIXELTYPE_HALF: {
        ...
        copyData(reinterpret_cast<unsigned char*>(outputBuffer), sizeof(defaultColor[0]),
                 &defaultColor[0]);
        break;
    }
    case TINYEXR_PIXELTYPE_FLOAT: {
