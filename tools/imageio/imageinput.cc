...
// and open the file.
std::unique_ptr<ImageInput> ImageInput::open(const std::string& filename,
    ...
    if (filename.compare("-")) {
        ...
        if (ifs.fail()) {
            throw std::runtime_error(
                fmt::format("Open of \"{}\" failed. {}", filename, strerror(errno)));
        }
...
/// - bit scaling
///   - unorm8<->unorm16
void ImageInput::readScanline(void* pBufferOut, size_t bufferByteCount, uint32_t y, uint32_t z,
    ...
    if (reinterpret_cast<uint16_t*>(pNativeBuffer) == nativeBuffer16.data()) {
        rescale(static_cast<uint8_t*>(pBufferOut),
                static_cast<uint8_t>(targetFormat.channelUpper()), nativeBuffer16.data(),
        ...
        rescale(static_cast<uint16_t*>(pBufferOut),
                static_cast<uint16_t>(targetFormat.channelUpper()), nativeBuffer8.data(),
...
void ImageInput::readImage(void* pBuffer, size_t bufferByteCount, uint32_t subimage,
