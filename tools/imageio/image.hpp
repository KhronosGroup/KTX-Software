...
struct RangeMapping {
    RangeMapping() {
        scale = 1.0f;
        offset = 0.0f;
        lower = std::numeric_limits<float>::min();
        upper = std::numeric_limits<float>::max();
    }

    float to(float value) const { return value * scale + offset; }
    float from(float value) const { return (value - offset) / scale; }

    void compute(float oldMin, float oldMax, float newMin, float newMax) {
        ...
        if (!(offsetMin != 0.0f && offsetMax != 0.0f) && (oldMax + offsetMin < newMax) && (oldMin - offsetMax < newMin) ) {
            scale = 1.0f;
            offset = offsetMin != 0.0f ? offsetMin : offsetMax;
        } else {
...
class Image {
  public:
    ...
    virtual RangeMapping mapRangeInfo(VkFormat format, ktx_basis_codec codec) = 0;
...
class ImageT : public Image {
  ...
  public:
    ...
    virtual RangeMapping mapRangeInfo(VkFormat format, ktx_basis_codec codec) override {
        // clang-format off
        const auto vkFormatRange = [](VkFormat targetFormat, float* rangeMin, float* rangeMax) -> bool {
            switch (targetFormat) {
                ...
                { *rangeMin = -65504.0f; *rangeMax = 65504.0f; return true; break; }
                case VK_FORMAT_B10G11R11_UFLOAT_PACK32:
                case VK_FORMAT_E5B9G9R9_UFLOAT_PACK32:
                case VK_FORMAT_BC6H_UFLOAT_BLOCK:
                { *rangeMin = 0.0f; *rangeMax = 65504.0f; return true; break; }
                ...
                { *rangeMin = std::numeric_limits<float>::min(); *rangeMax = std::numeric_limits<float>::max(); return true; break; }
                case VK_FORMAT_R64_SFLOAT:
                case VK_FORMAT_R64G64_SFLOAT:
                case VK_FORMAT_R64G64B64_SFLOAT:
                case VK_FORMAT_R64G64B64A64_SFLOAT:
                { *rangeMin = std::numeric_limits<double>::min(); *rangeMax = std::numeric_limits<double>::max(); return true; break; }
                ...
                default:
                { *rangeMin = 0.0f; *rangeMax = 0.0f; return false; break;};
            }
        };
        // clang-format on
        ...
        if constexpr (std::is_floating_point_v<componentType>) {
            ...
            float targetRangeMin = 0.0f;
            float targetRangeMax = 0.0f;
            if (codec == ktx_basis_codec_e::KTX_BASIS_CODEC_UASTC_HDR_4x4) {
                targetRangeMin = 0.0f;
                targetRangeMax = 65504.0f;
            } else {
                vkFormatRange(format, &targetRangeMin, &targetRangeMax);
            }

            float imageRangeMin = std::numeric_limits<componentType>::max();
            float imageRangeMax = std::numeric_limits<componentType>::min();
            for (uint32_t i = 0; i < pixelCount; ++i) {
                Color& c = pixels[i];
                for (uint32_t comp = 0; comp < compCount; comp++) {
                    imageRangeMin = std::min(imageRangeMin, c[comp]);
                    imageRangeMax = std::max(imageRangeMax, c[comp]);
                }
            }

            RangeMapping mapping;

            mapping.compute(imageRangeMin, imageRangeMax, targetRangeMin, targetRangeMax);
