...
class NpbmInput final : public ImageInput {
  public:
    ...
    //                                   void* data) override;
    virtual void readImage(void* buffer, size_t bufferByteCount, uint32_t subimage, uint32_t miplevel,
                           const FormatDescriptor& targetFormat) override;
    /// Read a single scanline (all channels) of native data into contiguous
    /// memory.
    virtual void readNativeScanline(void* buffer, size_t bufferByteCount, uint32_t y, uint32_t z, uint32_t subimage,
                                    uint32_t miplevel) override;

    virtual uint32_t subimageCount(void) const override { return static_cast<uint32_t>(images.size()); }
    virtual bool seekSubimage(uint32_t subimage, uint32_t miplevel = 0) override;

    virtual const ImageSpec& spec(void) const override { return images[curSubimage].spec; }
...
ImageInput* npbmInputCreate() { return new NpbmInput; }

const char* npbmInputExtensions[] = {"pam", "pbm", "pgm", "ppm", nullptr};

void NpbmInput::open(ImageSpec& newspec) {
...
void NpbmInput::close() {
    isp->exceptions(exceptionsIn);
    ImageInput::close();
}

static int tupleComponentCount(const char* tupleType) {
...
inline void NpbmInput::nextLine() {
    std::getline(*isp, currentLine);
    pos = 0;
}

inline void NpbmInput::nextToken() {
    while (1) {
        while (isspace(currentLine[pos])) pos++;
...
inline void NpbmInput::skipComments(char comment) {
    while (1) {
        nextToken();
...
void NpbmInput::readImageHeaders() {
    for (;;) {
        try {
            // MagicNumber
 ...
 */
static void swapEndian16(uint16_t* pData16, size_t nvals) {
...
static const union foo {
    uint16_t x;
    uint8_t c;
} bar{1};
#define IS_LITTLE_ENDIAN (bar.c)

bool littleendian(void) noexcept { return bar.c; }

void NpbmInput::swap(void* pBuffer, size_t nvals) {
...
void NpbmInput::readNativeScanline(void* bufferOut, size_t bufferByteCount, uint32_t y, uint32_t z, uint32_t subimage,
                                   uint32_t miplevel) {
    if (isp == nullptr) throw std::runtime_error("istream not initialized");
    if (z > 1) throw std::runtime_error("npbm does not support 3d images.");
    if (bufferByteCount < spec().scanlineByteCount()) throw buffer_too_small();

    seekSubimage(subimage, miplevel);

    if (y != curImageScanline) isp->seekg(images[currentSubimage()].filepos + (spec().scanlineByteCount() * y), isp->beg);
    isp->read((char*)bufferOut, spec().scanlineByteCount());
    swap(bufferOut, spec().scanlineByteCount());
}

bool NpbmInput::seekSubimage(uint32_t subimage, uint32_t miplevel) {
    if (subimage == currentSubimage() && miplevel == currentMiplevel()) return true;

    if (subimage >= images.size() || miplevel > 0) return false;
...
#define tupletype_sscanf_fmt(ms) "TUPLTYPE %" #ms "s"
    ...
    for (;;) {
        nextLine();
        skipComments();
        if (currentLine.compare("ENDHDR") == 0) break;
        ...
        if (sscanf(cur_line_cs, "HEIGHT %u", &height))
            ...
            numFieldsFound++;
        else if (sscanf(cur_line_cs, xtupletype_sscanf_fmt(MAX_TUPLETYPE_SIZE), tupleType))
            numFieldsFound++;
    };
    ...
    if ((tCompCount = tupleComponentCount(tupleType)) < 1) {
        throw invalid_file(fmt::format("Invalid TUPLTYPE: {}.", tupleType));
    }

    if (componentCount < tCompCount) {
        throw invalid_file(fmt::format("Mismatched TUPLTYPE, {}, and DEPTH, {}.", tupleType, componentCount));
    }
    ...
    images.emplace_back(ImageSpec(width, height, 1, componentCount, maxVal > 255 ? 16 : 8, 0U, maxVal,
                                  static_cast<khr_df_sample_datatype_qualifiers_e>(0), KHR_DF_TRANSFER_ITU, KHR_DF_PRIMARIES_BT709,
                                  tCompCount < 3 ? KHR_DF_MODEL_YUVSDA : KHR_DF_MODEL_RGBSDA),
...
void NpbmInput::parseGPHeader(filetype ftype) {
    ...
    numvals = sscanf(currentLine.c_str(), "%u %u", &width, &height);
    if (numvals != 2) {
    ...
    images.emplace_back(ImageSpec(width, height, 1, ftype == filetype::PPM ? 3 : 1, 8, 0, maxVal,
                                  static_cast<khr_df_sample_datatype_qualifiers_e>(0), KHR_DF_TRANSFER_ITU, KHR_DF_PRIMARIES_BT709,
                                  ftype == filetype::PPM ? KHR_DF_MODEL_RGBSDA : KHR_DF_MODEL_YUVSDA),
                        ImageInputFormatType::npbm);
}
...
void NpbmInput::readImage(void* pBuffer, size_t bufferByteCount, uint32_t subimage, uint32_t miplevel,
                          const FormatDescriptor& format) {
    const FormatDescriptor* targetFormat;
    if (isp == nullptr) throw std::runtime_error("No open input stream");

    if (bufferByteCount < spec().imageByteCount()) throw buffer_too_small();
    ...
    if (*targetFormat != spec().format()) {
        ...
        ImageInput::readImage(pBuffer, bufferByteCount, subimage, miplevel, format);
        return;
    }
