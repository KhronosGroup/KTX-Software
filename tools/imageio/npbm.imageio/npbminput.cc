...
class NpbmInput final : public ImageInput {
  public:
    ...
    //                                   void* data) override;
    virtual void readImage(void* buffer, size_t bufferByteCount, uint32_t subimage,
                           uint32_t miplevel, const FormatDescriptor& targetFormat) override;
...
void NpbmInput::readNativeScanline(void* bufferOut, size_t bufferByteCount, uint32_t y, uint32_t z,
    ...
    if (y != curImageScanline)
        isp->seekg(images[currentSubimage()].filepos + (spec().scanlineByteCount() * y), isp->beg);
...
#define tupletype_sscanf_fmt(ms) "TUPLTYPE %" #ms "s"
    ...
    if (componentCount < tCompCount) {
        throw invalid_file(
            fmt::format("Mismatched TUPLTYPE, {}, and DEPTH, {}.", tupleType, componentCount));
    }
    ...
    images.emplace_back(ImageSpec(width, height, 1, componentCount, maxVal > 255 ? 16 : 8, 0U,
                                  maxVal, static_cast<khr_df_sample_datatype_qualifiers_e>(0),
                                  KHR_DF_TRANSFER_ITU, KHR_DF_PRIMARIES_BT709,
...
void NpbmInput::parseGPHeader(filetype ftype) {
    ...
    images.emplace_back(
        ImageSpec(width, height, 1, ftype == filetype::PPM ? 3 : 1, 8, 0, maxVal,
                  static_cast<khr_df_sample_datatype_qualifiers_e>(0), KHR_DF_TRANSFER_ITU,
...
void NpbmInput::readImage(void* pBuffer, size_t bufferByteCount, uint32_t subimage,
                          uint32_t miplevel, const FormatDescriptor& format) {
