...
class JpegInput final : public ImageInput {
  public:
    ...
    virtual void readScanline(void* buffer, size_t bufferByteCount, uint y, uint z, uint usubimage,
                              uint miplevel, const FormatDescriptor& targetFormat) override;

    /// Read a single scanline (all channels) of native data into contiguous
    /// memory.
    virtual void readNativeScanline(void* /*buffer*/, size_t /*bufferByteCount*/, uint /*y*/,
                                    uint /*z*/, uint /*subimage*/, uint /*miplevel*/) override{};
...
void JpegInput::readHeader() {
    ...
    images.emplace_back(ImageSpec(pJd->get_width(), pJd->get_height(), 1, pJd->get_num_components(),
                                  8,  // component bit length
                                  static_cast<khr_df_sample_datatype_qualifiers_e>(0),
                                  KHR_DF_TRANSFER_SRGB, KHR_DF_PRIMARIES_BT709),
...
void JpegInput::readScanline(void* bufferOut, size_t bufferByteCount, uint y, uint, uint, uint,
    ...
    if (requestBits != 8)
        throw std::runtime_error(
            fmt::format("Requested decode into {}-bit format is not supported.", requestBits));
    ...
    if (targetE || targetL || targetS || targetF)
        throw std::runtime_error(
            fmt::format("Requested format conversion to {}-bit{}{}{}{} is not supported.",
    ...
    if (targetChannelCount == 2)
        throw std::runtime_error(fmt::format("Requested decode into 2 channels is not supported."));
...
void JpegInput::readImage(void* bufferOut, size_t bufferByteCount, uint subimage, uint miplevel,
                          const FormatDescriptor& format) {
    pJd->begin_decoding();
    decodingBegun = true;
