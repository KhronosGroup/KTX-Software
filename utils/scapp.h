...
ktx_pack_astc_block_dimension_e astcBlockDimension(const char* block_size) {
    static std::unordered_map<std::string, ktx_pack_astc_block_dimension_e> astc_blocks_mapping{
        {"4x4", KTX_PACK_ASTC_BLOCK_DIMENSION_4x4},
        ...
        struct basisOptions : public ktxBasisParams {
            ...
            basisOptions()
                : threadCount(ktxBasisParams::threadCount, 1, 10000),
                  ...
                  uastcRDOMaxSmoothBlockErrorScale(ktxBasisParams::uastcRDOMaxSmoothBlockErrorScale,
                                                   1.0f, 300.0f),
                  uastcRDOMaxSmoothBlockStdDev(ktxBasisParams::uastcRDOMaxSmoothBlockStdDev, 0.01f,
                                               65536.0f) {
...
bool scApp::processOption(argparser& parser, int opt) {
    ...
    case 1018:
        if (options.etc1s) {
            cerr
                << "Only one of `--encode etc1s | --bcmp` and `--uastc [<level>]` can be specified."
                << endl;
            usage();
            exit(1);
        }
...
int scApp::encode(ktxTexture2* texture, const string& swizzle, const string& filename) {
    ...
    if (options.etc1s || options.bopts.uastc) {
        ...
        if (KTX_SUCCESS != result) {
            cerr << name << " failed to compress KTX file \"" << filename
                 << "\" with ASTC; KTX error: " << ktxErrorString(result) << endl;
            return 2;
        }
    } else {
        result = KTX_SUCCESS;
        ...
        if (options.zcmp) {
            ...
            if (KTX_SUCCESS != result) {
                cerr << name << ": Zstd deflation of \"" << filename
                     << "\" failed; KTX error: " << ktxErrorString(result) << endl;
                return 2;
            }
        }
    }
