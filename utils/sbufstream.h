 ...
 * be passed either as a `unique_ptr` or a regular pointer.  As our use is to pass the streambuf
 * from an istream, potentially that from std::cin, or an istringstream that is not being not
 * allocated by `new` we can't use a `unique_ptr`. i.e. can't attempt to delete the stringbuf when
 * done. Also the destructor now destructs the stream, if not already destructed.
 *
 * @author Paolo Jovon
 * @author Mark Callow
 */
...
class StreambufStream {
  ...
  public:
    StreambufStream(T streambuf, std::ios::openmode seek_mode = std::ios::in | std::ios::out)
    ...
    virtual ~StreambufStream() {
        if (!_destructed) stream()->destruct(stream());
    }
