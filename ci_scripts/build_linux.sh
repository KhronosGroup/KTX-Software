...
docker exec -it emscripten sh -c "sudo apt-get install apt-transport-https ca-certificates gnupg software-properties-common wget"
docker exec -it emscripten sh -c "wget -O - https://apt.kitware.com/keys/kitware-archive-latest.asc 2>/dev/null | gpg --dearmor - | sudo tee /etc/apt/trusted.gpg.d/kitware.gpg >/dev/null"
docker exec -it emscripten sh -c "sudo apt-add-repository 'deb https://apt.kitware.com/ubuntu/ bionic main'"
docker exec -it emscripten sh -c "sudo apt-get update"
docker exec -it emscripten sh -c "sudo apt-get install cmake"
