...
SwipeDetector::result SwipeDetector::doEvent(SDL_Event* event) {
    ...
    case SDL_FINGERUP:
        ...
        SDL_Log("Finger: %" SDL_PRIs64 " up - x: %f, y: %f", event->tfinger.fingerId,
...
#if LOG_GESTURE_EVENTS
        SDL_Log("MG: x = %f, y = %f, dAng = %f (%f), dR = %f, numFingers = %i", event->mgesture.x,
                event->mgesture.y, event->mgesture.dTheta * 180.0 / M_PI, event->mgesture.dTheta,
                event->mgesture.dDist, event->mgesture.numFingers);
#endif
#if LOG_GESTURE_DETECTION
        SDL_Log("mgestureSwipe = %i, time = %i", mgestureSwipe,
            ...
            if (!mgestureSwipe) {
                ...
                if (!isinf(velocitySq) && velocitySq > 0.0002) {  // 0.08
                    ...
                    Swipe::Direction direction = Swipe::getDirection(
                        mgestureFirst.x, mgestureFirst.y, event->mgesture.x, event->mgesture.y);
