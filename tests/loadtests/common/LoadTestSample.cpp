...
int LoadTestSample::doEvent(SDL_Event* event) {
    switch (event->type) {
    ...
    case SDL_FINGERDOWN:
        ...
        if (SDL_GetNumTouchFingers(event->tfinger.touchId) > 1) mouseButtons.left = false;
    ...
    case SDL_MULTIGESTURE:
        if (!zooming && !rotating) {
            ...
            if (LOG_GESTURE_DETECTION) {
                ...
                SDL_Log("dTheta = %f°, accumTheta = %f°", event->mgesture.dTheta * 180.0 / M_PI,
                        accumTheta);
            }
        }
        if (zooming) {
        ...
        if (rotating) {
            ...
            if (fabs(accumTheta) > 20) {
                if (LOG_GESTURE_DETECTION) {
                    SDL_Log("rotation detected, accumTheta = %f°, rotation.z = %f°", accumTheta,
                            rotation.z);
                }
                rotating = true;
