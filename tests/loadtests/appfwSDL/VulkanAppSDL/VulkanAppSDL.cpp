...
bool VulkanAppSDL::initialize(Args &args) {
    ...
    pswMainWindow = SDL_CreateWindow(
        szName, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w_width, w_height,
...
#define GUARD 0
    ...
    (void)(prepareDepthBuffer()  // XXX Call it DepthStencil?
           && vkctx.createPresentCommandBuffers() && preparePresentCommandBuffers() &&
           prepareRenderPass() && prepareFramebuffers());
...
void VulkanAppSDL::submitFrame() {
    ...
    VkResult err = vkctx.swapchain.queuePresent(
        vkctx.queue, currentBuffer,
...
bool VulkanAppSDL::initializeVulkan() {
    if (createInstance() && findGpu() && setupDebugReporting() &&
        vkctx.swapchain.connectInstance(vkctx.instance, vkctx.gpu) && createSurface() &&
        createDevice() && vkctx.swapchain.connectDevice(vkctx.device) && createSemaphores() &&
        createSwapchain() && prepareDepthBuffer() && vkctx.createPresentCommandBuffers() &&
        preparePresentCommandBuffers() && prepareRenderPass() && createPipelineCache() &&
        prepareFramebuffers()) {
...
bool VulkanAppSDL::createInstance() {
    ...
    if (validate) {
        ...
        if (instanceLayerCount > 0) {
            ...
            validationFound = checkLayers((uint32_t)instanceValidationLayers_alt1.size(),
                                          instanceValidationLayers_alt1.data(), instanceLayerCount,
                                          instanceLayers);
            if (validationFound) {
                instanceValidationLayers = &instanceValidationLayers_alt1;
            } else {
                // Use alternative set of validation layers.
                validationFound = checkLayers((uint32_t)instanceValidationLayers_alt2.size(),
    ...
    if (!SDL_Vulkan_GetInstanceExtensions(pswMainWindow, &c, nullptr)) {
        ...
        (void)SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, title.c_str(), msg.str().c_str(),
                                       NULL);
        return false;
    }
    ...
    if (cerr != vk::Result::eSuccess) {
        ...
        if (cerr == vk::Result::eErrorIncompatibleDriver) {
            ...
            if (instanceExtensionCount > 0) {
                err = vkEnumerateInstanceExtensionProperties(NULL, &instanceExtensionCount,
                                                             instanceExtensions);
                assert(err == VK_SUCCESS);
            }
...
bool VulkanAppSDL::setupDebugReporting() {
    if (validate) {
        ...
        dbgCreateInfo.flags = VK_DEBUG_REPORT_ERROR_BIT_EXT | VK_DEBUG_REPORT_WARNING_BIT_EXT;
        VkResult err =
            pfnCreateDebugReportCallbackEXT(vkctx.instance, &dbgCreateInfo, NULL, &msgCallback);
        switch (err) {
        case VK_SUCCESS:
            return true;
            break;
...
bool VulkanAppSDL::createDevice() {
    ...
    err = vkctx.gpu.enumerateDeviceExtensionProperties(NULL, &deviceExtensionCount,
    ...
    for (uint32_t i = 0; i < wantedExtensions.size(); i++) {
        ...
        for (j = 0; j < deviceExtensions.size(); j++) {
            if (!wantedExtensions[i].name.compare(
                    static_cast<const char *>(deviceExtensions[j].extensionName))) {
    ...
    vk::DeviceCreateInfo deviceInfo(
        {}, 1, &queueInfo, (uint32_t)deviceValidationLayers.size(),
    ...
    const vk::CommandPoolCreateInfo cmdPoolInfo(vk::CommandPoolCreateFlagBits::eResetCommandBuffer,
...
bool VulkanAppSDL::createSemaphores() {
    ...
    VK_CHECK_RESULT(vkCreateSemaphore(vkctx.device, &semaphoreCreateInfo, nullptr,
    ...
    VK_CHECK_RESULT(
        vkCreateSemaphore(vkctx.device, &semaphoreCreateInfo, nullptr, &semaphores.renderComplete));
...
bool VulkanAppSDL::prepareDepthBuffer() {
    ...
    if (!getSupportedDepthFormat(vkctx.gpu, eNoStencil, e24bits, vk::ImageTiling::eOptimal,
    ...
    setImageLayout(vkctx.depthBuffer.image, static_cast<VkImageAspectFlags>(aspectMask),
...
bool VulkanAppSDL::preparePresentCommandBuffers() {
    ...
    for (uint32_t i = 0; i < vkctx.swapchain.imageCount; i++) {
        ...
        vkCmdPipelineBarrier(vkctx.postPresentCmdBuffers[i], VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
        ...
        vkCmdPipelineBarrier(vkctx.prePresentCmdBuffers[i], VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
...
bool VulkanAppSDL::createPipelineCache() {
    ...
    VK_CHECK_RESULT(vkCreatePipelineCache(vkctx.device, &pipelineCacheCreateInfo, nullptr,
                                          &vkctx.pipelineCache));
    return true;
}
...
void VulkanAppSDL::setWindowTitle() {
    if (!enableTextOverlay) {
        ...
        ss << std::fixed << std::setprecision(2) << lastFrameTime << "ms (" << fpsCounter.lastFPS
           << " fps)"
           << " ";
        wt = ss.str();
        wt += appTitle;
...
void VulkanAppSDL::prepareTextOverlay() {
    if (enableTextOverlay) {
        ...
        shaderStage = static_cast<VkPipelineShaderStageCreateInfo>(
            vkctx.loadShader(filepath, vk::ShaderStageFlagBits::eVertex));
        shaderStages.push_back(shaderStage);
        shaderModules.push_back(shaderStage.module);
        filepath = getAssetPath() + "textoverlay.frag.spv";
        shaderStage = static_cast<VkPipelineShaderStageCreateInfo>(
            vkctx.loadShader(filepath, vk::ShaderStageFlagBits::eFragment));
        shaderStages.push_back(shaderStage);
        shaderModules.push_back(shaderStage.module);

        textOverlay = new VulkanTextOverlay(
            vkctx.gpu, vkctx.device, vkctx.queue, vkctx.framebuffers, vkctx.swapchain.colorFormat,
...
// not required.
bool VulkanAppSDL::getSupportedDepthFormat(vk::PhysicalDevice gpu,
                                           stencilRequirement requiredStencil,
                                           depthRequirement requiredDepth, vk::ImageTiling tiling,
                                           vk::Format &depthFormat,
                                           vk::ImageAspectFlags &aspectMask) {
...
void VulkanAppSDL::setImageLayout(VkImage image, VkImageAspectFlags aspectMask,
    ...
    vkCmdPipelineBarrier(setupCmdBuffer, src_stages, dest_stages, 0, 0, NULL, 0, NULL, 1,
                         &imageMemoryBarrier);
}  // setImageLayout
 ...
 */
bool VulkanAppSDL::checkLayers(uint32_t nameCount, const char **names, uint32_t layerCount,
...
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanAppSDL::debugFunc(VkDebugReportFlagsEXT msgFlags,
...
#endif
    message << prefix << " [" << pLayerPrefix << "] Code " << std::showbase << std::internal
            << std::setfill('0') << std::hex << std::setw(8) << msgCode << ": " << std::endl
            << text;
...
std::string &VulkanAppSDL::wrapText(std::string &source, size_t width,
...
uint32_t VulkanAppSDL::showDebugReport(uint32_t mbFlags, const std::string title,
