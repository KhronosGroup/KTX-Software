...
#define ERROR_RETURN(msg)                                                            \
    (void)SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, theApp->name(), msg, NULL); \
    return false;
...
bool VulkanSwapchain::initSurface(SDL_Window *window) {
    ...
    err = vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface, &formatCount,
                                               surfaceFormats.data());
    assert(err == VK_SUCCESS);
...
bool VulkanSwapchain::connectDevice(VkDevice targetDevice) {
...
// Connect to the instance and device and get all required function pointers
bool VulkanSwapchain::connectInstance(VkInstance targetInstance,
...
void VulkanSwapchain::create(uint32_t *width, uint32_t *height, bool vsync) {
    ...
    err = vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &presentModeCount,
                                                    presentModes.data());
    assert(err == VK_SUCCESS);
...
VkResult VulkanSwapchain::acquireNextImage(VkSemaphore presentCompleteSemaphore,
    ...
    return vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, presentCompleteSemaphore,
