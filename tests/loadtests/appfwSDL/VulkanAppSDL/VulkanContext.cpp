...
void VulkanContext::destroyDrawCommandBuffers() {
    if (drawCmdBuffers.size() > 0) {
        vkFreeCommandBuffers(device, commandPool, static_cast<uint32_t>(drawCmdBuffers.size()),
                             drawCmdBuffers.data());
    }
...
bool VulkanContext::createBuffer(vk::BufferUsageFlags usageFlags,
                                 vk::MemoryPropertyFlags memoryPropertyFlags, vk::DeviceSize size,
...
bool VulkanContext::createBuffer(vk::BufferUsageFlags usage, vk::DeviceSize size, void* data,
                                 vk::Buffer* buffer, vk::DeviceMemory* memory) {
    return createBuffer(usage, vk::MemoryPropertyFlagBits::eHostVisible, size, data, buffer,
                        memory);
}
...
bool VulkanContext::createBuffer(vk::BufferUsageFlags usage,
                                 vk::MemoryPropertyFlags memoryPropertyFlags, vk::DeviceSize size,
                                 void* data, vk::Buffer* buffer, vk::DeviceMemory* memory,
                                 vk::DescriptorBufferInfo* descriptor) {
...
bool VulkanContext::getMemoryType(uint32_t typeBits, vk::MemoryPropertyFlags requirementsMask,
...
uint32_t VulkanContext::getMemoryType(uint32_t typeBits,
...
vk::PipelineShaderStageCreateInfo VulkanContext::loadShader(std::string filename,
