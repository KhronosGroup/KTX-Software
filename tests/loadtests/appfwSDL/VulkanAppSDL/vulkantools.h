...
#define VK_CHECK_RESULT(f)                                                                                                       \
    {                                                                                                                            \
        ...
        if (res != VK_SUCCESS) {                                                                                                 \
            ...
            msg << "Fatal error. VkResult is \"" << vkTools::errorString(res) << "\" in " << __FILE__ << " at line " << __LINE__ \
                << std::endl;                                                                                                    \
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, appName(), msg.str().c_str(), NULL);                                  \
            assert(res == VK_SUCCESS);                                                                                           \
        }                                                                                                                        \
    }
#else
...
VkDescriptorSetAllocateInfo descriptorSetAllocateInfo(VkDescriptorPool descriptorPool, const VkDescriptorSetLayout* pSetLayouts,
...
VkVertexInputAttributeDescription vertexInputAttributeDescription(uint32_t binding, uint32_t location, VkFormat format,
                                                                  uint32_t offset);
...
VkPipelineRasterizationStateCreateInfo pipelineRasterizationStateCreateInfo(VkPolygonMode polygonMode, VkCullModeFlags cullMode,
...
VkPipelineDepthStencilStateCreateInfo pipelineDepthStencilStateCreateInfo(VkBool32 depthTestEnable, VkBool32 depthWriteEnable,
