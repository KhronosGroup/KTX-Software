...
// Macro to check and display Vulkan return results
    #define VK_CHECK_RESULT(f)                                                                 \
        {                                                                                      \
            VkResult res = (f);                                                                \
            if (res != VK_SUCCESS) {                                                           \
                std::cout << "Fatal : VkResult is \"" << vkTools::errorString(res) << "\" in " \
    ...
    #define VK_CHECK_RESULT(f)                                                                \
        {                                                                                     \
            ...
            if (res != VK_SUCCESS) {                                                          \
                ...
                msg << "Fatal error. VkResult is \"" << vkTools::errorString(res) << "\" in " \
                    << __FILE__ << " at line " << __LINE__ << std::endl;                      \
                SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, appName(), msg.str().c_str(),  \
                                         NULL);                                               \
                assert(res == VK_SUCCESS);                                                    \
            }                                                                                 \
        }
...
VkDescriptorPoolCreateInfo descriptorPoolCreateInfo(uint32_t poolSizeCount,
...
VkWriteDescriptorSet writeDescriptorSet(VkDescriptorSet dstSet, VkDescriptorType type,
...
VkVertexInputAttributeDescription vertexInputAttributeDescription(uint32_t binding,
                                                                  uint32_t location,
...
VkGraphicsPipelineCreateInfo pipelineCreateInfo(VkPipelineLayout layout, VkRenderPass renderPass,
                                                VkPipelineCreateFlags flags);

VkComputePipelineCreateInfo computePipelineCreateInfo(VkPipelineLayout layout,
                                                      VkPipelineCreateFlags flags);

VkPushConstantRange pushConstantRange(VkShaderStageFlags stageFlags, uint32_t size,
                                      uint32_t offset);
}  // namespace initializers
