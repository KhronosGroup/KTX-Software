...
//                                   );
#define setAnimationCallback(win, cb, userdata) SDL_iPhoneSetAnimationCallback(win, 1, cb, userdata)
#elif defined(EMSCRIPTEN)
#define NEED_MAIN_LOOP 0
// void emscripten_set_main_loop_arg(em_arg_callback_func func, void *arg,
//                                   int fps, int simulate_infinite_loop);
#define setAnimationCallback(win, cb, userdata) emscripten_set_main_loop_arg(cb, userdata, 0, 0)
#else
...
int main(int argc, char* argv[]) {
    ...
    if (!theApp->initialize(args)) return 1;
