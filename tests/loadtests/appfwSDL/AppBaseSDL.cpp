...
void AppBaseSDL::initializeFPSTimer() {
    lastFrameTime = 0;
...
void AppBaseSDL::finalize() {}

int AppBaseSDL::doEvent(SDL_Event* event) {
    switch (event->type) {
    case SDL_QUIT:
        finalize();
        exit(0);
...
void AppBaseSDL::onFPSUpdate() {}

//  Protected, non-virtual
void AppBaseSDL::drawFrame() {
    ...
    if (endTicks - fpsCounter.startTicks > tps) {
        fpsCounter.lastFPS =
            (float)(fpsCounter.numFrames * tps) / (endTicks - fpsCounter.startTicks);
...
void AppBaseSDL::setAppTitle(const char* const szExtra) {
    appTitle = name();
...
void AppBaseSDL::setWindowTitle() {
    std::stringstream ss;
    std::string wt;
...
const char* appName() { return theApp->name(); }
