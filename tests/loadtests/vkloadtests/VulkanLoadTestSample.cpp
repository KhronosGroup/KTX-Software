...
void VulkanLoadTestSample::loadMesh(std::string filename, vkMeshLoader::MeshBuffer* meshBuffer,
                                    std::vector<vkMeshLoader::VertexLayout> vertexLayout, float scale) {
    ...
    mesh->createBuffers(vkctx.device, static_cast<VkPhysicalDeviceMemoryProperties>(vkctx.memoryProperties), meshBuffer,
                        vertexLayout, scale, true, copyCmd, vkctx.queue);
...
vk::PipelineShaderStageCreateInfo VulkanLoadTestSample::loadShader(std::string filename, vk::ShaderStageFlagBits stage,
                                                                   const char* modname) {
