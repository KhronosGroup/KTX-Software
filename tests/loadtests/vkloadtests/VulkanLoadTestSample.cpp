...
void DestroyVMA() { vmaDestroyAllocator(vmaAllocator); }
...
uint64_t AllocMemCWrapper(VkMemoryAllocateInfo* allocInfo, VkMemoryRequirements* memReq,
                          uint64_t* numPages) {
    uint64_t allocId = mt64();
    VmaAllocationCreateInfo pCreateInfo = {};
    if ((cachedDevMemProps.memoryTypes[allocInfo->memoryTypeIndex].propertyFlags &
         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) ||
        (cachedDevMemProps.memoryTypes[allocInfo->memoryTypeIndex].propertyFlags &
...
void VulkanLoadTestSample::loadMesh(std::string filename, vkMeshLoader::MeshBuffer* meshBuffer,
    ...
    vk::CommandBuffer copyCmd = vkctx.createCommandBuffer(vk::CommandBufferLevel::ePrimary, false);

    mesh->createBuffers(vkctx.device,
                        static_cast<VkPhysicalDeviceMemoryProperties>(vkctx.memoryProperties),
                        meshBuffer, vertexLayout, scale, true, copyCmd, vkctx.queue);
...
vk::PipelineShaderStageCreateInfo VulkanLoadTestSample::loadShader(std::string filename,
