...
VulkanLoadTestSample* TextureArray::create(VulkanContext& vkctx, uint32_t width, uint32_t height,
...
TextureArray::TextureArray(VulkanContext& vkctx, uint32_t width, uint32_t height,
                           const char* const szArgs, const std::string sBasePath)
    ...
    try {
        prepare("instancing.frag.spv", "instancing.vert.spv", INSTANCE_COUNT_CONST_ID,
                texture.layerCount, INSTANCES_DECLARED_IN_SHADER);
