  ...
  public:
    ...
    void setupVertexInputState(std::vector<vkMeshLoader::VertexLayout> layout) {
        bindingDescription = vkTools::initializers::vertexInputBindingDescription(
            vertexBufferBinding, vertexSize(layout), VK_VERTEX_INPUT_RATE_VERTEX);
        ...
        for (auto& layoutDetail : layout) {
            // Format (layout)
            VkFormat format = (layoutDetail == VERTEX_LAYOUT_UV) ? VK_FORMAT_R32G32_SFLOAT
                                                                 : VK_FORMAT_R32G32B32_SFLOAT;

            attributeDescriptions.push_back(vkTools::initializers::vertexInputAttributeDescription(
                vertexBufferBinding, binding, format, offset));

            // Offset
            offset +=
                (layoutDetail == VERTEX_LAYOUT_UV) ? (2 * sizeof(float)) : (3 * sizeof(float));
            binding++;
        }
  ...
  private:
    struct Vertex {
        ...
        Vertex(const glm::vec3& pos, const glm::vec2& tex, const glm::vec3& normal,
               const glm::vec3& tangent, const glm::vec3& bitangent, const glm::vec3& color) {
            m_pos = pos;
            m_tex = tex;
            m_normal = normal;
            m_color = color;
    ...
    VkBool32 getMemoryType(VkPhysicalDeviceMemoryProperties deviceMemoryProperties,
  ...
  public:
    ...
    void InitMesh(unsigned int index, const aiMesh* paiMesh, const aiScene* pSrcScene) {
        ...
        for (unsigned int i = 0; i < paiMesh->mNumVertices; i++) {
            ...
            aiVector3D* pTangent =
                (paiMesh->HasTangentsAndBitangents()) ? &(paiMesh->mTangents[i]) : &Zero3D;
            aiVector3D* pBiTangent =
                (paiMesh->HasTangentsAndBitangents()) ? &(paiMesh->mBitangents[i]) : &Zero3D;
    ...
    VkResult createBuffer(VkDevice device, VkPhysicalDeviceMemoryProperties deviceMemoryProperties,
        ...
        memAllocInfo.memoryTypeIndex =
            getMemoryType(deviceMemoryProperties, memReqs.memoryTypeBits, memoryPropertyFlags);
    ...
    void createBuffers(VkDevice device, VkPhysicalDeviceMemoryProperties deviceMemoryProperties,
                       ...
                       std::vector<vkMeshLoader::VertexLayout> layout, float scale, bool useStaging,
                       VkCommandBuffer copyCmd, VkQueue copyQueue) {
        ...
        if (useStaging && copyQueue != VK_NULL_HANDLE && copyCmd != VK_NULL_HANDLE) {
            ...
            // Vertex buffer
            createBuffer(device, deviceMemoryProperties, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, meshBuffer->vertices.size,
            ...
            // Index buffer
            createBuffer(device, deviceMemoryProperties, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, meshBuffer->indices.size,
            ...
            // Vertex buffer
            createBuffer(device, deviceMemoryProperties,
                         VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                         VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, meshBuffer->vertices.size,
                         &meshBuffer->vertices.buf, &meshBuffer->vertices.mem);

            // Index buffer
            createBuffer(device, deviceMemoryProperties,
                         VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                         VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, meshBuffer->indices.size,
                         &meshBuffer->indices.buf, &meshBuffer->indices.mem);
            ...
            vkCmdCopyBuffer(copyCmd, vertexStaging.buffer, meshBuffer->vertices.buf, 1,
                            &copyRegion);
            ...
            VK_CHECK_RESULT(vkMapMemory(device, meshBuffer->vertices.mem, 0,
                                        meshBuffer->vertices.size, 0, &data));
            memcpy(data, vertexBuffer.data(), meshBuffer->vertices.size);
            ...
            VK_CHECK_RESULT(vkMapMemory(device, meshBuffer->indices.mem, 0,
                                        meshBuffer->indices.size, 0, &data));
            memcpy(data, indexBuffer.data(), meshBuffer->indices.size);
    ...
    // Create vertex and index buffer with given layout
    void createVulkanBuffers(VkDevice device,
                             VkPhysicalDeviceMemoryProperties deviceMemoryProperties,
                             vkMeshLoader::MeshBuffer* meshBuffer,
                             std::vector<vkMeshLoader::VertexLayout> layout, float scale) {
        createBuffers(device, deviceMemoryProperties, meshBuffer, layout, scale, false,
