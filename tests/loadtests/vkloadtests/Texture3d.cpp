...
VulkanLoadTestSample* Texture3d::create(VulkanContext& vkctx, uint32_t width, uint32_t height,
...
Texture3d::Texture3d(VulkanContext& vkctx, uint32_t width, uint32_t height,
                     const char* const szArgs, const std::string sBasePath)
    ...
    try {
        prepare("instancing3d.frag.spv", "instancing3d.vert.spv", INSTANCE_COUNT_CONST_ID,
                texture.depth, INSTANCES_DECLARED_IN_SHADER);
...
void Texture3d::setSubclassPushConstants(uint32_t bufferIndex) {
    vkCmdPushConstants(vkctx.drawCmdBuffers[bufferIndex], pipelineLayout,
                       VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(instanceCount), &instanceCount);
}
