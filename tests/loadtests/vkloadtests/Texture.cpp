...
VulkanLoadTestSample* Texture::create(VulkanContext& vkctx, uint32_t width, uint32_t height,
                                      const char* const szArgs, const std::string sBasePath) {
    return new Texture(vkctx, width, height, szArgs, sBasePath);
}

Texture::Texture(VulkanContext& vkctx, uint32_t width, uint32_t height, const char* const szArgs,
    ...
    quadColor = {upperLeftColor, lowerLeftColor, upperRightColor, lowerRightColor};
    ...
    vk::FormatFeatureFlags wantedFeatures = vk::FormatFeatureFlagBits::eSampledImage |
    ...
    if (useSubAlloc == UseSuballocator::Yes) {
        ...
        ktxresult = ktxTexture_VkUploadEx_WithSuballocator(
            kTexture, &vdi, &texture, static_cast<VkImageTiling>(tiling),
    ...
    if (kTexture->orientation.y == KTX_ORIENT_Y_UP) sign_t = -1;

    ktx_uint32_t swizzleLen;
    char* swizzleStr;
    ktxresult = ktxHashList_FindValue(&kTexture->kvDataHead, KTX_SWIZZLE_KEY, &swizzleLen,
                                      (void**)&swizzleStr);
...
void Texture::run(uint32_t /*msTicks*/) {
...
void Texture::processArgs(std::string sArgs) {
    // Options descriptor
    ...
    while ((ch = ap.getopt(nullptr, longopts, nullptr)) != -1) {
        ...
        case 1: {
            ...
            if (i == 1) {
                for (; i < 4; i++) quadColor[i] = quadColor[0];
            }
            break;
        }
        default:
...
void Texture::cleanup() {
    ...
    if (useSubAlloc == UseSuballocator::Yes) {
        ...
        (void)ktxVulkanTexture_Destruct_WithSuballocator(&texture, vkDev, VK_NULL_HANDLE,
                                                         &subAllocatorCallbacks);
...
void Texture::buildCommandBuffers() {
...
#define NORMAL {0.0f, 0.0f, 1.0f}
    std::vector<Vertex> vertexBuffer = {{{-DIM, -DIM, 0.0f}, {0.0f, 0.0f}, NORMAL, {quadColor[0]}},
...
#undef NORMAL
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eVertexBuffer, vertexBuffer.size() * sizeof(Vertex),
                       vertexBuffer.data(), &quad.vertices.buf, &quad.vertices.mem);
...
void Texture::setupVertexDescriptions() {
    ...
    vertices.attributeDescriptions[2] = vk::VertexInputAttributeDescription(
        2, VERTEX_BUFFER_BIND_ID, vk::Format::eR32G32B32Sfloat, sizeof(float) * 5);
    // Location 3 : Color
    vertices.attributeDescriptions[3] = vk::VertexInputAttributeDescription(
...
void Texture::preparePipelines() {
    ...
    shaderStages[0] = loadShader(filepath + "texture.vert.spv", vk::ShaderStageFlagBits::eVertex);
    ...
    fragShader += ".frag.spv";
    shaderStages[1] = loadShader(fragShader, vk::ShaderStageFlagBits::eFragment);
...
void Texture::prepareUniformBuffers() {
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eUniformBuffer, sizeof(uboVS), &uboVS,
                       &uniformDataVS.buffer, &uniformDataVS.memory, &uniformDataVS.descriptor);
...
void Texture::updateUniformBuffers() {
    ...
    uint8_t* pData;
    VK_CHECK_RESULT(
        vkMapMemory(vkctx.device, uniformDataVS.memory, 0, sizeof(uboVS), 0, (void**)&pData));
...
void Texture::changeLodBias(float delta) {
    uboVS.lodBias += delta;
...
void Texture::keyPressed(uint32_t keyCode) {
    switch (keyCode) {
    case SDLK_KP_PLUS:
        changeLodBias(0.1f);
        break;
...
void Texture::getOverlayText(VulkanTextOverlay* textOverlay, float yOffset) {
    std::stringstream ss;
    ss << std::setprecision(2) << std::fixed << uboVS.lodBias;
    textOverlay->addText("LOD bias: " + ss.str() + " (numpad +/- to change)", 5.0f, yOffset,
                         VulkanTextOverlay::alignLeft);
}

const char* Texture::customizeTitle(const char* const baseTitle) {
    if (transcoded) {
