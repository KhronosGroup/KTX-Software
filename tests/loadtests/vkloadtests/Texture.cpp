...
VulkanLoadTestSample* Texture::create(VulkanContext& vkctx, uint32_t width, uint32_t height, const char* const szArgs,
                                      const std::string sBasePath) {
    return new Texture(vkctx, width, height, szArgs, sBasePath);
}

Texture::Texture(VulkanContext& vkctx, uint32_t width, uint32_t height, const char* const szArgs, const std::string sBasePath)
    ...
    if (useSubAlloc == UseSuballocator::Yes) {
        ...
        ktxresult = ktxTexture_VkUploadEx_WithSuballocator(kTexture, &vdi, &texture, static_cast<VkImageTiling>(tiling),
    ...
    char* swizzleStr;
    ktxresult = ktxHashList_FindValue(&kTexture->kvDataHead, KTX_SWIZZLE_KEY, &swizzleLen, (void**)&swizzleStr);
...
#undef NORMAL
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eVertexBuffer, vertexBuffer.size() * sizeof(Vertex), vertexBuffer.data(),
...
void Texture::preparePipelines() {
    vk::PipelineInputAssemblyStateCreateInfo inputAssemblyState({}, vk::PrimitiveTopology::eTriangleStrip);
...
void Texture::prepareUniformBuffers() {
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eUniformBuffer, sizeof(uboVS), &uboVS, &uniformDataVS.buffer, &uniformDataVS.memory,
...
void Texture::getOverlayText(VulkanTextOverlay* textOverlay, float yOffset) {
    ...
    textOverlay->addText("LOD bias: " + ss.str() + " (numpad +/- to change)", 5.0f, yOffset, VulkanTextOverlay::alignLeft);
}
