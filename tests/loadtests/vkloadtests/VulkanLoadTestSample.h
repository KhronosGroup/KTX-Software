...
class VulkanLoadTestSample : public LoadTestSample {
  public:
    ...
    typedef VulkanLoadTestSample* (*PFN_create)(VulkanContext&, uint32_t width, uint32_t height,
                                                const char* const szArgs,
                                                const std::string sBasePath);

  protected:
    ktxVulkanTexture_subAllocatorCallbacks subAllocatorCallbacks = {
        VMA_CALLBACKS::AllocMemCWrapper,        VMA_CALLBACKS::BindBufferMemoryCWrapper,
        VMA_CALLBACKS::BindImageMemoryCWrapper, VMA_CALLBACKS::MapMemoryCWrapper,
        VMA_CALLBACKS::UnmapMemoryCWrapper,     VMA_CALLBACKS::FreeMemCWrapper};
    ...
    void loadMesh(std::string filename, vkMeshLoader::MeshBuffer* meshBuffer,
