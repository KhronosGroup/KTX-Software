...
VulkanLoadTests::VulkanLoadTests(const sampleInvocation samples[], const uint32_t numSamples,
...
void VulkanLoadTests::getOverlayText(float yOffset) {
    if (enableTextOverlay) {
        ...
        yOffset += 20;
        textOverlay->addText("2-finger rotate or left mouse + drag to rotate.", 5.0f, yOffset,
...
void VulkanLoadTests::invokeSample(Direction dir) {
    ...
    for (;;) {
        try {
            if (infiles.size() > 0) {
                ...
                pCurSample =
                    sampleInv->createSample(vkctx, w_width, w_height, sampleInv->args, sBasePath);
            }
            break;
            ...
            if (unsupportedTypeExceptions == sampleIndex.getNumSamples()) {
                ...
                const std::string& message =
                    unsupportedTypeExceptions == 1 ? e.what() : manyFailMessage;
    ...
    prepared = true;
    setAppTitle(
        pCurSample->customizeTitle(infiles.size() > 0 ? fileTitle.c_str() : sampleInv->title));
}
...
VulkanLoadTestSample* VulkanLoadTests::showFile(std::string& filename) {
    ...
    ktxTexture* kTexture;
    ktxresult =
        ktxTexture_CreateFromNamedFile(filename.c_str(), KTX_TEXTURE_CREATE_NO_FLAGS, &kTexture);
...
const VulkanLoadTests::sampleInvocation siSamples[] = {
    {Texture::create, "etc1s_Iron_Bars_001_normal.ktx2",
     "Transcode of ETC1S+BasisLZ Compressed KTX2 XY normal map mipmapped"},
    {Texture::create, "uastc_Iron_Bars_001_normal.ktx2",
    ...
    {Texture::create, "ktx_document_uastc_rdo4_zstd5.ktx2",
    ...
    {Texture::create, "--qcolor 0.0,0.0,0.0 pattern_02_bc2.ktx2",
    ...
    {TextureCubemap::create, "cubemap_goldengate_uastc_rdo4_zstd5_rd.ktx2",
    ...
    {TextureCubemap::create, "cubemap_yokohama_basis_rd.ktx2",
    ...
    {TextureCubemap::create, "skybox_zstd.ktx2",
     "Zstd Compressed B10G11R11_UFLOAT Cube Map. Tests for correct blockSizeInBits after "
     "inflation"},
    ...
    {Texture::create, "--linear-tiling orient-up-metadata.ktx",
