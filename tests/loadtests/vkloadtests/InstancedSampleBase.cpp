...
InstancedSampleBase::InstancedSampleBase(VulkanContext& vkctx, uint32_t width, uint32_t height,
    ...
    ktxVulkanDeviceInfo_Construct(&vdi, vkctx.gpu, vkctx.device, vkctx.queue, vkctx.commandPool,
                                  nullptr);
    ...
    ktxresult =
        ktxTexture_CreateFromNamedFile(ktxfilepath.c_str(), KTX_TEXTURE_CREATE_NO_FLAGS, &kTexture);
    ...
    vk::Result res = vkctx.gpu.getImageFormatProperties(vkFormat, imageType, tiling, usageFlags,
    ...
    if (kTexture->generateMipmaps) {
        uint32_t max_dim =
            std::max(std::max(kTexture->baseWidth, kTexture->baseHeight), kTexture->baseDepth);
    ...
    if (kTexture->isArray && kTexture->numLevels > imageFormatProperties.maxArrayLayers) {
        ktxTexture_Destroy(kTexture);
        throw unsupported_ttype();
    }
    ...
    vk::FormatFeatureFlags neededFeatures = vk::FormatFeatureFlagBits::eSampledImage;
    if (kTexture->numLevels > 1) {
        neededFeatures |= vk::FormatFeatureFlagBits::eSampledImageFilterLinear;
    }
    if (kTexture->generateMipmaps) {
    ...
    ktxresult =
        ktxTexture_VkUploadEx(kTexture, &vdi, &texture, static_cast<VkImageTiling>(tiling),
...
InstancedSampleBase::~InstancedSampleBase() { cleanup(); }

void InstancedSampleBase::resize(uint32_t width, uint32_t height) {
    this->w_width = width;
...
void InstancedSampleBase::run(uint32_t /*msTicks*/) {
...
void InstancedSampleBase::processArgs(std::string sArgs) {
    // Options descriptor
    struct argparser::option longopts[] = {
        {"external", argparser::option::no_argument, &externalFile, 1},
        {"linear-tiling", argparser::option::no_argument, (int*)&tiling,
         (int)vk::ImageTiling::eLinear},
...
void InstancedSampleBase::cleanup() {
    ...
    if (pipelines.solid) vkctx.device.destroyPipeline(pipelines.solid);
...
void InstancedSampleBase::buildCommandBuffers() {
    ...
    vk::RenderPassBeginInfo renderPassBeginInfo(vkctx.renderPass, nullptr,
                                                {{0, 0}, {w_width, w_height}}, 2, clearValues);
    ...
    for (uint32_t i = 0; i < vkctx.drawCmdBuffers.size(); ++i) {
        ...
        VK_CHECK_RESULT(vkBeginCommandBuffer(
            vkctx.drawCmdBuffers[i], &static_cast<const VkCommandBufferBeginInfo&>(cmdBufInfo)));
        ...
        vkCmdBindPipeline(vkctx.drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipelines.solid);

        vkCmdDrawIndexed(vkctx.drawCmdBuffers[i], quad.indexCount, instanceCount, 0, 0, 0);
...
void InstancedSampleBase::generateQuad() {
#define dim 2.5f
    std::vector<TAVertex> vertexBuffer = {{{dim, dim, 0.0f}, {1.0f, 1.0f}},
                                          {{-dim, dim, 0.0f}, {0.0f, 1.0f}},
                                          {{-dim, -dim, 0.0f}, {0.0f, 0.0f}},
                                          {{dim, -dim, 0.0f}, {1.0f, 0.0f}}};
#undef dim

    vkctx.createBuffer(vk::BufferUsageFlagBits::eVertexBuffer,
                       vertexBuffer.size() * sizeof(TAVertex), vertexBuffer.data(),
                       &quad.vertices.buf, &quad.vertices.mem);

    // Setup indices
    std::vector<uint32_t> indexBuffer = {0, 1, 2, 2, 3, 0};
    quad.indexCount = static_cast<uint32_t>(indexBuffer.size());

    vkctx.createBuffer(vk::BufferUsageFlagBits::eIndexBuffer, indexBuffer.size() * sizeof(uint32_t),
                       indexBuffer.data(), &quad.indices.buf, &quad.indices.mem);
}

void InstancedSampleBase::setupVertexDescriptions() {
    // Binding description
    vertices.bindingDescriptions.resize(1);
    vertices.bindingDescriptions[0] = vk::VertexInputBindingDescription(
        VERTEX_BUFFER_BIND_ID, sizeof(TAVertex), vk::VertexInputRate::eVertex);

    // Attribute descriptions
    // Describes memory layout and shader positions
    vertices.attributeDescriptions.resize(2);
    // Location 0 : Position
    vertices.attributeDescriptions[0] = vk::VertexInputAttributeDescription(
        0, VERTEX_BUFFER_BIND_ID, vk::Format::eR32G32B32Sfloat, 0);
    // Location 1 : Texture coordinates
    vertices.attributeDescriptions[1] = vk::VertexInputAttributeDescription(
...
void InstancedSampleBase::setupDescriptorPool() {
    // Example uses one ubo and one image sampler
    std::vector<vk::DescriptorPoolSize> poolSizes = {
        {vk::DescriptorType::eUniformBuffer, 1}, {vk::DescriptorType::eCombinedImageSampler, 1}};

    vk::DescriptorPoolCreateInfo descriptorPoolInfo({}, 2, static_cast<uint32_t>(poolSizes.size()),
                                                    poolSizes.data());
    vk::Result res =
        vkctx.device.createDescriptorPool(&descriptorPoolInfo, nullptr, &descriptorPool);
    if (res != vk::Result::eSuccess) {
        throw bad_vulkan_alloc((int)res, "createDescriptorPool");
    }
}

void InstancedSampleBase::setupDescriptorSetLayout() {
    ...
    vk::DescriptorSetLayoutCreateInfo descriptorLayout(
        {}, static_cast<uint32_t>(descriptorBindings.size()), descriptorBindings.data());

    vk::Result res =
        vkctx.device.createDescriptorSetLayout(&descriptorLayout, nullptr, &descriptorSetLayout);
    if (res != vk::Result::eSuccess) {
...
void InstancedSampleBase::setupDescriptorSet() {
    vk::DescriptorSetAllocateInfo allocInfo(descriptorPool, 1, &descriptorSetLayout);

    vk::Result res = vkctx.device.allocateDescriptorSets(&allocInfo, &descriptorSet);
    if (res != vk::Result::eSuccess) {
        throw bad_vulkan_alloc((int)res, "allocateDescriptorSets");
    }

    // Image descriptor for the color map texture
    vk::DescriptorImageInfo texDescriptor(sampler, imageView,
                                          vk::ImageLayout::eShaderReadOnlyOptimal);

    std::vector<vk::WriteDescriptorSet> writeDescriptorSets;
    // Binding 0 : Vertex shader uniform buffer
    writeDescriptorSets.push_back(vk::WriteDescriptorSet(descriptorSet, 0, 0, 1,
                                                         vk::DescriptorType::eUniformBuffer,
                                                         nullptr, &uniformDataVS.descriptor));
    // Binding 1 : Fragment shader texture sampler
    writeDescriptorSets.push_back(vk::WriteDescriptorSet(
        descriptorSet, 1, 0, 1, vk::DescriptorType::eCombinedImageSampler, &texDescriptor));

    vkctx.device.updateDescriptorSets(static_cast<uint32_t>(writeDescriptorSets.size()),
                                      writeDescriptorSets.data(), 0, nullptr);
}

void InstancedSampleBase::preparePipelines(const char* const fragShaderName,
    ...
    vk::PipelineDynamicStateCreateInfo dynamicState(
        {}, static_cast<uint32_t>(dynamicStateEnables.size()), dynamicStateEnables.data());
    ...
    shaderStages[0] = loadShader(filepath + vertShaderName, vk::ShaderStageFlagBits::eVertex);
    shaderStages[1] = loadShader(filepath + fragShaderName, vk::ShaderStageFlagBits::eFragment);
    ...
    vk::Result res = vkctx.device.createGraphicsPipelines(
        vkctx.pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.solid);
    if (res != vk::Result::eSuccess) {
...
void InstancedSampleBase::prepareUniformBuffers(uint32_t shaderDeclaredInstances) {
    ...
    vkctx.createBuffer(
        ...
        uboSize, nullptr, &uniformDataVS.buffer, &uniformDataVS.memory, &uniformDataVS.descriptor);
    ...
    for (uint32_t i = 0; i < instanceCount; i++) {
        ...
        uboVS.instance[i].model =
            glm::translate(glm::mat4(), glm::vec3(0.0f, i * offset - center, 0.0f));
        uboVS.instance[i].model =
            glm::rotate(uboVS.instance[i].model, glm::radians(60.0f), glm::vec3(1.0f, 0.0f, 0.0f));
    }
    ...
    VK_CHECK_RESULT(
        vkMapMemory(vkctx.device, uniformDataVS.memory, dataOffset, dataSize, 0, (void**)&pData));
...
void InstancedSampleBase::updateUniformBufferMatrices() {
    // Only updates the uniform buffer block part containing the global matrices

    // Projection
    uboVS.matrices.projection =
        glm::perspective(glm::radians(60.0f), (float)w_width / (float)w_height, 0.001f, 256.0f);

    // View
    uboVS.matrices.view = glm::translate(glm::mat4(), glm::vec3(0.0f, -1.0f, zoom));
    uboVS.matrices.view *= glm::translate(glm::mat4(), cameraPos);
    uboVS.matrices.view =
        glm::rotate(uboVS.matrices.view, glm::radians(rotation.x), glm::vec3(1.0f, 0.0f, 0.0f));
    uboVS.matrices.view =
        glm::rotate(uboVS.matrices.view, glm::radians(rotation.y), glm::vec3(0.0f, 1.0f, 0.0f));
    uboVS.matrices.view =
        glm::rotate(uboVS.matrices.view, glm::radians(rotation.z), glm::vec3(0.0f, 0.0f, 1.0f));

    // Only update the matrices part of the uniform buffer
    uint8_t* pData;
    VK_CHECK_RESULT(vkMapMemory(vkctx.device, uniformDataVS.memory, 0, sizeof(uboVS.matrices), 0,
                                (void**)&pData));
    memcpy(pData, &uboVS.matrices, sizeof(uboVS.matrices));
    vkUnmapMemory(vkctx.device, uniformDataVS.memory);
}

void InstancedSampleBase::prepareSamplerAndView() {
    // Create sampler.
...
void InstancedSampleBase::prepare(const char* const fragShaderName,
                                  const char* const vertShaderName, uint32_t instanceCountConstId,
                                  uint32_t instanceCountIn, uint32_t shaderDeclaredInstances) {
    ...
    preparePipelines(fragShaderName, vertShaderName, instanceCountConstId);
    setupDescriptorPool();
    setupDescriptorSet();
    vkctx.createDrawCommandBuffers();
    buildCommandBuffers();
...
const char* InstancedSampleBase::customizeTitle(const char* const baseTitle) {
    if (transcoded) {
