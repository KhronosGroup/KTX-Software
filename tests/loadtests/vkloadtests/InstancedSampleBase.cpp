...
InstancedSampleBase::InstancedSampleBase(VulkanContext& vkctx, uint32_t width, uint32_t height, const char* const szArgs,
    ...
    case 3:
        if (kTexture->isArray) {
            ...
            message << "Texture in \"" << getAssetPath() << szArgs << "\" is a 3D array texture which are not supported by Vulkan.";
    ...
    vk::Result res =
        vkctx.gpu.getImageFormatProperties(vkFormat, imageType, tiling, usageFlags, createFlags, &imageFormatProperties);
    ...
    if (kTexture->generateMipmaps) {
        neededFeatures |= vk::FormatFeatureFlagBits::eBlitDst | vk::FormatFeatureFlagBits::eBlitSrc |
    ...
    ktxresult = ktxTexture_VkUploadEx(kTexture, &vdi, &texture, static_cast<VkImageTiling>(tiling), VK_IMAGE_USAGE_SAMPLED_BIT,
...
void InstancedSampleBase::buildCommandBuffers() {
    ...
    for (uint32_t i = 0; i < vkctx.drawCmdBuffers.size(); ++i) {
        ...
        vkCmdBindVertexBuffers(vkctx.drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1, &static_cast<const VkBuffer&>(quad.vertices.buf),
                               offsets);
...
#undef dim
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eVertexBuffer, vertexBuffer.size() * sizeof(TAVertex), vertexBuffer.data(),
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eIndexBuffer, indexBuffer.size() * sizeof(uint32_t), indexBuffer.data(),
                       &quad.indices.buf, &quad.indices.mem);
}
...
void InstancedSampleBase::setupVertexDescriptions() {
    ...
    vertices.bindingDescriptions[0] =
        vk::VertexInputBindingDescription(VERTEX_BUFFER_BIND_ID, sizeof(TAVertex), vk::VertexInputRate::eVertex);
...
void InstancedSampleBase::setupDescriptorSetLayout() {
    ...
    vk::DescriptorSetLayoutCreateInfo descriptorLayout({}, static_cast<uint32_t>(descriptorBindings.size()),
...
void InstancedSampleBase::setupDescriptorSet() {
    ...
    vk::DescriptorImageInfo texDescriptor(sampler, imageView, vk::ImageLayout::eShaderReadOnlyOptimal);
...
void InstancedSampleBase::preparePipelines(const char* const fragShaderName, const char* const vertShaderName,
    ...
    vk::Result res = vkctx.device.createGraphicsPipelines(vkctx.pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.solid);
    if (res != vk::Result::eSuccess) {
...
void InstancedSampleBase::prepareUniformBuffers(uint32_t shaderDeclaredInstances) {
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eUniformBuffer,
                       vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent, uboSize, nullptr,
...
void InstancedSampleBase::prepare(const char* const fragShaderName, const char* const vertShaderName, uint32_t instanceCountConstId,
