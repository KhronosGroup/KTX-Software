...
VulkanLoadTestSample* TexturedCube::create(VulkanContext& vkctx, uint32_t width, uint32_t height, const char* const szArgs,
...
TexturedCube::TexturedCube(VulkanContext& vkctx, uint32_t width, uint32_t height, const char* const /*szArgs*/,
                           const std::string sBasePath)
    : VulkanLoadTestSample(vkctx, width, height, sBasePath), numTextures(1) {
    zoom = 1.0f;
...
TexturedCube::~TexturedCube() { cleanup(); }

void TexturedCube::resize(uint32_t width, uint32_t height) {
    w_width = width;
    w_height = height;
...
void TexturedCube::run(uint32_t msTicks) {
    ...
    vec3 eye((float)cos(msTicks * 0.001f) * fDistance, (float)sin(msTicks * 0.0007f) * fDistance,
...
void TexturedCube::buildCommandBuffer(const int bufferIndex) {
    const VkCommandBuffer& cmdBuf = vkctx.drawCmdBuffers[bufferIndex];

    const VkCommandBufferBeginInfo cmd_buf_info = {VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO, NULL, 0, NULL};
...
typedef union VkClearValue {
    ...
    // in C++11.
    VkClearValue clear_values[2] = {{{{0.0f, 0.2f, 0.2f, 1.0f}}}, {{{0.0f, 0}}}};
    ...
    VkViewport viewport = {0, 0, (float)w_width, (float)w_height, 0.0f, 1.0f};
    ...
    vkCmdBindVertexBuffers(cmdBuf, VERTEX_BUFFER_FIRST_BINDING_ID, 1, &static_cast<const VkBuffer&>(vertices.buf), offsets);
...
void TexturedCube::prepareUniformBuffer() {
    vkctx.createBuffer(vk::BufferUsageFlagBits::eUniformBuffer, sizeof(Uniforms), &uniforms, &uniformData.buffer,
                       &uniformData.memory, &uniformData.descriptor);
}

void TexturedCube::updateUniformBuffer() {
#if 0 /* Move to run? */
    ...
    uint8_t* pData;
    VK_CHECK_RESULT(vkMapMemory(vkctx.device, uniformData.memory, 0, sizeof(uniforms), 0, (void**)&pData));
...
void TexturedCube::prepareCubeDataBuffers() {
#if 1
    std::vector<float> vertexBuffer;

    vertexBuffer.insert(vertexBuffer.end(), &cube_face[0], &cube_face[ARRAY_LEN(cube_face)]);
#if 0
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eVertexBuffer, vertexBuffer.size() * sizeof(float), vertexBuffer.data(),
                       &vertices.buf, &vertices.mem);
#else
    ...
    createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vertexBuffer.size() * sizeof(Vertex), vertexBuffer.data(), &vertices.buf,
                 &vertices.mem);
#endif

    // Setup indices
    indices.count = ARRAY_LEN(cube_index_buffer);
    vkctx.createBuffer(vk::BufferUsageFlagBits::eIndexBuffer, sizeof(cube_index_buffer), (void*)cube_index_buffer, &indices.buf,
                       &indices.mem);
}

static VkVertexInputBindingDescription initVertexInputBindingDescription(uint32_t binding, uint32_t stride,
...
static VkVertexInputAttributeDescription initVertexInputAttributeDescription(uint32_t binding, uint32_t location, VkFormat format,
                                                                             uint32_t offset) {
...
static VkPipelineVertexInputStateCreateInfo initPipelineVertexInputStateCreateInfo() {
...
void TexturedCube::setupVertexDescriptions() {
    // Binding description
    vertices.bindingDescriptions.resize(1);
    vertices.bindingDescriptions[0] =
        initVertexInputBindingDescription(VERTEX_BUFFER_FIRST_BINDING_ID,
                                          /*sizeof(Vertex),*/ CUBE_FACE_STRIDE, VK_VERTEX_INPUT_RATE_VERTEX);
    ...
    vertices.attributeDescriptions[0] =
        initVertexInputAttributeDescription(VERTEX_BUFFER_FIRST_BINDING_ID, 0, VK_FORMAT_R32G32B32_SFLOAT, 0);
#if 0
...
void TexturedCube::createDescriptorSetLayout() {
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(vkctx.device, &dslcInfo, NULL, &descriptorSetLayout));
}

void TexturedCube::preparePipeline() {
    const VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = {
        VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO, NULL, 0, 1, &descriptorSetLayout, 0, NULL,
    };

    VK_CHECK_RESULT(vkCreatePipelineLayout(vkctx.device, &pipelineLayoutCreateInfo, NULL, &pipelineLayout));
    ...
    shaderStages[0] =
        static_cast<VkPipelineShaderStageCreateInfo>(loadShader(filepath + "cube.vert.spv", vk::ShaderStageFlagBits::eVertex));
    shaderStages[1] =
        static_cast<VkPipelineShaderStageCreateInfo>(loadShader(filepath + "cube.frag.spv", vk::ShaderStageFlagBits::eFragment));

#endif

    VkPipelineCacheCreateInfo pc{};
    pc.sType = VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO;
    VK_CHECK_RESULT(vkCreatePipelineCache(vkctx.device, &pc, NULL, &pipelineCache));

    VkGraphicsPipelineCreateInfo pipelineCreateInfo{};
    pipelineCreateInfo.sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
    pipelineCreateInfo.renderPass = vkctx.renderPass;
    pipelineCreateInfo.layout = pipelineLayout;
    pipelineCreateInfo.pVertexInputState = &static_cast<const VkPipelineVertexInputStateCreateInfo&>(vertices.inputState);
    ...
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(vkctx.device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline));
}

#if defined(INCLUDE_SHADERS)
VkShaderModule TexturedCube::prepareVertShader() {
    size_t codeSize;
...
VkShaderModule TexturedCube::prepareFragShader() {
    size_t codeSize;
...
VkShaderModule TexturedCube::createShaderModule(uint32_t* spv, size_t size) {
    ...
    VK_CHECK_RESULT(vkCreateShaderModule(vkctx.device, &shCreateInfo, NULL, &module));

    return module;
}
#endif

void TexturedCube::prepareDescriptorPool() {
    ...
    const VkDescriptorPoolCreateInfo dpoolCreateInfo = {
        VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO, NULL, 0, 1, 2, typeCounts,
    };
    U_ASSERT_ONLY VkResult err;

    err = vkCreateDescriptorPool(vkctx.device, &dpoolCreateInfo, NULL, &descriptorPool);
    assert(!err);
}

static VkWriteDescriptorSet initWriteDescriptorSet(VkDescriptorSet dstSet, VkDescriptorType type, uint32_t binding,
...
void TexturedCube::prepareDescriptorSet() {
    ...
    VK_CHECK_RESULT(vkAllocateDescriptorSets(vkctx.device, &allocInfo, &descriptorSet));
    ...
    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        ...
        initWriteDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
    ...
    vkUpdateDescriptorSets(vkctx.device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
}
