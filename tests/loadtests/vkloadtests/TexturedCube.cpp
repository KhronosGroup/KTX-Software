...
VulkanLoadTestSample* TexturedCube::create(VulkanContext& vkctx, uint32_t width, uint32_t height,
...
TexturedCube::TexturedCube(VulkanContext& vkctx, uint32_t width, uint32_t height,
                           const char* const /*szArgs*/, const std::string sBasePath)
...
typedef union VkClearValue {
    ...
    vkCmdBindVertexBuffers(cmdBuf, VERTEX_BUFFER_FIRST_BINDING_ID, 1,
                           &static_cast<const VkBuffer&>(vertices.buf), offsets);
...
void TexturedCube::prepareUniformBuffer() {
    vkctx.createBuffer(vk::BufferUsageFlagBits::eUniformBuffer, sizeof(Uniforms), &uniforms,
                       &uniformData.buffer, &uniformData.memory, &uniformData.descriptor);
}
...
void TexturedCube::updateUniformBuffer() {
    ...
    uint8_t* pData;
    VK_CHECK_RESULT(
        vkMapMemory(vkctx.device, uniformData.memory, 0, sizeof(uniforms), 0, (void**)&pData));
...
void TexturedCube::prepareCubeDataBuffers() {
    ...
    vkctx.createBuffer(vk::BufferUsageFlagBits::eVertexBuffer, vertexBuffer.size() * sizeof(float),
                       vertexBuffer.data(), &vertices.buf, &vertices.mem);
#else
    ...
    createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vertexBuffer.size() * sizeof(Vertex),
                 vertexBuffer.data(), &vertices.buf, &vertices.mem);
#endif

    // Setup indices
    indices.count = ARRAY_LEN(cube_index_buffer);
    vkctx.createBuffer(vk::BufferUsageFlagBits::eIndexBuffer, sizeof(cube_index_buffer),
                       (void*)cube_index_buffer, &indices.buf, &indices.mem);
}

static VkVertexInputBindingDescription initVertexInputBindingDescription(
...
static VkVertexInputAttributeDescription initVertexInputAttributeDescription(uint32_t binding,
                                                                             uint32_t location,
                                                                             VkFormat format,
                                                                             uint32_t offset) {
...
void TexturedCube::setupVertexDescriptions() {
    ...
    vertices.bindingDescriptions[0] = initVertexInputBindingDescription(
    ...
    //  Location 0 : position
    vertices.attributeDescriptions[0] = initVertexInputAttributeDescription(
        VERTEX_BUFFER_FIRST_BINDING_ID, 0, VK_FORMAT_R32G32B32_SFLOAT, 0);
#if 0
...
void TexturedCube::createDescriptorSetLayout() {
    ...
    VK_CHECK_RESULT(
        vkCreateDescriptorSetLayout(vkctx.device, &dslcInfo, NULL, &descriptorSetLayout));
}
...
void TexturedCube::preparePipeline() {
    ...
    VK_CHECK_RESULT(
        vkCreatePipelineLayout(vkctx.device, &pipelineLayoutCreateInfo, NULL, &pipelineLayout));
    ...
    shaderStages[0] = static_cast<VkPipelineShaderStageCreateInfo>(
        loadShader(filepath + "cube.vert.spv", vk::ShaderStageFlagBits::eVertex));
    shaderStages[1] = static_cast<VkPipelineShaderStageCreateInfo>(
        loadShader(filepath + "cube.frag.spv", vk::ShaderStageFlagBits::eFragment));
    ...
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(vkctx.device, pipelineCache, 1, &pipelineCreateInfo,
                                              nullptr, &pipeline));
}
...
static VkWriteDescriptorSet initWriteDescriptorSet(VkDescriptorSet dstSet, VkDescriptorType type,
                                                   uint32_t binding,
...
void TexturedCube::prepareDescriptorSet() {
    ...
    vkUpdateDescriptorSets(vkctx.device, static_cast<uint32_t>(writeDescriptorSets.size()),
