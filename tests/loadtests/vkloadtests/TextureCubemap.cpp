...
std::vector<vkMeshLoader::VertexLayout> vertexLayout = {vkMeshLoader::VERTEX_LAYOUT_POSITION,
                                                        vkMeshLoader::VERTEX_LAYOUT_NORMAL,
                                                        vkMeshLoader::VERTEX_LAYOUT_UV};

VulkanLoadTestSample* TextureCubemap::create(VulkanContext& vkctx, uint32_t width, uint32_t height,
                                             const char* const szArgs,
                                             const std::string sBasePath) {
    return new TextureCubemap(vkctx, width, height, szArgs, sBasePath, USE_GL_RH_NDC ? 1 : -1);
}

TextureCubemap::TextureCubemap(VulkanContext& vkctx, uint32_t width, uint32_t height,
                               const char* const szArgs, const std::string sBasePath, int32_t yflip)
    ...
    ktxresult = ktxTexture_CreateFromNamedFile(
        ktxfilepath.c_str(),
        preloadImages ? KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT : KTX_TEXTURE_CREATE_NO_FLAGS,
        &kTexture);
    ...
    for (uint32_t i = 0; i < vkctx.drawCmdBuffers.size(); ++i) {
        ...
        if (displaySkybox) {
            vkCmdBindDescriptorSets(
                vkctx.drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1,
            ...
            vkCmdBindVertexBuffers(vkctx.drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                                   &meshes.skybox.vertices.buf, offsets);
            vkCmdBindIndexBuffer(vkctx.drawCmdBuffers[i], meshes.skybox.indices.buf, 0,
        ...
        vkCmdBindVertexBuffers(vkctx.drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                               &meshes.objects[meshes.objectIndex].vertices.buf, offsets);
        vkCmdBindIndexBuffer(vkctx.drawCmdBuffers[i],
                             meshes.objects[meshes.objectIndex].indices.buf, 0,
                             VK_INDEX_TYPE_UINT32);
        vkCmdBindPipeline(vkctx.drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipelines.reflect);
        vkCmdDrawIndexed(vkctx.drawCmdBuffers[i], meshes.objects[meshes.objectIndex].indexCount, 1,
                         0, 0, 0);
...
void TextureCubemap::setupVertexDescriptions() {
    ...
    vertices.bindingDescriptions[0] = vk::VertexInputBindingDescription(
        VERTEX_BUFFER_BIND_ID, vkMeshLoader::vertexSize(vertexLayout),
...
void TextureCubemap::setupDescriptorPool() {
    ...
    std::vector<vk::DescriptorPoolSize> poolSizes = {
        {vk::DescriptorType::eUniformBuffer, 2}, {vk::DescriptorType::eCombinedImageSampler, 2}};
...
void TextureCubemap::setupDescriptorSets() {
    ...
    vk::DescriptorImageInfo cubeMapDescriptor(sampler, imageView,
                                              vk::ImageLayout::eShaderReadOnlyOptimal);

    std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
        // Binding 0 : Vertex shader uniform buffer
        vk::WriteDescriptorSet(descriptorSets.object, 0, 0, 1, vk::DescriptorType::eUniformBuffer,
                               nullptr, &uniformData.object.descriptor),
        // Binding 1 : Fragment shader cubemap sampler
        vk::WriteDescriptorSet(descriptorSets.object, 1, 0, 1,
                               vk::DescriptorType::eCombinedImageSampler, &cubeMapDescriptor)};
    ...
    writeDescriptorSets = {
        // Binding 0 : Vertex shader uniform buffer
        vk::WriteDescriptorSet(descriptorSets.skybox, 0, 0, 1, vk::DescriptorType::eUniformBuffer,
                               nullptr, &uniformData.skybox.descriptor),
        // Binding 1 : Fragment shader texture sampler
        vk::WriteDescriptorSet(descriptorSets.skybox, 1, 0, 1,
                               vk::DescriptorType::eCombinedImageSampler, &cubeMapDescriptor)};
...
void TextureCubemap::preparePipelines() {
    ...
    vk::Result res = vkctx.device.createGraphicsPipelines(
        vkctx.pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.skybox);
    if (res != vk::Result::eSuccess) {
    ...
    res = vkctx.device.createGraphicsPipelines(vkctx.pipelineCache, 1, &pipelineCreateInfo, nullptr,
                                               &pipelines.reflect);
    if (res != vk::Result::eSuccess) {
        throw bad_vulkan_alloc((int)res, "createGraphicsPipelines");
    }
...
void TextureCubemap::prepareUniformBuffers() {
    ...
    vkctx.createBuffer(
        ...
        sizeof(ubo), nullptr, &uniformData.object.buffer, &uniformData.object.memory,
    ...
    vkctx.createBuffer(
        vk::BufferUsageFlagBits::eUniformBuffer,
        vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
        sizeof(ubo), nullptr, &uniformData.skybox.buffer, &uniformData.skybox.memory,
...
void TextureCubemap::updateUniformBuffers() {
    ...
    ubo.modelView =
        glm::rotate(ubo.modelView, glm::radians(rotation.x), glm::vec3(1.0f, 0.0f, 0.0f));
    ubo.modelView =
        glm::rotate(ubo.modelView, glm::radians(rotation.y), glm::vec3(0.0f, 1.0f, 0.0f));
    ubo.modelView =
        glm::rotate(ubo.modelView, glm::radians(rotation.z), glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    uint8_t* pData;
    VK_CHECK_RESULT(
        vkMapMemory(vkctx.device, uniformData.object.memory, 0, sizeof(ubo), 0, (void**)&pData));
    ...
    VK_CHECK_RESULT(
        vkMapMemory(vkctx.device, uniformData.skybox.memory, 0, sizeof(ubo), 0, (void**)&pData));
    memcpy(pData, &ubo, sizeof(ubo));
...
void TextureCubemap::getOverlayText(VulkanTextOverlay* textOverlay, float yOffset) {
    ...
    textOverlay->addText("Press \"s\" to toggle skybox", 5.0f, yOffset,
                         VulkanTextOverlay::alignLeft);
    textOverlay->addText("Press \"space\" to change object", 5.0f, yOffset + 20.0f,
                         VulkanTextOverlay::alignLeft);
    textOverlay->addText("LOD bias: " + ss.str() + " (numpad +/- to change)", 5.0f, yOffset + 40.0f,
                         VulkanTextOverlay::alignLeft);
}
