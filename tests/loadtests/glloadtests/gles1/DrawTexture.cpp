...
DrawTexture::DrawTexture(uint32_t width, uint32_t height, const char *const szArgs,
    ...
    ktxresult =
        ktxTexture_CreateFromNamedFile(pathname.c_str(), KTX_TEXTURE_CREATE_NO_FLAGS, &kTexture);
...
void DrawTexture::resize(uint32_t uNewWidth, uint32_t uNewHeight) {
    ...
    framePMatrix *=
        glm::translate(glm::mat4(), glm::vec3((float)uWidth / 2, (float)uHeight / 2, 0));
...
void DrawTexture::run(uint32_t /*msTicks*/) {
    ...
    glDrawTexiOES(uWidth / 2 - uTexWidth / 2, (uHeight) / 2 - uTexHeight / 2, 0, uTexWidth,
                  uTexHeight);
