...
struct MeshBuffer {
    ...
    void FreeGLResources() {
        if (vertices.name) glDeleteBuffers(1, &vertices.name);
        if (indices.name) glDeleteBuffers(1, &indices.name);
        if (vao) glDeleteVertexArrays(1, &vao);
    }

    ~MeshBuffer() { FreeGLResources(); }
...
class GLMeshLoader {
  ...
  public:
    ...
    ~GLMeshLoader() { m_Entries.clear(); }

    // Load a mesh with some default flags
    void LoadMesh(const std::string& filename) {
        int flags = aiProcess_Triangulate | aiProcess_PreTransformVertices | aiProcess_JoinIdenticalVertices |
    ...
    void LoadMesh(const std::string& filename, int flags) {
        ...
        if (!pScene || pScene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !pScene->mRootNode) {
            ...
            message << "  Import via ASSIMP from\"" << filename << "\" failed. " << importer.GetErrorString() << std::endl;
    ...
    void InitMesh(unsigned int index, const aiMesh* paiMesh, const aiScene* pSrcScene) {
        ...
        case aiPrimitiveType_TRIANGLE:
            ...
            break;
        default:
            assert(false);  // Shouldn't happen because of triangulate.
        }
    }

    void CreateBuffers(glMeshLoader::MeshBuffer& meshBuffer, std::vector<glMeshLoader::VertexLayout> layout, float scale) {
        ...
        glBufferData(GL_ARRAY_BUFFER, meshBuffer.vertices.size, vertexBuffer.data(), GL_STATIC_DRAW);
        ...
        for (auto& layoutDetail : layout) {
            ...
            if (layoutDetail == glMeshLoader::VERTEX_LAYOUT_POSITION) {
                ...
                glVertexAttribPointer(attribNumber, 3, GL_FLOAT, GL_FALSE, stride, (void*)attribOffset);
            ...
            if (layoutDetail == glMeshLoader::VERTEX_LAYOUT_UV) {
                glEnableVertexAttribArray(attribNumber);
                glVertexAttribPointer(attribNumber, 2, GL_FLOAT, GL_FALSE, stride, (void*)attribOffset);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, meshBuffer.indices.size, indexBuffer.data(), GL_STATIC_DRAW);
