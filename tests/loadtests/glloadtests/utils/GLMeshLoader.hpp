...
class GLMeshLoader {
  ...
  public:
    ...
    void CreateBuffers(glMeshLoader::MeshBuffer& meshBuffer,
                       std::vector<glMeshLoader::VertexLayout> layout, float scale) {
        ...
        glBufferData(GL_ARRAY_BUFFER, meshBuffer.vertices.size, vertexBuffer.data(),
                     GL_STATIC_DRAW);
        ...
        for (auto& layoutDetail : layout) {
            ...
            if (layoutDetail == glMeshLoader::VERTEX_LAYOUT_POSITION) {
                ...
                glVertexAttribPointer(attribNumber, 3, GL_FLOAT, GL_FALSE, stride,
                                      (void*)attribOffset);
            ...
            if (layoutDetail == glMeshLoader::VERTEX_LAYOUT_UV) {
                glEnableVertexAttribArray(attribNumber);
                glVertexAttribPointer(attribNumber, 2, GL_FLOAT, GL_FALSE, stride,
                                      (void*)attribOffset);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, meshBuffer.indices.size, indexBuffer.data(),
                     GL_STATIC_DRAW);
