...
class GL3LoadTestSample : public LoadTestSample {
  public:
    ...
    GL3LoadTestSample(uint32_t width, uint32_t height, const char* const /*szArgs*/, const std::string sBasePath)
  ...
  protected:
    ...
    void loadMesh(std::string filename, glMeshLoader::MeshBuffer& meshBuffer, std::vector<glMeshLoader::VertexLayout> vertexLayout,
                  float scale);
