 ...
 */
InstancedSampleBase::InstancedSampleBase(uint32_t width, uint32_t height, const char* const szArgs,
    ...
    ktxTexture* kTexture;
    GLenum glerror;
    string ktxfilepath = externalFile ? ktxfilename : getAssetPath() + ktxfilename;
    ktxresult =
    ...
    ktxresult = ktxTexture_GLUpload(kTexture, &gnTexture, &texTarget, &glerror);
...
void InstancedSampleBase::run(uint32_t /*msTicks*/) {
    ...
    glDrawElementsInstanced(GL_TRIANGLES, quad.indexCount, GL_UNSIGNED_INT,
                            (GLvoid*)quad.indicesOffset, instanceCount);
...
void InstancedSampleBase::processArgs(string sArgs) {
    // Options descriptor
...
#define dim 2.5f
    ...
    TAVertex vertices[] = {{{dim, dim, 0.0f}, {1.0f, 1.0f}},
...
#undef dim
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(TAVertex), (GLvoid*)offset);
    offset += member_size(TAVertex, pos);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(TAVertex), (GLvoid*)offset);
...
void InstancedSampleBase::prepareUniformBuffers() {
    ...
    uint32_t uboSize = _PAD16(sizeof(uboVS.matrices)) + instanceCount * sizeof(UboInstanceData);
    ...
    for (uint32_t i = 0; i < instanceCount; i++) {
        ...
        uboVS.instance[i].model =
            glm::rotate(uboVS.instance[i].model, glm::radians(120.0f), glm::vec3(1.0f, 0.0f, 0.0f));
    }
    ...
    glUniformBlockBinding(gnInstancingProg, uProgramUniforms, uniformBufferBindId);
    updateUniformBufferMatrices();
    glUseProgram(0);
...
void InstancedSampleBase::updateUniformBufferMatrices() {
    ...
    uboVS.matrices.projection =
        glm::perspective(glm::radians(60.0f), (float)w_width / w_height, .01f, 256.f);
    ...
    uint8_t* pData =
        (uint8_t*)glMapBufferRange(GL_UNIFORM_BUFFER, 0, sizeof(uboVS.matrices), GL_MAP_WRITE_BIT);
    memcpy(pData, &uboVS.matrices, sizeof(uboVS.matrices));
    glUnmapBuffer(GL_UNIFORM_BUFFER);
#else
    glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(uboVS.matrices), &uboVS.matrices);
#endif
}

void InstancedSampleBase::prepareSampler() {
    ...
    glUseProgram(gnInstancingProg);
    if ((uSampler = glGetUniformLocation(gnInstancingProg, "uSampler")) == -1) {
        std::stringstream message;
...
void InstancedSampleBase::prepareProgram(ShaderSource& fs, ShaderSource& vs) {
...
void InstancedSampleBase::prepare(ShaderSource& fs, ShaderSource& vs) {
