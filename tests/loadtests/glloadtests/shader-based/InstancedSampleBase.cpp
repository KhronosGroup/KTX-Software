 ...
 */
InstancedSampleBase::InstancedSampleBase(uint32_t width, uint32_t height, const char* const szArgs, const string sBasePath)
    : GL3LoadTestSample(width, height, szArgs, sBasePath), texUnit(GL_TEXTURE0), uniformBufferBindId(0), bInitialized(false) {
    zoom = -15.0f;
    rotationSpeed = 0.25f;
    ...
    if (kTexture->generateMipmaps) {
        ...
        uint32_t maxDim = (std::max)((std::max)(kTexture->baseWidth, kTexture->baseHeight), kTexture->baseDepth);
...
void InstancedSampleBase::run(uint32_t /*msTicks*/) {
    ...
    glDrawElementsInstanced(GL_TRIANGLES, quad.indexCount, GL_UNSIGNED_INT, (GLvoid*)quad.indicesOffset, instanceCount);
...
void InstancedSampleBase::prepareUniformBuffers() {
    ...
    for (uint32_t i = 0; i < instanceCount; i++) {
        ...
        uboVS.instance[i].model = glm::rotate(uboVS.instance[i].model, glm::radians(120.0f), glm::vec3(1.0f, 0.0f, 0.0f));
    }
...
void InstancedSampleBase::updateUniformBufferMatrices() {
    ...
    // Projection
    uboVS.matrices.projection = glm::perspective(glm::radians(60.0f), (float)w_width / w_height, .01f, 256.f);
    ...
    uint8_t* pData = (uint8_t*)glMapBufferRange(GL_UNIFORM_BUFFER, 0, sizeof(uboVS.matrices), GL_MAP_WRITE_BIT);
