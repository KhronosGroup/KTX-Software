...
DrawTexture::DrawTexture(uint32_t width, uint32_t height, const char* const szArgs, const std::string sBasePath)
    ...
    if (KTX_SUCCESS == ktxresult) {
        ...
        if (target != GL_TEXTURE_1D && target != GL_TEXTURE_2D) {
            ...
            message << "DrawTexture supports only 1D & 2D textures. \"" << ktxfilename << "\" is not one of these.";
            throw std::runtime_error(message.str());
        }
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(frame_position) + sizeof(frame_color) + sizeof(quad_position) + sizeof(quad_color) + sizeof(quad_texture),
                 NULL, GL_STATIC_DRAW);
    ...
    quadMvMatrix = glm::scale(glm::mat4(), glm::vec3((float)uTexWidth / 2, (float)uTexHeight / 2, 1));
...
void DrawTexture::resize(uint32_t uNewWidth, uint32_t uNewHeight) {
    ...
    frameMvMatrix = glm::scale(glm::mat4(), glm::vec3((float)(uWidth - 1) / 2, (float)(uHeight - 1) / 2, 1));
}
