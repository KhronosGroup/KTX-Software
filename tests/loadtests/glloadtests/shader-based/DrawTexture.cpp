...
LoadTestSample* DrawTexture::create(uint32_t width, uint32_t height, const char* const szArgs,
                                    const std::string sBasePath) {
    return new DrawTexture(width, height, szArgs, sBasePath);
}

DrawTexture::DrawTexture(uint32_t width, uint32_t height, const char* const szArgs,
                         const std::string sBasePath)
    ...
    if (KTX_SUCCESS == ktxresult) {
        ...
        if (target != GL_TEXTURE_1D && target != GL_TEXTURE_2D) {
            ...
            message << "DrawTexture supports only 1D & 2D textures. \"" << ktxfilename
                    << "\" is not one of these.";
            throw std::runtime_error(message.str());
        }
    ...
    quadMvMatrix =
        glm::scale(glm::mat4(), glm::vec3((float)uTexWidth / 2, (float)uTexHeight / 2, 1));
...
void DrawTexture::processArgs(std::string sArgs) {
    // Options descriptor
...
void DrawTexture::resize(uint32_t uNewWidth, uint32_t uNewHeight) {
    ...
    pMatrix *= glm::translate(glm::mat4(), glm::vec3((float)uWidth / 2, (float)uHeight / 2, 0));
    ...
    frameMvMatrix =
        glm::scale(glm::mat4(), glm::vec3((float)(uWidth - 1) / 2, (float)(uHeight - 1) / 2, 1));
}
...
void DrawTexture::run(uint32_t /*msTicks*/) {
    ...
    glUniformMatrix4fv(gulMvMatrixLocCP, 1, GL_FALSE, glm::value_ptr(frameMvMatrix));
    glUniformMatrix4fv(gulPMatrixLocCP, 1, GL_FALSE, glm::value_ptr(pMatrix));
    glDrawArrays(GL_LINE_LOOP, 0, 4);

    glBindVertexArray(gnVaos[QUAD]);
    glUseProgram(gnTexProg);
    glUniformMatrix4fv(gulMvMatrixLocTP, 1, GL_FALSE, glm::value_ptr(quadMvMatrix));
