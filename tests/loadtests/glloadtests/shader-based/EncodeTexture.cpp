 ...
 *
 * This is used principally to check the encoders are properly linked on platforms where the ktx
 * tools are unavailable and libktx is a static library.
 *
...
LoadTestSample* EncodeTexture::create(uint32_t width, uint32_t height, const char* const szArgs,
                                      const std::string sBasePath) {
    return new EncodeTexture(width, height, szArgs, sBasePath);
}

EncodeTexture::EncodeTexture(uint32_t width, uint32_t height, const char* const szArgs,
                             const std::string sBasePath)
    ...
    ktxresult = ktxTexture_CreateFromNamedFile(filename.c_str(), KTX_TEXTURE_CREATE_NO_FLAGS,
                                               (ktxTexture**)&kTexture);
    ...
    ktxresult = ktxTexture_GLUpload(ktxTexture(kTexture), &gnTexture, &target, &glerror);
    ...
    glBufferData(
        GL_ARRAY_BUFFER,
        sizeof(cube_face) + sizeof(cube_color) + sizeof(cube_texture) + sizeof(cube_normal), NULL,
        GL_STATIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(cube_index_buffer), cube_index_buffer,
                 GL_STATIC_DRAW);
...
void EncodeTexture::processArgs(std::string sArgs) {
    // Options descriptor
...
void EncodeTexture::resize(uint32_t uWidth, uint32_t uHeight) {
    glm::mat4 matProj;
...
void EncodeTexture::run(uint32_t msTicks) {
