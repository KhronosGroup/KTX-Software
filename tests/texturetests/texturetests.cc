...
class TextureWriterTestHelper
  ...
  public:
    ...
    void resize(createFlags flags, ktx_uint32_t layers, ktx_uint32_t faces, ktx_uint32_t dimensions,
                ktx_uint32_t w, ktx_uint32_t h, ktx_uint32_t d) {
        WriterTestHelper<component_type, numComponents, internalformat>::resize(
            flags, layers, faces, dimensions, w, h, d);
    ...
    bool compareTexture1Images(ktx_uint8_t* pData) {
        for (ktx_uint32_t level = 0; level < images.size(); level++) {
            ...
            for (ktx_uint32_t layer = 0; layer < images[0].size(); layer++) {
                for (ktx_uint32_t faceSlice = 0; faceSlice < images[level][layer].size();
                     faceSlice++) {
                    if (rowPadding == 0) {
            ...
            for (ktx_uint32_t layer = 0; layer < images[level].size(); layer++) {
                for (ktx_uint32_t faceSlice = 0; faceSlice < images[level][layer].size();
                    ...
                    ktx_uint8_t* imageDataPtr =
                        (ktx_uint8_t*)(images[level][layer][faceSlice].data());
                    result = ktxTexture1_SetImageFromMemory(texture, level, layer, faceSlice,
                                                            imageDataPtr, imageBytes);
    ...
    class createInfo : public ktxTextureCreateInfo {
      public:
        ...
        void resize(createFlags flags, ktx_uint32_t layers, ktx_uint32_t faces,
                    ktx_uint32_t dimensions, ktx_uint32_t w, ktx_uint32_t h, ktx_uint32_t d) {
            baseWidth = w;
            baseHeight = h;
            baseDepth = d;
  ...
  protected:
    ktxTextureTestBase(ktxFormatVersionEnum fv) : pixelSize(16) {
        ...
        if (fv == KTX_FORMAT_VERSION_ONE) {
            ...
            errorCode = ktxTexture1_Create(&texinfo, KTX_TEXTURE_CREATE_ALLOC_STORAGE,
                                           (ktxTexture1**)&texture);
        } else {
            ...
            errorCode = ktxTexture2_Create(&texinfo, KTX_TEXTURE_CREATE_ALLOC_STORAGE,
                                           (ktxTexture2**)&texture);
        ...
        for (ktx_uint32_t level = 0; level < texinfo.numLevels; level++) {
            ...
            for (ktx_uint32_t layer = 0; layer < texinfo.numLayers; layer++) {
                ...
                for (ktx_uint32_t faceSlice = 0; faceSlice < numImages; faceSlice++) {
                    ktxTexture_SetImageFromMemory(texture, level, layer, faceSlice, it->data,
                                                  it->size);
                }
            }
            it++;
    ...
    KTX_error_code iterCallback(int miplevel, int /*face*/, int width, int /*height*/,
                                int /*depth*/, ktx_uint64_t faceLodSize, void* pixels) {
    ...
    static KTX_error_code iterCallback(int miplevel, int face, int width, int height, int depth,
...
template <typename component_type, ktx_uint32_t numComponents, GLenum internalformat>
class ktxTexture2TestBase
    : public ktxTextureTestBase<component_type, numComponents, internalformat> {
  protected:
    ...
    ktxTexture2TestBase()
        : ktxTextureTestBase<component_type, numComponents, internalformat>(
              KTX_FORMAT_VERSION_TWO) {}

    bool compareTexture(ktxTexture2* texture) {
        if (texture->vkFormat !=
            (uint32_t)vkGetFormatFromOpenGLInternalFormat(helper.texinfo.glInternalformat))
            return false;
        if (texture->baseWidth != texinfo.baseWidth) return false;
        if (texinfo.baseHeight == 0) {
            if (texture->baseHeight != 1) return false;
...
class ktxTexture2_CreateTest : public ::testing::Test {
  protected:
    ktx_error_code_e create(VkFormat format, ktx_uint32_t width = 16u, ktx_uint32_t height = 16u,
                            ktx_uint32_t depth = 1u, ktx_uint32_t dimensions = 2u,
                            ktx_uint32_t levels = 1u, ktx_uint32_t layers = 1u,
                            ktx_uint32_t faces = 1u, bool isArray = false,
...
class ktxTexture1WriteTestBase : public ::testing::Test {
  public:
    using createFlags =
        typename WriterTestHelper<component_type, numComponents, internalformat>::createFlags;
    ...
    void runTest(bool writeMetadata) {
        ...
        if (writeMetadata)
            ktxHashList_AddKVPair(&texture->kvDataHead, KTX_ORIENTATION_KEY,
                                  (unsigned int)strlen(helper.orientation) + 1, helper.orientation);
        ...
        ASSERT_TRUE(result == KTX_SUCCESS)
            << "ktxTexture_WriteToMemory failed: " << ktxErrorString(result);
...
TEST_F(ktxTexture1_CreateTest, InvalidOpOnSetImagesNoStorage) {
    ...
    EXPECT_EQ(ktxTexture1_SetImageFromMemory(texture, 0, 0, 0, imageDataPtr, imageBytes),
              KTX_INVALID_OPERATION);
    ASSERT_TRUE(result == KTX_SUCCESS);
...
TEST_F(ktxTexture1_CreateTest, CreateEmptySetImagesWriteToMemory) {
    ...
    ktxHashList_AddKVPair(&texture->kvDataHead, KTX_ORIENTATION_KEY,
                          (unsigned int)strlen(orientation) + 1, orientation);
    result = helper.copyImagesToTexture(texture);
    ASSERT_TRUE(result == KTX_SUCCESS);
    EXPECT_EQ(helper.compareTexture1Images(texture->pData), true);
...
TEST_F(ktxTexture_KVDataTest, KVDataDeserialized) {
    ...
    if (ktxMemFile != NULL) {
        ...
        char s, t;
        result = ktxHashList_FindValue(&texture->kvDataHead, KTX_ORIENTATION_KEY, &valueLen,
                                       (void**)&pValue);
        EXPECT_EQ(result, KTX_SUCCESS);
...
TEST_F(ktxTexture_KVDataTest, LoadRawKVData) {
    ...
    if (ktxMemFile != NULL) {
        result = ktxTexture_CreateFromMemory(ktxMemFile, ktxMemFileLen,
                                             KTX_TEXTURE_CREATE_RAW_KVDATA_BIT, &texture);
        EXPECT_EQ(result, KTX_SUCCESS);
        ASSERT_TRUE(texture != NULL)
            << "ktxTexture_CreateFromMemory failed: " << ktxErrorString(result);
...
TEST_F(ktxTexture_KVDataTest, SkipKVData) {
    ...
    if (ktxMemFile != NULL) {
        result = ktxTexture_CreateFromMemory(ktxMemFile, ktxMemFileLen,
                                             KTX_TEXTURE_CREATE_SKIP_KVDATA_BIT, &texture);
        EXPECT_EQ(result, KTX_SUCCESS);
        ASSERT_TRUE(texture != NULL)
            << "ktxTexture_CreateFromMemory failed: " << ktxErrorString(result);
        ASSERT_TRUE(texture->kvData == NULL) << "Raw KVData should not be loaded";
...
TEST_F(ktxTexture2_CreateCopyTest, CreateCopy) {
    ...
    if (ktxMemFile != NULL) {
        ...
        EXPECT_EQ(result, KTX_SUCCESS);
        ASSERT_TRUE(copyTexture != NULL)
            << "ktxTexture_CreateFromMemory failed: " << ktxErrorString(result);

        EXPECT_EQ(compareTexture(copyTexture), true);
        EXPECT_EQ(memcmp(texture->pData, copyTexture->pData, texture->dataSize), 0);
        EXPECT_EQ(
            memcmp(texture->_protected, copyTexture->_protected, sizeof(ktxTexture_protected)), 0);
        ktx_size_t privateSize =
            sizeof(ktxTexture2_private) + sizeof(ktxLevelIndexEntry) * (texture->numLevels - 1);
...
class TestCreateInfo : public ktxTextureCreateInfo {
  public:
    ...
    TestCreateInfo(ktx_uint32_t width, ktx_uint32_t height, ktx_uint32_t depth)
        : TestCreateInfo(width, height, depth, 2, GL_RGBA8, VK_FORMAT_R8G8B8A8_UNORM, KTX_FALSE, 1,
                         1) {}

    TestCreateInfo(ktx_uint32_t width, ktx_uint32_t height, ktx_uint32_t depth,
                   ktx_uint32_t dimensions, ktx_uint32_t internalformat, ktx_uint32_t vkformat,
                   ktx_bool_t array, ktx_uint32_t faces, ktx_uint32_t layers) {
        baseWidth = width;
        baseHeight = height;
        baseDepth = depth;
    ...
    static ktx_uint32_t levelsFromSize(ktx_uint32_t width, ktx_uint32_t height,
                                       ktx_uint32_t depth) {
...
TEST(ktxTexture_calcImageSize, ImageSizeAtEachLevelRGBA2D) {
    ...
    result =
        ktxTexture1_Create(&createInfo, KTX_TEXTURE_CREATE_NO_STORAGE, (ktxTexture1**)&texture);
    EXPECT_EQ(result, KTX_SUCCESS);
    ASSERT_TRUE(texture != NULL) << "ktxTexture1_Create failed: " << ktxErrorString(result);
...
TEST(ktxTexture_GetImageOffsetTest, ImageOffsetLevel) {
    ...
    result = ktxTexture1_Create(&helper.createInfo, KTX_TEXTURE_CREATE_NO_STORAGE,
                                (ktxTexture1**)&texture);
    EXPECT_EQ(result, KTX_SUCCESS);
    ASSERT_TRUE(texture != NULL) << "ktxTexture1_Create failed: " << ktxErrorString(result);
  ...
  public:
    ...
    void runTest(bool writeOrientationMeta, bool writeWriterMeta = true) {
        ...
        if (writeWriterMeta) {
            ktxHashList_AddKVPair(&texture->kvDataHead, KTX_WRITER_KEY,
                                  (uint32_t)helper.writer_ktx2.size(), helper.writer_ktx2.data());
            ktxHashList_AddKVPair(hl, KTX_WRITER_KEY, (uint32_t)helper.writer_ktx2.size(),
        ...
        ASSERT_TRUE(result == KTX_SUCCESS)
            << "ktxTexture_WriteKTX2ToMemory failed: " << ktxErrorString(result);
    ...
    void runTest(const char* unrecognizedKey, const char* proprietaryKey) {
        ...
        for (uint32_t i = 0; i < 2; i++) {
            // N.B. Writer metadata is not legal in a KTX v1 file but we know
            // we're going to write this out as a v2 file so okay.
            ktxHashList_AddKVPair(hlists[i], KTX_WRITER_KEY, (uint32_t)helper.writer_ktx2.size(),
                                  helper.writer_ktx2.data());
            if (unrecognizedKey) {
                ktxHashList_AddKVPair(hlists[i], unrecognizedKey, sizeof(rubbishValue),
                                      rubbishValue);
            }
            if (proprietaryKey) {
                ktxHashList_AddKVPair(hlists[i], proprietaryKey, sizeof(rubbishValue),
                                      rubbishValue);
            }
...
class ktxTexture2ReadTestBase
  ...
  public:
    ...
    void resize(createFlags flags, ktx_uint32_t numLayers, ktx_uint32_t numFaces,
                ktx_uint32_t numDimensions, ktx_uint32_t width, ktx_uint32_t height,
                ktx_uint32_t depth) {
        ktxTexture1* texture = 0;
        KTX_error_code result;
        ...
        ASSERT_TRUE(result == KTX_SUCCESS)
            << "ktxTexture1_WriteKTX2ToMemory failed: " << ktxErrorString(result);
    ...
    void runTest() {
        ...
        result = ktxTexture2_CreateFromMemory(ktx2MemFile, ktx2MemFileLen,
                                              KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &texture2);
...
TEST_F(ktxTexture2_MetadataTest, EmptyValue) {
    ...
    if (ktxMemFile != NULL) {
        ...
        ktx_uint8_t* value;
        result =
            ktxHashList_FindValue(&texture->kvDataHead, "MSCtestKey", &valueLen, (void**)&value);
        EXPECT_EQ(result, KTX_SUCCESS);
...
TEST_F(ktxTexture2_MetadataTest, NoLibVersionDupOnMultipleWrites) {
    ...
    if (ktxMemFile != NULL) {
        ...
        for (uint32_t i = 0; i < iterations; i++) {
            result =
                ktxTexture_WriteToMemory(ktxTexture(texture), &newMemFiles[i], &newMemFileLens[i]);
            EXPECT_EQ(result, KTX_SUCCESS);
        }
        ...
        for (uint32_t i = 0; i < iterations; i++) {
            ktx_uint32_t valueLen;
            ktx_uint8_t* value;
            result = ktxTexture2_CreateFromMemory(newMemFiles[i], newMemFileLens[i],
                                                  KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &texture);
            ASSERT_TRUE(result == KTX_SUCCESS);
            ASSERT_TRUE(texture != NULL)
                << "ktxTexture_CreateFromMemory failed: " << ktxErrorString(result);
            ...
            result =
                ktxHashList_FindValue(&texture->kvDataHead, "KTXwriter", &valueLen, (void**)&value);
            EXPECT_EQ(result, KTX_SUCCESS);
...
TEST_F(ktxTexture2_MetadataTest, LibVersionUpdatedCorrectly) {
    ...
    if (ktxMemFile != NULL) {
        ...
        result = ktxHashList_FindValue(&texture->kvDataHead, "KTXwriter", &curWriterLen,
                                       (void**)&curWriterVal);
        EXPECT_EQ(result, KTX_SUCCESS);
        // We want ktxWriteTo* to NUL terminate the value when adding
        // the libktx version.
        ...
        result = ktxHashList_AddKVPair(&texture->kvDataHead, "KTXwriter",
                                       (ktx_uint32_t)writer.length(), writer.c_str());
        EXPECT_EQ(result, KTX_SUCCESS);
        ...
        result = ktxHashList_FindValue(&texture->kvDataHead, "KTXwriter", &newWriterLen,
                                       (void**)&newWriterVal);
        EXPECT_EQ(result, KTX_SUCCESS);
...
TEST(UnicodeFileNames, CreateFrom) {
    std::vector<std::string> fileSet = {
        "hűtő.ktx",        "hűtő.ktx2", "نَسِيج.ktx",  "نَسِيج.ktx2", "テクスチャ.ktx",
    ...
    for (it = fileSet.begin(); it < fileSet.end(); it++) {
        ...
        result =
            ktxTexture_CreateFromNamedFile(path.c_str(), KTX_TEXTURE_CREATE_NO_FLAGS, &texture);
        EXPECT_EQ(result, KTX_SUCCESS);
        ...
        if (path.substr(dotIndex + 1).compare("ktx") == 0) {
            result = ktxTexture1_CreateFromNamedFile(path.c_str(), KTX_TEXTURE_CREATE_NO_FLAGS,
                                                     (ktxTexture1**)&texture);
        } else {
            result = ktxTexture2_CreateFromNamedFile(path.c_str(), KTX_TEXTURE_CREATE_NO_FLAGS,
                                                     (ktxTexture2**)&texture);
        }
        EXPECT_EQ(result, KTX_SUCCESS);
        EXPECT_NE(texture, (ktxTexture*)0);
