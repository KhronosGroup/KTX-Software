...
std::ostream& operator<<(std::ostream& out, const TextureSet& h) { return out << h.ktxPath; }
...
std::ostream& operator<<(std::ostream& out, const FormatFeature& h) { return out << ktxTranscodeFormatString(h.format); }

vector<TextureSet> allTextureSets = {{"color_grid_basis.ktx2", "color_grid.basis", true, false},
#if 1
...
class TextureCombinationsTest : public ::testing::TestWithParam<tuple<TextureSet, FormatFeature>> {};

INSTANTIATE_TEST_SUITE_P(AllCombinations, TextureCombinationsTest,
                         ::testing::Combine(::testing::ValuesIn(allTextureSets), ::testing::ValuesIn(allFormats)));
...
bool isPo2(uint32_t i) { return (i & (i - 1)) == 0; }
...
void test_texture_set(TextureSet& textureSet, FormatFeature& format) {
    ...
    if (!(isPo2(bWidth) && isPo2(bHeight)) && !format.supportsNonPo2) {
        return;
    }
    ...
    result = ktxTexture2_CreateFromMemory((const ktx_uint8_t*)data, (ktx_size_t)fsize, KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT,
                                          (ktxTexture2**)&newTex);

    ASSERT_EQ(result, KTX_SUCCESS);

    result = ktxTexture2_TranscodeBasis(newTex, format.format, 0);
