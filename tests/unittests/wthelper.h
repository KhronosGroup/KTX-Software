...
class WriterTestHelper {
  public:
    ...
    void resize(createFlags flags, ktx_uint32_t layers, ktx_uint32_t faces, ktx_uint32_t dimensions,
        ...
        ktxHashList_AddKVPair(&kvHash, KTX_ORIENTATION_KEY, (unsigned int)strlen(orientation) + 1,
                              orientation);
        ktxHashList_Serialize(&kvHash, &kvDataLen, &kvData);

        ktxHashList_Construct(&kvHash_ktx2);
        ktxHashList_AddKVPair(&kvHash_ktx2, KTX_WRITER_KEY, (ktx_uint32_t)writer_ktx2.size(),
                              writer_ktx2.data());
        ...
        for (ktx_uint32_t level = 0; level < numLevels; level++) {
            ...
            for (ktx_uint32_t layer = 0; layer < numLayers; layer++) {
                for (ktx_uint32_t faceSlice = 0; faceSlice < numImages; faceSlice++) {
                    ...
                    if (memcmp(images[level][layer][faceSlice].data(), pData, imageBytes))
                        return false;
                    pData += imageBytes;
                }
            }
    ...
    class texinfo : public wthTexInfo {
      public:
        ...
        void resize(GLuint levels, GLuint layers, GLuint faces, GLuint dimensions, bool array,
        ...
        bool compare(KTX_header* header) {
            if (header->glType == glType && header->glTypeSize == glTypeSize &&
                header->glFormat == glFormat && header->glInternalformat == glInternalformat &&
        ...
        bool compare(KTX_header2* header) {
            ...
            if (header->vkFormat == (ktx_uint32_t)format && header->pixelWidth == baseWidth &&
                ...
                header->pixelDepth == headerPixelDepth && header->layerCount == headerNumLayers &&
                header->faceCount == numFaces && header->levelCount == numLevels &&
        ...
        bool compare(ktxTexture2* texture) {
            ...
            if (texture->vkFormat == (ktx_uint32_t)format && texture->baseWidth == baseWidth &&
                texture->baseHeight == baseHeight && texture->baseDepth == baseDepth &&
                texture->numLayers == numLayers && texture->numFaces == numFaces &&
