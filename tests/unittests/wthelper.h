...
extern "C" KTX_error_code appendLibId(ktxHashList* head, ktxHashListEntry* writerEntry);
...
class WriterTestHelper {
  public:
    enum createFlagBits { eNone = 0x00, eMipmapped = 0x01, eGenerateMipmaps = 0x02, eArray = 0x04 };
    ...
    void resize(createFlags flags, ktx_uint32_t layers, ktx_uint32_t faces, ktx_uint32_t dimensions, ktx_uint32_t w, ktx_uint32_t h,
                ktx_uint32_t d, std::vector<component_type>* requestedColor = nullptr) {
        assert(faces == 1 || d == 1);
        ...
        texinfo.resize(numLevels, layers, faces, dimensions, isArray, w, h, d);
        ...
        for (ktx_uint32_t level = 0, count = 0; level < numLevels; level++) {
            ...
            for (ktx_uint32_t layer = 0; layer < numLayers; layer++) {
                ...
                for (ktx_uint32_t faceSlice = 0; faceSlice < numImages; faceSlice++) {
                    ...
                    imageList[count].size = pixelCount * numComponents * sizeof(component_type);
                    imageDataSize += imageList[count].size;
                    imageList[count++].data = (GLubyte*)images[level][layer][faceSlice].data();
                }
            }
        }
        ...
        case 3:
            ...
            snprintf(orientation, sizeof(orientation), KTX_ORIENTATION3_FMT, 'r', 'd', 'i');
            break;
        }
        ...
        ktxHashList_AddKVPair(&kvHash, KTX_ORIENTATION_KEY, (unsigned int)strlen(orientation) + 1, orientation);
        ktxHashList_Serialize(&kvHash, &kvDataLen, &kvData);

        ktxHashList_Construct(&kvHash_ktx2);
        ktxHashList_AddKVPair(&kvHash_ktx2, KTX_WRITER_KEY, (ktx_uint32_t)writer_ktx2.size(), writer_ktx2.data());

        // Get the library to add its Id to the writer key so it will be
        // included in the serialized data.
        ktxHashListEntry* pWriter;
        ktxHashList_FindEntry(&kvHash_ktx2, KTX_WRITER_KEY, &pWriter);
        appendLibId(&kvHash_ktx2, pWriter);

        ktxHashList_Serialize(&kvHash_ktx2, &kvDataLenWriter_ktx2, &kvDataWriter_ktx2);
        ktxHashList_AddKVPair(&kvHash_ktx2, KTX_ORIENTATION_KEY, dimensions + 1, orientation_ktx2);
    ...
    bool compareRawImages(ktx_uint8_t* pData) {
        for (ktx_uint32_t level = 0; level < numLevels; level++) {
            ...
            rowBytes = levelWidth * sizeof(component_type) * numComponents;
            ...
            if (faceLodSize != expectedFaceLodSize) return false;
    ...
    bool compareRawImages(ktxLevelIndexEntry levelIndex[], ktx_uint8_t* baseAddr) {
        for (ktx_uint32_t level = 0; level < numLevels; level++) {
            ...
            if (levelSize != expectedLevelSize) return false;
            ...
            for (ktx_uint32_t layer = 0; layer < numLayers; layer++) {
                for (ktx_uint32_t faceSlice = 0; faceSlice < numImages; faceSlice++) {
                    ...
                    if (memcmp(images[level][layer][faceSlice].data(), pData, imageBytes)) return false;
                    pData += imageBytes;
                }
            }
        }
    ...
    KTX_error_code copyImagesToTexture(ktxTexture* texture) {
        ...
        for (ktx_uint32_t level = 0; level < images.size(); level++) {
            for (ktx_uint32_t layer = 0; layer < images[level].size(); layer++) {
                for (ktx_uint32_t faceSlice = 0; faceSlice < images[level][layer].size(); faceSlice++) {
                    ...
                    result = ktxTexture_SetImageFromMemory(texture, level, layer, faceSlice, imageDataPtr, imageBytes);
    ...
    class texinfo : public wthTexInfo {
      public:
        ...
        void resize(GLuint levels, GLuint layers, GLuint faces, GLuint dimensions, bool array, GLsizei width, GLsizei height,
                    GLsizei depth) {
            this->numLayers = layers;
            this->numFaces = faces;
        ...
        bool compare(KTX_header* header) {
            if (header->glType == glType && header->glTypeSize == glTypeSize && header->glFormat == glFormat &&
                header->glInternalformat == glInternalformat && header->glBaseInternalformat == glBaseInternalformat &&
                header->pixelWidth == baseWidth && header->pixelHeight == headerPixelHeight &&
                header->pixelDepth == headerPixelDepth && header->numberOfArrayElements == headerNumLayers &&
                header->numberOfFaces == numFaces && header->numberOfMipLevels == numLevels)
                return true;
            else
                return false;
        }
        ...
        bool compare(KTX_header2* header) {
            VkFormat format = vkGetFormatFromOpenGLInternalFormat(glInternalformat);

            // Should find better way to test this. Code we're testing uses the
            // same switch to convert format.
            if (header->vkFormat == (ktx_uint32_t)format && header->pixelWidth == baseWidth &&
                header->pixelHeight == headerPixelHeight && header->pixelDepth == headerPixelDepth &&
                header->layerCount == headerNumLayers && header->faceCount == numFaces && header->levelCount == numLevels &&
                header->supercompressionScheme >= KTX_SS_BEGIN_RANGE && header->supercompressionScheme <= KTX_SS_END_RANGE)
                return true;
            else
                return false;
        }
        ...
        bool compare(ktxTexture2* texture) {
            ...
            if (texture->vkFormat == (ktx_uint32_t)format && texture->baseWidth == baseWidth && texture->baseHeight == baseHeight &&
                texture->baseDepth == baseDepth && texture->numLayers == numLayers && texture->numFaces == numFaces &&
                texture->numLevels == numLevels && texture->supercompressionScheme >= KTX_SS_BEGIN_RANGE &&
