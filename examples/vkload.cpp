...
Texture::Texture(const std::string ktxfile) {
    ...
    */
    ...
    ktxresult = ktxTexture_CreateFromNamedFile((getAssetPath() + ktxfile).c_str(),
                                               KTX_TEXTURE_CREATE_NO_FLAGS, &kTexture);
    if (KTX_SUCCESS != ktxresult) {
        std::stringstream message;

        message << "Creation of ktxTexture from \"" << getAssetPath() << ktxfile
    ...
    char* pValue;
    uint32_t valueLen;
    if (KTX_SUCCESS == ktxHashTable_FindValue(&kTexture->kvDataHead, KTX_ORIENTATION_KEY, &valueLen,
                                              (void**)&pValue)) {
        char s, t;
