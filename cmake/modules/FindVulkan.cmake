...
# This is FindVulkan.cmake from CMake 4.4 with the following changes needed
# by, but by no means unique to, the vkloadtests target of KTX-Software:
# - Add "KosmicKrisp" component for those who want to use this instead of
#   MoltenVK.
# - Add "validation" component for those who need the path to the Khronos
#   validation layer library to ship it with the app, e.g. in an Apple
#   app bundle.
# - Temporarily set CMAKE_FIND_FRAMEWORKS to FIRST when searching for iOS
#   libraries so the iOS frameworks will be found instead of either nothing
#   or the macOS dylibs. Arguably the user could set this before calling
#   find_package but the user would have to know to do that. This prevents
#   them encountering mysterious find failures.
...
``MoltenVK``
  ...
  Finds the MoltenVK library, which is available on macOS and iOS, and
  implements a subset of Vulkan API over Apple Metal graphics framework.

``KosmicKrisp``

  Finds the KosmicKrisp library, which is available on macOS, and
  implements the Vulkan API over Apple Metal hardware.
...
``validation``
  ...
  Finds Khronos validation layer library. Only needed for macOS
  to find the layer for copying into an app bundle and on IOS because the
  library must be linked to the application. Set automatically for IOS.
  TODO: Add components for other layer libraries.
...
``Vulkan::KosmicKrisp``
  ...
  Target encapsulating the KosmicKrisp library usage requirements, available if
  this library is found in the SDK.
...
``Vulkan_KosmicKrisp_FOUND``
  .. versionadded:: 3.24

  Boolean indicating whether the SDK provides the KosmicKrisp library.
...
``Vulkan_khronos_validation_FOUND``

  Boolean indicating whether the SDK provides the Khronos validation layer.
...
if (IOS OR validation IN_LIST Vulkan_FIND_COMPONENTS)
...
if(KosmicKrisp IN_LIST Vulkan_FIND_COMPONENTS)
  find_library(Vulkan_KosmicKrisp_LIBRARY
    NAMES vulkan_kosmickrisp
    HINTS
      ${_Vulkan_hint_library_search_paths})
  mark_as_advanced(Vulkan_KosmicKrisp_LIBRARY)
endif()
...
if(Vulkan_KosmicKrisp_LIBRARY)
  set(Vulkan_KosmicKrisp_FOUND TRUE)
else()
  set(Vulkan_KosmicKrisp_FOUND FALSE)
endif()

if(Vulkan_Layer_KHRONOS_VALIDATION_LIBRARY)
  set(Vulkan_validation_FOUND TRUE)
else()
  set(Vulkan_validation_FOUND FALSE)
endif()
...
if(Vulkan_KosmicKrisp_FOUND)
  if(Vulkan_KosmicKrisp_LIBRARY AND NOT TARGET Vulkan::KosmicKrisp)
    add_library(Vulkan::KosmicKrisp SHARED IMPORTED)
    set_target_properties(Vulkan::KosmicKrisp
      PROPERTIES
