...
ktxWriteToMemory PY_ktxTexture_WriteToMemory(ktxTexture *texture) {
    ...
    return (ktxWriteToMemory){.bytes = ppDstBytes, .size = pSize, .error = error};
}

ktxImageOffset PY_ktxTexture_GetImageOffset(ktxTexture *texture, ktx_uint32_t level, ktx_uint32_t layer, ktx_uint32_t faceSlice) {
...
ktxWriteToMemory PY_ktxHashList_FindValue(ktxHashList *list, const char *key) {
    ...
    return (ktxWriteToMemory){.bytes = pValue, .size = valueLen, .error = err};
}
...
ktxWriteToMemory PY_ktxHashListEntry_GetKey(ktxHashListEntry *entry) {
    ...
    return (ktxWriteToMemory){.bytes = pKey, .size = keyLen, .error = err};
}
...
ktxHashList *PY_ktxTexture_get_kvDataHead(ktxTexture *texture) { return &texture->kvDataHead; }

ktxHashListEntry *PY_ktxHashList_get_listHead(ktxHashList *list) { return *list; }
