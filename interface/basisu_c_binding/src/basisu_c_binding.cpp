...
uint32_t basis_file::getImageTranscodedSizeInBytes(uint32_t image_index, uint32_t level_index, uint32_t format) {
    ...
    if (format >= (uint32_t)basist::transcoder_texture_format::cTFTotalTextureFormats) return 0;

    uint32_t orig_width, orig_height, total_blocks;
    if (!m_transcoder.get_image_level_desc(m_file, (uint32_t)byteLength, image_index, level_index, orig_width, orig_height,
                                           total_blocks))
        return 0;
    ...
    if (basis_transcoder_format_is_uncompressed(transcoder_format)) {
        ...
        if (transcoder_format == transcoder_texture_format::cTFPVRTC1_4_RGB ||
            ...
            // For PVRTC1, Basis only writes (or requires) total_blocks * bytes_per_block. But GL requires extra padding for very
            // small textures: https://www.khronos.org/registry/OpenGL/extensions/IMG/IMG_texture_compression_pvrtc.txt
...
uint32_t basis_file::transcodeImage(void* dst, uint32_t dst_size, uint32_t image_index, uint32_t level_index, uint32_t format,
                                    uint32_t /*pvrtc_wrap_addressing*/, uint32_t get_alpha_for_opaque_formats) {
    assert(m_magic == MAGIC);
    if (m_magic != MAGIC) return 0;
    ...
    if (basis_transcoder_format_is_uncompressed(transcoder_format)) {
        ...
        status = m_transcoder.transcode_image_level(m_file, byteLength, image_index, level_index, dst, orig_width * orig_height,
                                                    transcoder_format, flags, orig_width, nullptr, orig_height);
    } else {
        ...
        if (transcoder_format == transcoder_texture_format::cTFPVRTC1_4_RGB ||
            transcoder_format == transcoder_texture_format::cTFPVRTC1_4_RGBA) {
            // For PVRTC1, Basis only writes (or requires) total_blocks * bytes_per_block. But GL requires extra padding for very
            // small textures: https://www.khronos.org/registry/OpenGL/extensions/IMG/IMG_texture_compression_pvrtc.txt The
            // transcoder will clear the extra bytes followed the used blocks to 0.
            const uint32_t width = (orig_width + 3) & ~3;
            const uint32_t height = (orig_height + 3) & ~3;
        ...
        status = m_transcoder.transcode_image_level(m_file, byteLength, image_index, level_index, dst, dst_size / bytes_per_block,
                                                    static_cast<basist::transcoder_texture_format>(format), flags);
    }
...
KTX_BASISU_API void ktx_basisu_basis_init() { basisu_transcoder_init(); }
...
KTX_BASISU_API void ktx_basisu_close_basis(basis_file* basis) { basis->close(); }

KTX_BASISU_API void ktx_basisu_delete_basis(basis_file* basis) { delete basis; }
...
KTX_BASISU_API uint32_t ktx_basisu_getNumImages(basis_file* basis) { return basis->getNumImages(); }
...
KTX_BASISU_API uint32_t ktx_basisu_get_y_flip(basis_file* basis) { return basis->getYFlip(); }

KTX_BASISU_API uint32_t ktx_basisu_get_is_etc1s(basis_file* basis) { return basis->getIsEtc1s(); }

KTX_BASISU_API basis_texture_type ktx_basisu_get_texture_type(basis_file* basis) { return basis->getTextureType(); }

KTX_BASISU_API uint32_t ktx_basisu_getImageTranscodedSizeInBytes(basis_file* basis, uint32_t image_index, uint32_t level_index,
                                                                 uint32_t format) {
    return basis->getImageTranscodedSizeInBytes(image_index, level_index, format);
}

KTX_BASISU_API uint32_t ktx_basisu_startTranscoding(basis_file* basis) { return basis->startTranscoding(); }

KTX_BASISU_API uint32_t ktx_basisu_transcodeImage(basis_file* basis, void* dst, uint32_t dst_size, uint32_t image_index,
                                                  uint32_t level_index, uint32_t format, uint32_t pvrtc_wrap_addressing,
                                                  uint32_t get_alpha_for_opaque_formats) {
    return basis->transcodeImage(dst, dst_size, image_index, level_index, format, pvrtc_wrap_addressing,
