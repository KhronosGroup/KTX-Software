...
uint32_t basis_file::getImageWidth(uint32_t image_index, uint32_t level_index) {
    ...
    if (!m_transcoder.get_image_level_desc(m_file, byteLength, image_index, level_index, orig_width,
...
uint32_t basis_file::getImageTranscodedSizeInBytes(uint32_t image_index, uint32_t level_index,
                                                   uint32_t format) {
    assert(m_magic == MAGIC);
    if (m_magic != MAGIC) return 0;
    ...
    uint32_t orig_width, orig_height, total_blocks;
    if (!m_transcoder.get_image_level_desc(m_file, (uint32_t)byteLength, image_index, level_index,
                                           orig_width, orig_height, total_blocks))
        return 0;

    const transcoder_texture_format transcoder_format =
        static_cast<transcoder_texture_format>(format);
...
uint32_t basis_file::transcodeImage(void* dst, uint32_t dst_size, uint32_t image_index,
                                    uint32_t level_index, uint32_t format,
                                    uint32_t /*pvrtc_wrap_addressing*/,
    ...
    uint32_t flags =
        get_alpha_for_opaque_formats ? cDecodeFlagsTranscodeAlphaDataToOpaqueFormats : 0;

    uint32_t status;

    if (basis_transcoder_format_is_uncompressed(transcoder_format)) {
        MAYBE_UNUSED const uint32_t bytes_per_pixel =
        ...
        status = m_transcoder.transcode_image_level(
            m_file, byteLength, image_index, level_index, dst, orig_width * orig_height,
        ...
        status = m_transcoder.transcode_image_level(
            m_file, byteLength, image_index, level_index, dst, dst_size / bytes_per_block,
...
KTX_BASISU_API uint32_t ktx_basisu_open_basis(basis_file* basis, const uint8_t* data,
                                              uint32_t length) {
...
KTX_BASISU_API uint32_t ktx_basisu_getImageWidth(basis_file* basis, uint32_t image_index,
                                                 uint32_t level_index) {
    return basis->getImageWidth(image_index, level_index);
}

KTX_BASISU_API uint32_t ktx_basisu_getImageHeight(basis_file* basis, uint32_t image_index,
                                                  uint32_t level_index) {
...
KTX_BASISU_API uint32_t ktx_basisu_getImageTranscodedSizeInBytes(basis_file* basis,
...
KTX_BASISU_API uint32_t ktx_basisu_transcodeImage(basis_file* basis, void* dst, uint32_t dst_size,
                                                  uint32_t image_index, uint32_t level_index,
                                                  uint32_t format, uint32_t pvrtc_wrap_addressing,
                                                  uint32_t get_alpha_for_opaque_formats) {
    return basis->transcodeImage(dst, dst_size, image_index, level_index, format,
                                 pvrtc_wrap_addressing, get_alpha_for_opaque_formats);
}
