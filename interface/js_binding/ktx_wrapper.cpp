...
class texture {
  public:
    ...
    texture(const emscripten::val& data) : m_ptr{nullptr, &destroy} {
        ...
        emscripten::val memoryView = data["constructor"].new_(
            memory, reinterpret_cast<uintptr_t>(bytes.data()), data["length"].as<uint32_t>());
        ...
        KTX_error_code result = ktxTexture_CreateFromMemory(
            bytes.data(), bytes.size(), KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &ptr);
        if (result != KTX_SUCCESS) {
            std::cout << "ERROR: Failed to create from memory: " << ktxErrorString(result)
                      << std::endl;
        }
    ...
    struct ktxOrientation orientation() const { return m_ptr->orientation; }
    ...
    ktx_error_code_e transcodeBasis(const val& targetFormat, const val& decodeFlags) {
        ...
        KTX_error_code result = ktxTexture2_TranscodeBasis(
            ktxTexture2(m_ptr.get()), targetFormat.as<ktx_texture_transcode_fmt_e>(),
