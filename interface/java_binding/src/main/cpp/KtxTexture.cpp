...
extern "C" JNIEXPORT jbyteArray JNICALL Java_org_khronos_ktx_KtxTexture_getData(JNIEnv *env, jobject thiz) {
    ...
    env->SetByteArrayRegion(outputArray, 0, static_cast<jsize>(dataSize), reinterpret_cast<jbyte *>(data));
...
extern "C" JNIEXPORT jint JNICALL Java_org_khronos_ktx_KtxTexture_getRowPitch(JNIEnv *env, jobject thiz, jint level) {
    return static_cast<jint>(ktxTexture_GetRowPitch(get_ktx_texture(env, thiz), level));
}

extern "C" JNIEXPORT jlong JNICALL Java_org_khronos_ktx_KtxTexture_getImageSize(JNIEnv *env, jobject thiz, jint level) {
    return ktxTexture_GetImageSize(get_ktx_texture(env, thiz), level);
}

extern "C" JNIEXPORT jlong JNICALL Java_org_khronos_ktx_KtxTexture_getImageOffset(JNIEnv *env, jobject thiz, jint level, jint layer,
                                                                                  jint faceSlice) {
    ktx_size_t pOffset = 0;
    KTX_error_code result = ktxTexture_GetImageOffset(get_ktx_texture(env, thiz), level, layer, faceSlice, &pOffset);
...
extern "C" JNIEXPORT jint JNICALL Java_org_khronos_ktx_KtxTexture_setImageFromMemory(JNIEnv *env, jobject thiz, jint level,
                                                                                     jint layer, jint faceSlice,
                                                                                     jbyteArray srcArray) {
    ...
    jint result = ktxTexture_SetImageFromMemory(get_ktx_texture(env, thiz), level, layer, faceSlice, src, srcSize);
...
extern "C" JNIEXPORT jint JNICALL Java_org_khronos_ktx_KtxTexture_writeToNamedFile(JNIEnv *env, jobject thiz, jstring dstName) {
...
extern "C" JNIEXPORT jbyteArray JNICALL Java_org_khronos_ktx_KtxTexture_writeToMemory(JNIEnv *env, jobject thiz) {
    ...
    env->SetByteArrayRegion(out, 0, static_cast<jsize>(pSize), reinterpret_cast<const jbyte *>(ppDstBytes));
