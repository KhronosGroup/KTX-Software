...
public class KtxTranscodeFlagBits {
	...
	public static String stringFor(int n) {
		...
		if ((n & KTX_TF_PVRTC_DECODE_TO_NEXT_POW2) != 0) {
			if (sb.length() != 0) {
				sb.append("|");
			}
			sb.append("KTX_TF_PVRTC_DECODE_TO_NEXT_POW2");
		}
		if ((n & KTX_TF_TRANSCODE_ALPHA_DATA_TO_OPAQUE_FORMATS) != 0) {
			if (sb.length() != 0) {
				sb.append("|");
			}
			sb.append("KTX_TF_TRANSCODE_ALPHA_DATA_TO_OPAQUE_FORMATS");
		}
		if ((n & KTX_TF_HIGH_QUALITY) != 0) {
			if (sb.length() != 0) {
				sb.append("|");
			}
			sb.append("KTX_TF_HIGH_QUALITY");
		}
		if (sb.length() == 0)
		{
