...
static void
copy_rgba_to_rgba(uint8_t* rgbadst, uint8_t* rgbasrc, uint32_t, ktx_size_t image_size,
                  swizzle_e[4]) {
...
void
swizzle_to_rgba(uint8_t* rgbadst, uint8_t* rgbasrc, uint32_t src_len, ktx_size_t image_size,
                swizzle_e swizzle[4]) {
...
ktxTexture2_rewriteDfd4BasisLzETC1S(ktxTexture2* This, alpha_content_e alphaContent,
                                    bool isLuminance, swizzle_e swizzle[4]) {
...
static KTX_error_code
ktxTexture2_rewriteDfd4Uastc(ktxTexture2* This, alpha_content_e alphaContent, bool isLuminance,
                             swizzle_e swizzle[4]) {
    uint32_t* cdfd = This->pDfd;
    uint32_t* cbdb = cdfd + 1;
    ...
    for (uint32_t level = 0; level < This->numLevels; level++) {
        ...
        for (uint32_t layer = 0; layer < This->numLayers; layer++) {
            ...
            for (ktx_uint32_t slice = 0; slice < faceSlices; slice++) {
                ...
                copycb((uint8_t*)iit->get_ptr(), This->pData + offset, num_components, image_size,
                       comp_mapping);
                ++iit;
            }
        }
...
#define DEBUG_ENCODER 0
    ...
    if (params->uastc) {
        ...
        bgd_size = sizeof(ktxBasisLzGlobalHeader) + image_desc_size * num_images +
        ...
        for (uint32_t level = 0; level < This->numLevels; level++) {
            ...
            for (uint32_t layer = 0; layer < This->numLayers; layer++) {
                ...
                for (uint32_t faceSlice = 0; faceSlice < faceSlices; faceSlice++) {
                    ...
                    kimages[image].rgbSliceByteOffset =
    ...
    for (int32_t level = This->numLevels - 1; level >= 0; level--) {
        ...
        memcpy(This->pData + level_offset, &bf[level_file_offsets[level]],
