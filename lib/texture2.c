...
KTX_error_code
  ...
  private
	->_firstLevelFileOffset = private->_levelIndex[This->numLevels - 1].byteOffset;
	for (ktx_uint32_t level = 0; level < This->numLevels; level++) {
	  private
		->_levelIndex[level].byteOffset -= private->_firstLevelFileOffset;
	}
	...
	if (pHeader->supercompressionGlobalData.byteLength > 0) {
	  ...
	  private
		->_sgdByteLength = pHeader->supercompressionGlobalData.byteLength;
