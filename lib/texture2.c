...
ktx_uint32_t e5b9g9r9_ufloat_comparator[e5b9g9r9_bdbwordcount] = {
    ...
    shift(DESCRIPTORBLOCKSIZE, e5b9g9r9_bdbwordcount * sizeof(ktx_uint32_t)) |
        shift(VERSIONNUMBER, 2),
    // N.B. Allow various values of primaries, transfer & flags
    shift(FLAGS, KHR_DF_FLAG_ALPHA_STRAIGHT) | shift(TRANSFER, KHR_DF_TRANSFER_LINEAR) |
        shift(PRIMARIES, KHR_DF_PRIMARIES_BT709) | shift(MODEL, KHR_DF_MODEL_RGBSDA),
    0,                      // texelBlockDimension3~0
    shift(BYTESPLANE0, 4),  // All other bytesPlane fields are 0.
    0,                      // bytesPlane7~4
    sampleshift(CHANNELID, KHR_DF_CHANNEL_RGBSDA_RED) | sampleshift(BITLENGTH, 8) |
        sampleshift(BITOFFSET, 0),
    0,     // samplePosition3~0
    0,     // sampleLower
    8448,  // sampleUpper
    sampleshift(CHANNELID, KHR_DF_CHANNEL_RGBSDA_RED | KHR_DF_SAMPLE_DATATYPE_EXPONENT) |
        sampleshift(BITLENGTH, 4) | sampleshift(BITOFFSET, 27),
    0,   // samplePosition3~0
    15,  // sampleLower
    31,  // sampleUpper
    sampleshift(CHANNELID, KHR_DF_CHANNEL_RGBSDA_GREEN) | sampleshift(BITLENGTH, 8) |
        sampleshift(BITOFFSET, 9),
    0,     // samplePosition3~0
    0,     // sampleLower
    8448,  // sampleUpper
    sampleshift(CHANNELID, KHR_DF_CHANNEL_RGBSDA_GREEN | KHR_DF_SAMPLE_DATATYPE_EXPONENT) |
        sampleshift(BITLENGTH, 4) | sampleshift(BITOFFSET, 27),
    0,   // samplePosition3~0
    15,  // sampleLower
    31,  // sampleUpper
    sampleshift(CHANNELID, KHR_DF_CHANNEL_RGBSDA_BLUE) | sampleshift(BITLENGTH, 8) |
        sampleshift(BITOFFSET, 18),
    0,     // samplePosition3~0
    0,     // sampleLower
    8448,  // sampleUpper
    sampleshift(CHANNELID, KHR_DF_CHANNEL_RGBSDA_BLUE | KHR_DF_SAMPLE_DATATYPE_EXPONENT) |
        sampleshift(BITLENGTH, 4) | sampleshift(BITOFFSET, 27),
    0,   // samplePosition3~0
    15,  // sampleLower
    31,  // sampleUpper
...
#if !defined(BITFIELD_ORDER_FROM_MSB)
                   && !memcmp(((uint32_t*)&e5b9g9r9_ufloat_comparator) +
                                  KHR_DF_WORD_TEXELBLOCKDIMENSION0,
                              &pBdb[KHR_DF_WORD_TEXELBLOCKDIMENSION0],
                              sizeof(e5b9g9r9_ufloat_comparator) -
                                  (KHR_DF_WORD_TEXELBLOCKDIMENSION0) * sizeof(uint32_t))) {
#else
...
ktxTexture2_construct(ktxTexture2* This, ktxTextureCreateInfo* createInfo,
    ...
    for (ktx_uint32_t level = 0; level < This->numLevels; level++) {
        levelIndex[level].uncompressedByteLength =
            ktxTexture_calcLevelSize(ktxTexture(This), level, KTX_FORMAT_VERSION_TWO);
...
ktxTexture2_constructCopy(ktxTexture2* This, ktxTexture2* orig) {
    ...
    ktx_size_t privateSize =
        sizeof(ktxTexture2_private) + sizeof(ktxLevelIndexEntry) * (orig->numLevels - 1);
    This->_private = (ktxTexture2_private*)malloc(privateSize);
    if (This->_private == NULL) {
        result = KTX_OUT_OF_MEMORY;
        goto cleanup;
    ...
    if (orig->_private->_sgdByteLength > 0) {
        ...
        memcpy(This->_private->_supercompressionGlobalData,
               orig->_private->_supercompressionGlobalData, orig->_private->_sgdByteLength);
    }
...
ktxTexture2_constructFromStreamAndHeader(ktxTexture2* This, ktxStream* pStream,
                                         KTX_header2* pHeader, ktxTextureCreateFlags createFlags) {
    ...
    if (pHeader->dataFormatDescriptor.byteOffset == 0 ||
        pHeader->dataFormatDescriptor.byteLength < 16) {
    ...
    if (pBDFD->model == KHR_DF_MODEL_ETC1S || pBDFD->model == KHR_DF_MODEL_UASTC) {
        if (sampleCount < 1 || sampleCount > 2 ||
            (sampleCount == 2 && pBDFD->model == KHR_DF_MODEL_UASTC)) {
            // Invalid sample count
            result = KTX_FILE_DATA_ERROR;
            goto cleanup;
        }
    ...
    if (pHeader->keyValueData.byteLength > 0) {
        uint32_t expectedOffset =
            pHeader->dataFormatDescriptor.byteOffset + pHeader->dataFormatDescriptor.byteLength;
        ...
        if (!(createFlags & KTX_TEXTURE_CREATE_SKIP_KVDATA_BIT)) {
            ...
            if (!(createFlags & KTX_TEXTURE_CREATE_RAW_KVDATA_BIT)) {
                ...
                result = ktxHashList_FindValue(&This->kvDataHead, KTX_ORIENTATION_KEY,
                                               &orientationLen, (void**)&orientationStr);
                ...
                result = ktxHashList_FindValue(&This->kvDataHead, KTX_ANIMDATA_KEY, &animDataLen,
                                               (void**)animData);
                assert(result != KTX_INVALID_VALUE);
                if (result == KTX_SUCCESS) {
    ...
    if (pHeader->supercompressionGlobalData.byteLength > 0) {
        ...
        result = stream->setpos(stream, pHeader->supercompressionGlobalData.byteOffset);
        if (result != KTX_SUCCESS) goto cleanup;
        ...
        result =
            stream->read(stream, private->_supercompressionGlobalData, private->_sgdByteLength);
 ...
 */
static KTX_error_code
ktxTexture2_constructFromNamedFile(ktxTexture2* This, const char* const filename,
                                   ktxTextureCreateFlags createFlags) {
    KTX_error_code result;
    ktxStream stream;
    FILE* file;
 ...
 */
static KTX_error_code
ktxTexture2_constructFromMemory(ktxTexture2* This, const ktx_uint8_t* bytes, ktx_size_t size,
                                ktxTextureCreateFlags createFlags) {
    KTX_error_code result;
    ktxStream stream;
 ...
 */
KTX_error_code
ktxTexture2_Create(ktxTextureCreateInfo* createInfo, ktxTextureCreateStorageEnum storageAllocation,
                   ktxTexture2** newTex) {
    KTX_error_code result;
 ...
 */
KTX_error_code
ktxTexture2_CreateFromStdioStream(FILE* stdioStream, ktxTextureCreateFlags createFlags,
                                  ktxTexture2** newTex) {
    KTX_error_code result;
    if (newTex == NULL) return KTX_INVALID_VALUE;
 ...
 */
KTX_error_code
ktxTexture2_CreateFromNamedFile(const char* const filename, ktxTextureCreateFlags createFlags,
                                ktxTexture2** newTex) {
    KTX_error_code result;
 ...
 */
KTX_error_code
ktxTexture2_CreateFromStream(ktxStream* stream, ktxTextureCreateFlags createFlags,
                             ktxTexture2** newTex) {
    KTX_error_code result;
    if (newTex == NULL) return KTX_INVALID_VALUE;
...
ktxTexture2_calcDataSizeLevels(ktxTexture2* This, ktx_uint32_t levels) {
    ...
    for (ktx_uint32_t i = levels - 1; i > 0; i--) {
        ktx_size_t levelSize =
            ktxTexture_calcLevelSize(ktxTexture(This), i, KTX_FORMAT_VERSION_TWO);
 ...
 */
KTX_error_code
ktxTexture2_GetImageOffset(ktxTexture2* This, ktx_uint32_t level, ktx_uint32_t layer,
...
ktxTexture2_IterateLoadLevelFaces(ktxTexture2* This, PFNKTXITERCB iterCb, void* userdata) {
    ...
    if (This->supercompressionScheme != KTX_SS_NONE &&
        This->supercompressionScheme != KTX_SS_ZSTD && This->supercompressionScheme != KTX_SS_ZLIB)
    ...
    for (ktx_int32_t level = This->numLevels - 1; level >= 0; --level) {
        ...
        if (This->supercompressionScheme == KTX_SS_ZSTD) {
            ...
            result = ktxUncompressZLIBInt(uncompressedDataBuf, &uncompressedDataSize, dataBuf,
                                          levelSize);
            if (result != KTX_SUCCESS) return result;
        }
        ...
        if (This->isCubemap && !This->isArray) {
            ...
            for (ktx_uint32_t face = 0; face < This->numFaces; ++face) {
                result = iterCb(level, face, width, height, depth, (ktx_uint32_t)faceSize, pFace,
                                userdata);
                pFace += faceSize;
                if (result != KTX_SUCCESS) goto cleanup;
            }
            ...
            result =
                iterCb(level, 0, width, height, depth, (ktx_uint32_t)levelSize, pData, userdata);
            if (result != KTX_SUCCESS) goto cleanup;
        }
    }
...
KTX_error_code ktxTexture2_inflateZstdInt(ktxTexture2* This, ktx_uint8_t* pDeflatedData,
                                          ktx_uint8_t* pInflatedData,
                                          ktx_size_t inflatedDataCapacity);

KTX_error_code ktxTexture2_inflateZLIBInt(ktxTexture2* This, ktx_uint8_t* pDeflatedData,
                                          ktx_uint8_t* pInflatedData,
                                          ktx_size_t inflatedDataCapacity);
...
ktxTexture2_inflateZstdInt(ktxTexture2* This, ktx_uint8_t* pDeflatedData,
                           ktx_uint8_t* pInflatedData, ktx_size_t inflatedDataCapacity) {
    DECLARE_PROTECTED(ktxTexture);
    ...
    for (int32_t level = This->numLevels - 1; level >= 0; level--) {
        size_t levelByteLength =
            ZSTD_decompressDCtx(dctx, pInflatedData + levelOffset, inflatedDataCapacity,
