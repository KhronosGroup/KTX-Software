...
KTX_error_code ktxTexture2_transcodeLzEtc1s(ktxTexture2* This, alpha_content_e alphaContent,
                                            ktxTexture2* prototype,
                                            ktx_transcode_fmt_e outputFormat,
                                            ktx_transcode_flags transcodeFlags);
KTX_error_code ktxTexture2_transcodeUastc(ktxTexture2* This, alpha_content_e alphaContent,
                                          ktxTexture2* prototype, ktx_transcode_fmt_e outputFormat,
                                          ktx_transcode_flags transcodeFlags);
 ...
 */
KTX_error_code
ktxTexture2_TranscodeBasis(ktxTexture2* This, ktx_transcode_fmt_e outputFormat,
                           ktx_transcode_flags transcodeFlags) {
    ...
    case KTX_TTF_PVRTC1_4_RGBA:
        vkFormat =
            srgb ? VK_FORMAT_PVRTC1_4BPP_SRGB_BLOCK_IMG : VK_FORMAT_PVRTC1_4BPP_UNORM_BLOCK_IMG;
        break;
    case KTX_TTF_PVRTC2_4_RGB:
    case KTX_TTF_PVRTC2_4_RGBA:
        vkFormat =
            srgb ? VK_FORMAT_PVRTC2_4BPP_SRGB_BLOCK_IMG : VK_FORMAT_PVRTC2_4BPP_UNORM_BLOCK_IMG;
        break;
    ...
    if (textureFormat == basis_tex_format::cETC1S) {
        result = ktxTexture2_transcodeLzEtc1s(This, alphaContent, prototype, outputFormat,
                                              transcodeFlags);
    } else {
        result =
            ktxTexture2_transcodeUastc(This, alphaContent, prototype, outputFormat, transcodeFlags);
    }
 ...
 */
KTX_error_code
ktxTexture2_transcodeLzEtc1s(ktxTexture2* This, alpha_content_e alphaContent,
                             ktxTexture2* prototype, ktx_transcode_fmt_e outputFormat,
                             ktx_transcode_flags transcodeFlags) {
    DECLARE_PRIVATE(priv, This);
    DECLARE_PRIVATE(protoPriv, prototype);
    ...
    for (uint32_t level = 1; level <= This->numLevels; level++) {
        ...
        firstImages[level] =
            firstImages[level - 1] + layersFaces * MAX(This->baseDepth >> (level - 1), 1);
    }
    ...
    bit.decode_palettes(bgdh.endpointCount, BGD_ENDPOINTS_ADDR(bgd, imageCount),
                        bgdh.endpointsByteLength, bgdh.selectorCount,
    ...
    for (int32_t level = This->numLevels - 1; level >= 0; level--) {
        ...
        for (; image < endImage; image++) {
            ...
            status = bit.transcode_image(
                (transcoder_texture_format)outputFormat, pXcodedData + writeOffset,
                ...
                level, (uint32_t)(levelOffset + imageDesc.rgbSliceByteOffset),
                imageDesc.rgbSliceByteLength,
                (uint32_t)(levelOffset + imageDesc.alphaSliceByteOffset),
                imageDesc.alphaSliceByteLength, transcodeFlags, alphaContent != eNone, isVideo,
