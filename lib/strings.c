...
typedef int
    errorStrings_SIZE_ASSERT[sizeof(errorStrings) / sizeof(char*) - 1 == KTX_ERROR_MAX_ENUM];
...
ktxErrorString(KTX_error_code error) {
    if (error > KTX_ERROR_MAX_ENUM) return "Unrecognized error code";
...
ktxSupercompressionSchemeString(ktxSupercmpScheme scheme) {
    ...
    case KTX_SS_ZLIB:
        return "KTX_SS_ZLIB";
    default:
        if (scheme < KTX_SS_BEGIN_VENDOR_RANGE || scheme >= KTX_SS_BEGIN_RESERVED)
 ...
 */
const char*
ktxBUImageFlagsBitString(ktx_uint32_t bit_index, bool bit_value) {
    if (!bit_value) return NULL;

    switch (1u << bit_index) {
    case ETC1S_P_FRAME:
        return "ETC1S_P_FRAME";
    default:
        return NULL;
    }
}
