...
ktxTexture2_CompressBasis(ktxTexture2* This)
{
    ...
    if (This->isCompressed)
        return KTX_INVALID_OPERATION;  // Best to do compression &
    ...
    //if (bfh.m_flags & cBASISHeaderFlagHasAlphaSlices)
        slice_desc_size = sizeof(ktxBasisGlobalFullSliceDesc);
    //else
    ...
    ktxBasisGlobalHeader& sgdh = *reinterpret_cast<ktxBasisGlobalHeader*>(bgd);
    sgdh.globalFlags = bfh.m_flags;
    sgdh.endpointCount = bfh.m_total_endpoints;
    sgdh.endpointsByteLength = bfh.m_endpoint_cb_file_size;
    sgdh.selectorCount = bfh.m_total_selectors;
    sgdh.selectorsByteLength = bfh.m_selector_cb_file_size;
    sgdh.tablesByteLength = bfh.m_tables_file_size;
    ...
    ktxBasisGlobalFullSliceDesc* kslice
                = reinterpret_cast<ktxBasisGlobalFullSliceDesc*>(&bgd[sizeof(sgdh)]);
    ...
    // Assumption here is that input slices are ordered by mip level.
    ...
    // Copy the endpoints
    memcpy(dstptr,
           &bf[bfh.m_endpoint_cb_file_ofs + sizeof(basis_file_header)],
           bfh.m_endpoint_cb_file_size);
    dstptr += sgdh.endpointsByteLength;
    // ... selectors
    memcpy(dstptr,
           &bf[bfh.m_selector_cb_file_ofs + sizeof(basis_file_header)],
           bfh.m_selector_cb_file_size);
    dstptr += sgdh.selectorsByteLength;
    // and the huffman tables.
    memcpy(dstptr,
           &bf[bfh.m_tables_file_ofs + sizeof(basis_file_header)],
           bfh.m_tables_file_size);

    assert((dstptr + sgdh.tablesByteLength - bgd) <= bgd_size);
    ...
    for (int32_t level = This->numLevels - 1; level >= 0; level--) {
        priv._levelIndex[level].byteOffset = levelOffset;
        // byteLength was set in loop above
        memcpy(This->pData + levelOffset,
               &bf[level_file_offsets[level] + sizeof(basis_file_header)],
               priv._levelIndex[level].byteLength);
        levelOffset += priv._levelIndex[level].byteLength;
    }
