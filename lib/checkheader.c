...
KTX_error_code
ktxCheckHeader1_(KTX_header* pHeader, KTX_supplemental_info* pSuppInfo) {
    ...
    if (pHeader->endianness == KTX_ENDIAN_REF_REV) {
        ...
        if (pHeader->glTypeSize != 1 && pHeader->glTypeSize != 2 && pHeader->glTypeSize != 4) {
    ...
    if ((pHeader->pixelWidth == 0) || (pHeader->pixelDepth > 0 && pHeader->pixelHeight == 0)) {
 ...
 */
KTX_error_code
ktxCheckHeader2_(KTX_header2* pHeader, KTX_supplemental_info* pSuppInfo) {
    ...
    if (pHeader->supercompressionScheme == KTX_SS_BASIS_LZ &&
        pHeader->vkFormat != VK_FORMAT_UNDEFINED) {
        return KTX_FILE_DATA_ERROR;
    }
