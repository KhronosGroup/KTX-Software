...
KTX_error_code
texImage1DCallback(int miplevel, int face, int width, int height, int depth,
    ...
    gl.glTexImage1D(cbData->glTarget + face, miplevel, cbData->glInternalformat, width, 0,
...
KTX_error_code
compressedTexImage1DCallback(int miplevel, int face, int width, int height, int depth,
                             ktx_uint64_t faceLodSize, void* pixels, void* userdata) {
    ktx_cbdata* cbData = (ktx_cbdata*)userdata;
    UNUSED(depth);
    UNUSED(height);
    ...
    gl.glCompressedTexImage1D(cbData->glTarget + face, miplevel, cbData->glInternalformat, width, 0,
...
KTX_error_code
texImage2DCallback(int miplevel, int face, int width, int height, int depth,
                   ktx_uint64_t faceLodSize, void* pixels, void* userdata) {
    ktx_cbdata* cbData = (ktx_cbdata*)userdata;
    UNUSED(depth);
    UNUSED(faceLodSize);
    ...
    glTexImage2D(cbData->glTarget + face, miplevel, cbData->glInternalformat, width,
                 cbData->numLayers == 0 ? (GLuint)height : cbData->numLayers, 0, cbData->glFormat,
...
KTX_error_code
compressedTexImage2DCallback(int miplevel, int face, int width, int height, int depth,
                             ktx_uint64_t faceLodSize, void* pixels, void* userdata) {
    ktx_cbdata* cbData = (ktx_cbdata*)userdata;
    GLenum glerror;
    ...
    if ((glerror == GL_INVALID_ENUM || glerror == GL_INVALID_VALUE) &&
        ...
        result = _ktxUnpackETC((GLubyte*)pixels, cbData->glInternalformat, width, height, &unpacked,
                               &format, &internalformat, &type, R16Formats, supportsSRGB);
        ...
        glTexImage2D(cbData->glTarget + face, miplevel, internalformat, width,
                     cbData->numLayers == 0 ? (GLuint)height : cbData->numLayers, 0, format, type,
                     unpacked);
...
KTX_error_code
texImage3DCallback(int miplevel, int face, int width, int height, int depth,
                   ktx_uint64_t faceLodSize, void* pixels, void* userdata) {
    ktx_cbdata* cbData = (ktx_cbdata*)userdata;
    UNUSED(faceLodSize);
    ...
    gl.glTexImage3D(cbData->glTarget + face, miplevel, cbData->glInternalformat, width, height,
                    cbData->numLayers == 0 ? (GLuint)depth : cbData->numLayers, 0, cbData->glFormat,
                    cbData->glType, pixels);
...
KTX_error_code
compressedTexImage3DCallback(int miplevel, int face, int width, int height, int depth,
                             ktx_uint64_t faceLodSize, void* pixels, void* userdata) {
    ktx_cbdata* cbData = (ktx_cbdata*)userdata;
    ...
    gl.glCompressedTexImage3D(cbData->glTarget + face, miplevel, cbData->glInternalformat, width,
                              height, cbData->numLayers == 0 ? (GLuint)depth : cbData->numLayers, 0,
                              (ktx_uint32_t)faceLodSize, pixels);
...
/* [loadGLTexture] */
KTX_error_code
ktxTexture_GLUploadPrivate(ktxTexture* This, ktx_glformatinfo* formatInfo, GLuint* pTexture,
    ...
    if (target == GL_TEXTURE_1D && ((This->isCompressed && (gl.glCompressedTexImage1D == NULL)) ||
                                    (!This->isCompressed && (gl.glTexImage1D == NULL)))) {
        return KTX_UNSUPPORTED_TEXTURE_TYPE;
    }

    /* Reject 3D texture if unsupported. */
    if (target == GL_TEXTURE_3D && ((This->isCompressed && (gl.glCompressedTexImage3D == NULL)) ||
 ...
 * @brief Create a GL texture object from a ktxTexture1 object.
 *
 * In order to ensure that the GL uploader is not linked into an application unless explicitly
