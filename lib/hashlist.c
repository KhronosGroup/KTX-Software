...
ktxHashList_ConstructCopy(ktxHashList* pHead, ktxHashList orig) {
    ...
    *pHead = NULL;
    for (; entry != NULL; entry = ktxHashList_Next(entry)) {
        (void)ktxHashList_AddKVPair(pHead, entry->key, entry->valueLen, entry->value);
    }
}
...
ktxHashList_Create(ktxHashList** ppHl) {
    ...
    if (hl == NULL) return KTX_OUT_OF_MEMORY;
 ...
 */
KTX_error_code
ktxHashList_AddKVPair(ktxHashList* pHead, const char* key, unsigned int valueLen,
                      const void* value) {
    if (pHead && key && (valueLen == 0 || value)) {
        unsigned int keyLen = (unsigned int)strlen(key) + 1;
        ktxKVListEntry* kv;

        if (keyLen == 1) return KTX_INVALID_VALUE; /* Empty string */
 ...
 */
KTX_error_code
ktxHashList_FindEntry(ktxHashList* pHead, const char* key, ktxHashListEntry** ppEntry) {
    if (pHead && key) {
        ktxKVListEntry* kv;
 ...
 */
KTX_error_code
ktxHashList_FindValue(ktxHashList* pHead, const char* key, unsigned int* pValueLen,
                      void** ppValue) {
 ...
 */
KTX_error_code
ktxHashList_Serialize(ktxHashList* pHead, unsigned int* pKvdLen, unsigned char** ppKvd) {
    if (pHead && pKvdLen && ppKvd) {
        ...
        if (bytesOfKeyValueData == 0) {
            ...
            *ppKvd = NULL;
            ...
            if (!sd) return KTX_OUT_OF_MEMORY;
            ...
            *ppKvd = sd;
            ...
            for (kv = *pHead; kv != NULL; kv = kv->hh.next) {
                ...
                *(ktx_uint32_t*)sd = keyValueLen;
                ...
                sd += kv->keyLen;
                if (kv->valueLen > 0) memcpy(sd, kv->value, kv->valueLen);
                sd += kv->valueLen;
...
int
sort_by_key_codepoint(ktxKVListEntry* a, ktxKVListEntry* b) {
...
ktxHashList_Deserialize(ktxHashList* pHead, unsigned int kvdLen, void* pKvd) {
    ...
    if (kvdLen == 0 || pKvd == NULL || pHead == NULL) return KTX_INVALID_VALUE;

    if (*pHead != NULL) return KTX_INVALID_OPERATION;
    ...
    while (result == KTX_SUCCESS && src < (char*)pKvd + kvdLen) {
        ...
        result = ktxHashList_AddKVPair(pHead, key, valueLen, valueLen > 0 ? value : NULL);
        if (result == KTX_SUCCESS) {
 ...
 */
KTX_error_code
ktxHashListEntry_GetKey(ktxHashListEntry* This, unsigned int* pKeyLen, char** ppKey) {
    if (pKeyLen && ppKey) {
 ...
 */
KTX_error_code
ktxHashListEntry_GetValue(ktxHashListEntry* This, unsigned int* pValueLen, void** ppValue) {
    if (pValueLen && ppValue) {
        ktxKVListEntry* kv = (ktxKVListEntry*)This;
