...
/** @internal */
static void
printFlagBitsJSON(uint32_t indent, const char* nl, uint32_t flags,
    ...
    if (!first) printf("%s", nl);
}

/** @internal */
bool
isKnownKeyValueUINT32(const char* key) {
    if (strcmp(key, "KTXdxgiFormat__") == 0) return true;
    if (strcmp(key, "KTXmetalPixelFormat") == 0) return true;
...
/** @internal */
bool
isKnownKeyValueString(const char* key) {
    if (strcmp(key, "KTXorientation") == 0) return true;
    if (strcmp(key, "KTXswizzle") == 0) return true;
    if (strcmp(key, "KTXwriter") == 0) return true;
    if (strcmp(key, "KTXwriterScParams") == 0) return true;
    if (strcmp(key, "KTXastcDecodeMode") == 0) return true;
...
/** @internal */
bool
isKnownKeyValue(const char* key) {
    if (isKnownKeyValueUINT32(key)) return true;
    if (isKnownKeyValueString(key)) return true;
    if (strcmp(key, "KTXglFormat") == 0) return true;
    if (strcmp(key, "KTXanimData") == 0) return true;
    if (strcmp(key, "KTXcubemapIncomplete") == 0) return true;
    return false;
}
...
printKVData(ktx_uint8_t* pKvd, ktx_uint32_t kvdLen) {
    ...
    if (kvDataHead == NULL) return;

    int entryIndex = 0;
    ktxHashListEntry* entry = kvDataHead;
    for (; entry != NULL && entryIndex < MAX_NUM_KVD_ENTRIES;
 ...
 */
void
printKVDataJSON(ktx_uint8_t* pKvd, ktx_uint32_t kvdLen, ktx_uint32_t base_indent,
                ktx_uint32_t indent_width, bool minified) {
    ...
    result = ktxHashList_Deserialize(&kvDataHead, kvdLen, pKvd);
    if (result != KTX_SUCCESS) {
        // Logging while printing JSON is not possible, we rely on the validation step to provide
        // meaningful errors fprintf(stdout, "Failed to parse or not enough memory to build list of
        // key/value pairs.\n");
        return;
    }
    ...
    ktxHashListEntry* entry = kvDataHead;
    bool firstPrint =
        true;  // Marks if the first print did not occur yet (first print != first entry)
    for (; entry != NULL && entryIndex < MAX_NUM_KVD_ENTRIES;
         entry = ktxHashList_Next(entry), ++entryIndex) {
        char* key;
        char* value;
        ...
        if (!value) {
            if (!isKnownKeyValue(key)) {
                ...
                if (!firstPrint) fprintf(stdout, ",%s", nl);
                firstPrint = false;
            ...
            if (strcmp(key, "KTXglFormat") == 0) {
                ...
                if (valueLen == sizeof(ktx_uint8_t)) {
                    ...
                    ktx_uint8_t faces = *value;
                    PRINT_INDENT(0, "\"%s\":%s{%s", key, space, nl)
                    PRINT_INDENT(1, "\"positiveX\":%s%s,%s", space,
                                 faces & 1u << 0u ? "true" : "false", nl)
                    PRINT_INDENT(1, "\"negativeX\":%s%s,%s", space,
                                 faces & 1u << 1u ? "true" : "false", nl)
                    PRINT_INDENT(1, "\"positiveY\":%s%s,%s", space,
                                 faces & 1u << 2u ? "true" : "false", nl)
                    PRINT_INDENT(1, "\"negativeY\":%s%s,%s", space,
                                 faces & 1u << 3u ? "true" : "false", nl)
                    PRINT_INDENT(1, "\"positiveZ\":%s%s,%s", space,
                                 faces & 1u << 4u ? "true" : "false", nl)
                    PRINT_INDENT(1, "\"negativeZ\":%s%s%s", space,
                                 faces & 1u << 5u ? "true" : "false", nl)
                    PRINT_INDENT_NOARG(0, "}")
                }
            } else if (isKnownKeyValueUINT32(key)) {
                if (valueLen == sizeof(ktx_uint32_t)) {
...
printKTXInfo2(ktxStream* stream, KTX_header* pHeader) {
    ...
    if (result != KTX_SUCCESS) {
        ...
        case KTX_UNSUPPORTED_FEATURE:
            ...
            break;
        default:;  // _ktxCheckHeader returns only the above 2 errors.
        }
        return;
    }
    ...
    for (uint32_t level = 0; level < levelCount; level++) {
        ...
        if (pHeader->endianness == KTX_ENDIAN_REF_REV) _ktxSwapEndian32(&faceLodSize, 1);
        if (nonArrayCubemap) {
 ...
 */
void
printBasisSGDInfo(ktx_uint8_t* bgd, ktx_uint64_t byteLength, ktx_uint32_t numImages) {
    ktxBasisLzGlobalHeader* bgdh = (ktxBasisLzGlobalHeader*)(bgd);
    if (byteLength < sizeof(ktxBasisLzGlobalHeader)) return;
    ...
    ktxBasisLzEtc1sImageDesc* slices =
        (ktxBasisLzEtc1sImageDesc*)(bgd + sizeof(ktxBasisLzGlobalHeader));
    for (ktx_uint32_t i = 0; i < numImages; i++) {
        if (byteLength <
            (i + 1) * sizeof(ktxBasisLzEtc1sImageDesc) + sizeof(ktxBasisLzGlobalHeader))
            break;
...
printKTX2Info2(ktxStream* stream, KTX_header2* pHeader) {
    const bool hasDFD = pHeader->dataFormatDescriptor.byteOffset != 0 &&
                        pHeader->dataFormatDescriptor.byteLength != 0;
    const bool hasKVD =
        pHeader->keyValueData.byteOffset != 0 && pHeader->keyValueData.byteLength != 0;
    const bool hasSGD = pHeader->supercompressionGlobalData.byteOffset != 0 &&
    ...
    if (levelIndex == NULL) return KTX_OUT_OF_MEMORY;
    ...
    if (hasDFD) {
        ...
        if (dfd == NULL) return KTX_OUT_OF_MEMORY;
    ...
    if (hasKVD) {
        ...
        if (kvd == NULL) return KTX_OUT_OF_MEMORY;
    ...
    if (hasSGD) {
        if (pHeader->supercompressionScheme == KTX_SS_BASIS_LZ) {
            ...
            if (sgd == NULL) return KTX_OUT_OF_MEMORY;
 ...
 */
KTX_error_code
printKTX2Info2JSON(ktxStream* stream, KTX_header2* pHeader, ktx_uint32_t base_indent,
                   ktx_uint32_t indent_width, bool minified) {
    if (minified) {
        base_indent = 0;
        indent_width = 0;
    ...
    const char* scSchemeStr = ktxSupercompressionSchemeString(pHeader->supercompressionScheme);
    if (strcmp(scSchemeStr, "Invalid scheme value") == 0 ||
        strcmp(scSchemeStr, "Vendor or reserved scheme") == 0)
        PRINT_INDENT(1, "\"supercompressionScheme\":%s%u%s", space,
                     (uint32_t)pHeader->supercompressionScheme, nl)
    else
    ...
    PRINT_INDENT(2, "\"byteOffset\":%s%" PRId64 ",%s", space,
                 pHeader->supercompressionGlobalData.byteOffset, nl);
    PRINT_INDENT(2, "\"byteLength\":%s%" PRId64 "%s", space,
                 pHeader->supercompressionGlobalData.byteLength, nl);
    PRINT_INDENT(1, "},%s", nl)
    ...
    for (ktx_uint32_t level = 0; level < numLevels; level++) {
        ...
        PRINT_INDENT(3, "\"uncompressedByteLength\":%s%" PRId64 "%s", space,
                     levelIndex[level].uncompressedByteLength, nl);
    ...
    if (hasDFD) {
        ...
        printDFDJSON(dfd, pHeader->dataFormatDescriptor.byteLength, base_indent + 1, indent_width,
                     minified);
        free(dfd);
        PRINT_INDENT_NOARG(0, "}")
    }
    ...
    if (hasKVD) {
        ...
        printKVDataJSON(kvd, pHeader->keyValueData.byteLength, base_indent + 1, indent_width,
                        minified);
        free(kvd);
        PRINT_INDENT_NOARG(0, "}")
    }
    ...
    if (hasSGD) {
        ...
        case KTX_SS_BASIS_LZ: {
            ...
            ktxBasisLzEtc1sImageDesc* slices =
                (ktxBasisLzEtc1sImageDesc*)(sgd + sizeof(ktxBasisLzGlobalHeader));
            for (ktx_uint32_t i = 0; i < numImages; i++) {
                if (sgdByteLength <
                    (i + 1) * sizeof(ktxBasisLzEtc1sImageDesc) + sizeof(ktxBasisLzGlobalHeader))
                    break;
                ...
                if (imageFlags == 0) {
                    ...
                    PRINT_INDENT(3, "\"imageFlags\":%s[%s", space, nl)
                    printFlagBitsJSON(LENGTH_OF_INDENT(4), nl, imageFlags,
                                      ktxBUImageFlagsBitString);
                    PRINT_INDENT(3, "],%s", nl)
                }

                PRINT_INDENT(3, "\"rgbSliceByteLength\":%s%u,%s", space,
                             slices[i].rgbSliceByteLength, nl)
                PRINT_INDENT(3, "\"rgbSliceByteOffset\":%s%u,%s", space,
                             slices[i].rgbSliceByteOffset, nl)
                PRINT_INDENT(3, "\"alphaSliceByteLength\":%s%u,%s", space,
                             slices[i].alphaSliceByteLength, nl)
                PRINT_INDENT(3, "\"alphaSliceByteOffset\":%s%u%s", space,
                             slices[i].alphaSliceByteOffset, nl)
 ...
 */
KTX_error_code
ktxPrintKTX2InfoJSONForStream(ktxStream* stream, ktx_uint32_t base_indent,
                              ktx_uint32_t indent_width, bool minified) {
    ktx_uint8_t ktx2_ident_ref[12] = KTX2_IDENTIFIER_REF;
    KTX_header2 header;
    KTX_error_code result;
    ...
    if (memcmp(header.identifier, ktx2_ident_ref, 12)) return KTX_UNKNOWN_FILE_FORMAT;
...
ktxPrintInfoForStdioStream(FILE* stdioStream) {
    ...
    if (stdioStream == NULL) return KTX_INVALID_VALUE;

    result = ktxFileStream_construct(&stream, stdioStream, KTX_FALSE);
    if (result == KTX_SUCCESS) result = ktxPrintInfoForStream(&stream);
    return result;
}
 ...
 */
KTX_error_code
ktxPrintKTX2InfoJSONForStdioStream(FILE* stdioStream, ktx_uint32_t base_indent,
                                   ktx_uint32_t indent_width, bool minified) {
    KTX_error_code result;
    ktxStream stream;
 ...
 */
KTX_error_code
ktxPrintKTX2InfoJSONForNamedFile(const char* const filename, ktx_uint32_t base_indent,
                                 ktx_uint32_t indent_width, bool minified) {
    FILE* file = NULL;
    ...
    KTX_error_code result =
        ktxPrintKTX2InfoJSONForStdioStream(file, base_indent, indent_width, minified);
 ...
 */
KTX_error_code
ktxPrintKTX2InfoJSONForMemory(const ktx_uint8_t* bytes, ktx_size_t size, ktx_uint32_t base_indent,
                              ktx_uint32_t indent_width, bool minified) {
    KTX_error_code result;
    ktxStream stream;
...
ktxPrintKTX2InfoTextForStdioStream(FILE* stdioStream) {
    ...
    result = ktxFileStream_construct(&stream, stdioStream, KTX_FALSE);
    if (result == KTX_SUCCESS) result = ktxPrintKTX2InfoTextForStream(&stream);
    return result;
}
