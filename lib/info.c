...
/** @internal */
static void
printFlagBitsJSON(uint32_t indent, const char* nl, uint32_t flags, const char* (*toStringFn)(uint32_t, bool)) {
    bool first = true;
    ...
    for (; entry != NULL && entryIndex < MAX_NUM_KVD_ENTRIES; entry = ktxHashList_Next(entry), ++entryIndex) {
        ...
        if (!value) {
            ...
            if (strcmp(key, "KTXglFormat") == 0) {
                ...
                fprintf(stdout, " [");
                for (ktx_uint32_t i = 0; i < valueLen; i++) fprintf(stdout, "%d%s", (int)value[i], i + 1 == valueLen ? "" : ", ");
                fprintf(stdout, "]\n");
            }
        }
    }
    ...
    for (uint32_t i = 0; i < 12 && idlen < sizeof(u8identifier); i++, idlen++) {
        ...
        if (identifier[i] == U'\xAB') {
            ...
            case '\r':
                ...
                break;
            default:
                nchars = snprintf(&u8identifier[idlen], sizeof(u8identifier) - idlen, json ? "\\u%04X" : "\\x%02X", identifier[i]);
                idlen += nchars - 1;
            }
        } else {
            u8identifier[idlen] = identifier[i];
...
printBasisSGDInfo(ktx_uint8_t* bgd, ktx_uint64_t byteLength, ktx_uint32_t numImages) {
    ...
    for (ktx_uint32_t i = 0; i < numImages; i++) {
        if (byteLength < (i + 1) * sizeof(ktxBasisLzEtc1sImageDesc) + sizeof(ktxBasisLzGlobalHeader)) break;
...
printKTX2Info2(ktxStream* stream, KTX_header2* pHeader) {
    const bool hasDFD = pHeader->dataFormatDescriptor.byteOffset != 0 && pHeader->dataFormatDescriptor.byteLength != 0;
    const bool hasKVD = pHeader->keyValueData.byteOffset != 0 && pHeader->keyValueData.byteLength != 0;
    const bool hasSGD = pHeader->supercompressionGlobalData.byteOffset != 0 && pHeader->supercompressionGlobalData.byteLength != 0;
...
printKTX2Info2JSON(ktxStream* stream, KTX_header2* pHeader, ktx_uint32_t base_indent, ktx_uint32_t indent_width, bool minified) {
    ...
    if (hasSGD) {
        ...
        case KTX_SS_BASIS_LZ: {
            ...
            for (ktx_uint32_t i = 0; i < numImages; i++) {
                if (sgdByteLength < (i + 1) * sizeof(ktxBasisLzEtc1sImageDesc) + sizeof(ktxBasisLzGlobalHeader)) break;
...
ktxPrintInfoForStream(ktxStream* stream) {
    ...
    if (result == KTX_SUCCESS) {
        ...
        if (fileType == KTX) {
            // Read rest of header.
            result = stream->read(stream, &header.ktx.endianness, KTX_HEADER_SIZE - sizeof(ktx_ident_ref));
            printKTXInfo2(stream, &header.ktx);
        } else {
            // Read rest of header.
            result = stream->read(stream, &header.ktx2.vkFormat, KTX2_HEADER_SIZE - sizeof(ktx2_ident_ref));
...
ktxPrintKTX2InfoJSONForStdioStream(FILE* stdioStream, ktx_uint32_t base_indent, ktx_uint32_t indent_width, bool minified) {
    ...
    result = ktxFileStream_construct(&stream, stdioStream, KTX_FALSE);
    if (result == KTX_SUCCESS) result = ktxPrintKTX2InfoJSONForStream(&stream, base_indent, indent_width, minified);
    return result;
}
 ...
 */
KTX_error_code
ktxPrintKTX2InfoJSONForMemory(const ktx_uint8_t* bytes, ktx_size_t size, ktx_uint32_t base_indent, ktx_uint32_t indent_width,
                              bool minified) {
    KTX_error_code result;
    ktxStream stream;
