...
defaultGLGetProcAddress(const char* proc) {
    ...
    if (pfnWglGetProcAddress) pfnGLProc = pfnWglGetProcAddress(proc);
    if (!pfnGLProc) {
        pfnGLProc = (PFNVOIDFUNCTION)GetProcAddress(ktxOpenGLModuleHandle, proc);
    }
    return pfnGLProc;
}
#elif MACOS || UNIX || IOS
...
PFNVOIDFUNCTION
defaultGLGetProcAddress(const char* proc) { return dlsym(ktxOpenGLModuleHandle, proc); }
#elif WEB
...
ktxFindOpenGL() {
    ...
    found =
        GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (LPCSTR)ktxFindOpenGL, &module);
    if (found) {
        if (GetProcAddress(module, "glGetError") != NULL) return module;
    }
    ...
    if (found) {
        ...
        pfnWglGetProcAddress = (PFNGLGETPROCADDRESS)GetProcAddress(module, "wglGetProcAddress");
        if (pfnWglGetProcAddress != NULL) return module;
    }
    return 0;
}
#endif
...
ktxLoadOpenGLLibrary(void) {
    if (openGLLoaded) return KTX_SUCCESS;
