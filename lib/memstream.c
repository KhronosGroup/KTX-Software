...
ktxMem_create(ktxMem** ppMem) {
    ...
    if (pNewMem) {
        ...
        if (result == KTX_SUCCESS) *ppMem = pNewMem;
        return result;
    } else {
...
ktxMem_expand(ktxMem* pMem, const ktx_size_t newsize) {
    ...
    if (new_alloc_size == pMem->alloc_size) return KTX_SUCCESS;
...
ktxMemStream_read(ktxStream* str, void* dst, const ktx_size_t count) {
    ...
    if (!str || (mem = str->data.mem) == 0) return KTX_INVALID_VALUE;

    newpos = mem->pos + count;
    /* The first clause checks for overflow. */
    if (newpos < mem->pos || (ktx_size_t)newpos > mem->used_size) return KTX_FILE_UNEXPECTED_EOF;
 ...
 */
static KTX_error_code
ktxMemStream_skip(ktxStream* str, const ktx_size_t count) {
    ktxMem* mem;
    ktx_off_t newpos;
 ...
 */
static KTX_error_code
ktxMemStream_write(ktxStream* str, const void* src, const ktx_size_t size, const ktx_size_t count) {
    ktxMem* mem;
    ...
    if (mem->robytes) return KTX_INVALID_OPERATION; /* read-only */

    new_size = mem->pos + (size * count);
    // if (new_size < mem->used_size)
    if ((ktx_off_t)new_size < mem->pos) return KTX_FILE_OVERFLOW;

    if (mem->alloc_size < new_size) {
        rc = ktxMem_expand(mem, new_size);
        if (rc != KTX_SUCCESS) return rc;
    }

    memcpy(mem->bytes + mem->pos, src, size * count);
    mem->pos += size * count;
    if (mem->pos > (ktx_off_t)mem->used_size) mem->used_size = mem->pos;
 ...
 */
static KTX_error_code
ktxMemStream_setpos(ktxStream* str, ktx_off_t pos) {
    if (!str) return KTX_INVALID_VALUE;

    assert(str->type == eStreamTypeMemory);

    if (pos < 0 || (ktx_size_t)pos > str->data.mem->alloc_size) return KTX_INVALID_OPERATION;
...
ktxMemStream_getdata(ktxStream* str, ktx_uint8_t** ppBytes) {
    if (!str || !ppBytes) return KTX_INVALID_VALUE;
...
ktxMemStream_getsize(ktxStream* str, ktx_size_t* pSize) {
    if (!str || !pSize) return KTX_INVALID_VALUE;
 ...
 */
KTX_error_code
ktxMemStream_construct_ro(ktxStream* str, const ktx_uint8_t* bytes, const ktx_size_t numBytes) {
    ktxMem* mem;
    KTX_error_code result = KTX_SUCCESS;

    if (!str || !bytes || numBytes == 0) return KTX_INVALID_VALUE;
