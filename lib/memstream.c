...
ktxMemStream_read(ktxStream* str, void* dst, const ktx_size_t count) {
    ...
    if (newpos < mem->pos || (ktx_uint32_t)newpos > mem->used_size) return KTX_FILE_UNEXPECTED_EOF;
...
ktxMemStream_setpos(ktxStream* str, ktx_off_t pos) {
    ...
    if (pos > (ktx_off_t)str->data.mem->alloc_size) return KTX_INVALID_OPERATION;
