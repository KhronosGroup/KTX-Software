...
static void
astcBlockDimensions(ktx_uint32_t block_size, uint32_t &block_x, uint32_t &block_y,
                    uint32_t &block_z) {
    switch (block_size) {
    case KTX_PACK_ASTC_BLOCK_DIMENSION_4x4:
        block_x = 4;
...
compressionWorkloadRunner(int threadCount, int threadId, void *payload) {
    ...
    astcenc_error error = astcenc_compress_image(work->context, work->image, &work->swizzle,
...
ktxTexture2_CompressAstcEx(ktxTexture2 *This, ktxAstcParams *params) {
    ...
    astcenc_error astc_error = astcenc_config_init(profile, block_size_x, block_size_y,
                                                   block_size_z, quality, flags, &astc_config);
