...
imageAllocate(uint32_t bitness, uint32_t dim_x, uint32_t dim_y, uint32_t dim_z) {
...
astcEncoderAction(const ktxAstcParams &params, const uint32_t *bdb) {
    ...
    if (transfer == KHR_DF_TRANSFER_SRGB && params.mode == KTX_PACK_ASTC_ENCODER_MODE_LDR)
...
static void
astcBlockDimensions(ktx_uint32_t block_size, uint32_t &block_x, uint32_t &block_y, uint32_t &block_z) {
    switch (block_size) {
    case KTX_PACK_ASTC_BLOCK_DIMENSION_4x4:
        block_x = 4;
        block_y = 4;
...
compressionWorkloadRunner(int threadCount, int threadId, void *payload) {
    ...
    astcenc_error error =
        astcenc_compress_image(work->context, work->image, &work->swizzle, work->data_out, work->data_len, threadId);
...
launchThreads(int threadCount, void (*func)(int, int, void *), void *payload) {
    ...
    for (int i = 0; i < threadCount; i++) {
        ...
        pthread_create(&(threadDescs[i].threadHandle), nullptr, launchThreadsHelper, (void *)&(threadDescs[i]));
    }
...
ktxTexture2_CompressAstcEx(ktxTexture2 *This, ktxAstcParams *params) {
    ...
    if (component_size != 1) return KTX_INVALID_OPERATION;  // Can only deal with 8-bit components at the moment
    ...
    astcBlockDimensions(params->blockDimension, block_size_x, block_size_y, block_size_z);
    ...
    astcenc_error astc_error = astcenc_config_init(profile, block_size_x, block_size_y, block_size_z, quality, flags, &astc_config);

    if (astc_error != ASTCENC_SUCCESS) return KTX_INVALID_OPERATION;
    ...
    for (int32_t level = This->numLevels - 1; level >= 0; level--) {
        ...
        for (uint32_t image = 0; image < levelImages; image++) {
            ...
            if (num_components == 1)
                input_image = unorm8x1ArrayToImage(This->pData + offset, width, height);
            else if (num_components == 2)
                input_image = unorm8x2ArrayToImage(This->pData + offset, width, height);
            else if (num_components == 3)
                input_image = unorm8x3ArrayToImage(This->pData + offset, width, height);
            else  // assume (num_components == 4)
                input_image = unorm8x4ArrayToImage(This->pData + offset, width, height);
    ...
    assert((transfer == KHR_DF_TRANSFER_SRGB ? KHR_DFDVAL(prototype->pDfd + 1, PRIMARIES) == KHR_DF_PRIMARIES_SRGB : true) &&
...
#define DECLARE_PROTECTED(n, t2) ktxTexture_protected &n = *(t2->_protected)
    ...
    memcpy(This->_private->_levelIndex, protoPriv._levelIndex, This->numLevels * sizeof(ktxLevelIndexEntry));
...
ktxTexture2_CompressAstc(ktxTexture2 *This, ktx_uint32_t quality) {
    ...
    if (quality >= KTX_PACK_ASTC_QUALITY_LEVEL_FASTEST) params.qualityLevel = KTX_PACK_ASTC_QUALITY_LEVEL_FASTEST;

    if (quality >= KTX_PACK_ASTC_QUALITY_LEVEL_FAST) params.qualityLevel = KTX_PACK_ASTC_QUALITY_LEVEL_FAST;

    if (quality >= KTX_PACK_ASTC_QUALITY_LEVEL_MEDIUM) params.qualityLevel = KTX_PACK_ASTC_QUALITY_LEVEL_MEDIUM;

    if (quality >= KTX_PACK_ASTC_QUALITY_LEVEL_THOROUGH) params.qualityLevel = KTX_PACK_ASTC_QUALITY_LEVEL_THOROUGH;

    if (quality >= KTX_PACK_ASTC_QUALITY_LEVEL_EXHAUSTIVE) params.qualityLevel = KTX_PACK_ASTC_QUALITY_LEVEL_EXHAUSTIVE;
