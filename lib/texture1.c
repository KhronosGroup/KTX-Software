        ...
        if (!(createFlags & KTX_TEXTURE_CREATE_SKIP_KVDATA_BIT)) {
            ...
            if (!(createFlags & KTX_TEXTURE_CREATE_RAW_KVDATA_BIT)) {
                ...
                if (result == KTX_SUCCESS) {
                    ...
                    count = sscanf(orientation, KTX_ORIENTATION3_FMT, &orient[0], &orient[1],
                                   &orient[2]);
 ...
 */
static KTX_error_code
ktxTexture1_constructFromNamedFile(ktxTexture1* This, const char* const filename,
                                   ktxTextureCreateFlags createFlags) {
    FILE* file;
    ktxStream stream;
    KTX_error_code result;
 ...
 */
static KTX_error_code
ktxTexture1_constructFromMemory(ktxTexture1* This, const ktx_uint8_t* bytes, ktx_size_t size,
                                ktxTextureCreateFlags createFlags) {
    ktxStream stream;
    KTX_error_code result;
 ...
 */
KTX_error_code
ktxTexture1_CreateFromStdioStream(FILE* stdioStream, ktxTextureCreateFlags createFlags,
                                  ktxTexture1** newTex) {
    KTX_error_code result;
    if (newTex == NULL) return KTX_INVALID_VALUE;
 ...
 */
KTX_error_code
ktxTexture1_CreateFromNamedFile(const char* const filename, ktxTextureCreateFlags createFlags,
                                ktxTexture1** newTex) {
    KTX_error_code result;
...
ktxTexture1_CreateFromMemory(const ktx_uint8_t* bytes, ktx_size_t size,
                             ktxTextureCreateFlags createFlags, ktxTexture1** newTex) {
    KTX_error_code result;
    if (newTex == NULL) return KTX_INVALID_VALUE;
 ...
 */
KTX_error_code
ktxTexture1_CreateFromStream(ktxStream* pStream, ktxTextureCreateFlags createFlags,
                             ktxTexture1** newTex) {
    KTX_error_code result;
    if (newTex == NULL) return KTX_INVALID_VALUE;
...
ktxTexture1_calcDataSizeLevels(ktxTexture1* This, ktx_uint32_t levels) {
    ...
    for (i = 0; i < levels; i++) {
        ktx_size_t levelSize =
            ktxTexture_calcLevelSize(ktxTexture(This), i, KTX_FORMAT_VERSION_ONE);
 ...
 */
KTX_error_code
ktxTexture1_GetImageOffset(ktxTexture1* This, ktx_uint32_t level, ktx_uint32_t layer,
...
ktxTexture1_IterateLevels(ktxTexture1* This, PFNKTXITERCB iterCb, void* userdata) {
    ...
    for (miplevel = 0; miplevel < This->numLevels; ++miplevel) {
        ...
        levelSize = (ktx_uint32_t)ktxTexture_calcLevelSize(ktxTexture(This), miplevel,
        ...
        result =
            iterCb(miplevel, 0, width, height, depth, levelSize, This->pData + offset, userdata);
...
KTX_error_code
ktxTexture1_SetImageFromMemory(ktxTexture1* This, ktx_uint32_t level, ktx_uint32_t layer,
...
KTX_error_code
ktxTexture1_SetImageFromStdioStream(ktxTexture1* This, ktx_uint32_t level, ktx_uint32_t layer,
