...
ktxTexture_construct(ktxTexture* This, ktxTextureCreateInfo* createInfo,
    ...
    if (createInfo->numLevels > 1) {
        GLuint max_dim =
            MAX(MAX(createInfo->baseWidth, createInfo->baseHeight), createInfo->baseDepth);
...
ktxTexture_constructFromStream(ktxTexture* This, ktxStream* pStream,
    ...
    assert(pStream->type == eStreamTypeFile || pStream->type == eStreamTypeMemory ||
 ...
 */
KTX_error_code
ktxTexture_CreateFromStream(ktxStream* pStream, ktxTextureCreateFlags createFlags,
                            ktxTexture** newTex) {
 ...
 */
KTX_error_code
ktxTexture_CreateFromStdioStream(FILE* stdioStream, ktxTextureCreateFlags createFlags,
                                 ktxTexture** newTex) {
    ktxStream stream;
    KTX_error_code result;
 ...
 */
KTX_error_code
ktxTexture_CreateFromNamedFile(const char* const filename, ktxTextureCreateFlags createFlags,
                               ktxTexture** newTex) {
    KTX_error_code result;
    ktxStream stream;
    FILE* file;
...
ktxTexture_CreateFromMemory(const ktx_uint8_t* bytes, ktx_size_t size,
                            ktxTextureCreateFlags createFlags, ktxTexture** newTex) {
    KTX_error_code result;
    ktxStream stream;
...
ktxTexture_IterateLevelFaces(ktxTexture* This, PFNKTXITERCB iterCb, void* userdata) {
    ...
    for (miplevel = 0; miplevel < This->numLevels; ++miplevel) {
        ...
        for (face = 0; face < innerIterations; ++face) {
            ...
            result = iterCb(miplevel, face, width, height, depth, faceLodSize, This->pData + offset,
                            userdata);
...
ktxTexture_layerSize(ktxTexture* This, ktx_uint32_t level, ktxFormatVersionEnum fv) {
    ...
    blockCountZ = ((This->baseDepth >> level) + prtctd->_formatSize.blockDepth - 1) /
                  prtctd->_formatSize.blockDepth;
 ...
 */
void
ktxTexture_rowInfo(ktxTexture* This, ktx_uint32_t level, ktx_uint32_t* numRows,
